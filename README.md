
# 工业时代 2：重构

<img src="https://img.shields.io/badge/Minecraft-1.21.1-green" alt="Minecraft 1.21.1">
<img src="https://img.shields.io/badge/NeoForge-21.1.238-orange" alt="NeoForge 21.1.238">
<img src="https://img.shields.io/badge/Version-21.1.47-blue" alt="Version 21.1.47">
<img src="https://img.shields.io/badge/License-AGPLv3-yellow" alt="License AGPLv3">

## 附属与联动

[IC++](https://github.com/HalfCooler/ICPlusPlus) 传送门

添加本模组 Gradle 依赖：在 `build.gradle`。

```gradle
repositories { mavenCentral() }
dependencies { implementation "io.github.halfcooler:ic2r-neoforge:{$ic2r_version}" }
```

## 写在前面

IC2 大概是死了。没有官方回复的开源，继任开发者仍在缺席。

但社区的呼声从未消停。玩家们等待了太久，等待一个公开的、能在高版本运行的、原汁原味的 IC2。

于是有人站了出来。[IC2CR](https://github.com/yu1745/ic2-fabric) 率先开源，声势不小。由此，众人拾柴，万物竞发。IC2R 便是其中之一。

我们的目标是：**在尽可能不改变原有功能与数值的前提下，将 IC2，以及部分声名远扬的附属联动模组，完整、现代化地带到 1.20.1、1.21.1 和更高版本。**

然而我们从未打算喧宾夺主。国内的舆论环境极差，我曾遭受恶意攻击和威胁。再次强调，**我们不是 IC2 官方或官方授权的继任者**。

## 与原版的差异

我们深知玩家选择本 mod 是为了还原游戏体验，因此对原版的改动极为克制。以下是部分由于原版 IC2 思维定势可能导致游戏体验受到影响的重大变更。

### 移除或大量修改的物品、方块与功能

1. **移除了物品形态的 UU 物质及其下游相关机器**。
    - IC2 Dev Team 早在 `2.8.222-ex112` 中便已删除物质生成机（`mass_fabricator`）的合成配方，遗留的 UU 物质复制功能已无存在意义。
    - 目前 UU 物质可复制物品与花销仍停留在 1.12.2，更新版本的物品无法复制。我们正在思考解决方案，但它可能会花费很长时间。
2. **移除了精炼铁锭**。
    - 这是 IC2C 的物品，并非 IC2Exp 的组成部分。烧铁变成精炼铁锭，它的职能是替代钢锭。
3. **移除了脚手架**。
    - 高版本 Minecraft 已经自带了脚手架，IC2 的脚手架功能重复且不必要。
    - 另外对于强化脚手架喷淋建筑泡沫变成防爆石的生成方式，我们回退为合成设计，即 8 个石头和 1 个高级合金。
4. **移除了管道与两个电动泵**。
    - 旧版管道功能不全且存在兼容性问题。
    - 我们删除管道，以鼓励玩家使用更现代的物流系统（如 AE2、Ender IO 等）来替代。
    - 今后我们可能会重新设计或直接借鉴 GT 管道。
5. **调整了锡、铅、铀矿石的生成参数**。
    - 原版锡矿偏向高山生成，而玩家对锡的需求远高于铅、且更倾向于探索深层矿脉。
    - 我们在保持锡总量略多于铅的前提下，优化了其竖直分布，使游戏节奏更为合理。
    - 在原数值的基础上普遍降低了生成数量，原数值设计将矿石生成过分密集。
6. **删除了工业钻石**。
    - 工业钻石是 IC 的物品（是的，你没看错，是 `工业 1` 的物品）。
    - 已全面使用原版钻石替代。

### 保留在本体 Mod 中添加的新物品、方块与功能

1. **添加FE能源转换器**。
    - 任意面可吸收 EU 能量并按 1:4 比例输出 FE。仅能单向转换。
2. **添加樱花树苗作物**。
    - 作物数值与其他树苗无异。
3. **添加采矿过滤升级**。
    - 放置在高级采矿机中，可更大量地调节采矿机的黑白名单。

## 版权声明

[IC2 Dev Team 引文](https://forum.industrial-craft.net/thread/9843-mc-1-7-ic%C2%B2-v-2-1-x-2-2-x-experimental/?postID=131008#post131008)

**本仓库的资源文件版权归属 IC2 Dev Team。**

仓库许可证为 GNU Affero General Public License v3.0（AGPLv3）。查看 [LICENSE](./LICENSE) 了解更多。

## 快速开始

请使用 Java 21 构建和编译本分支。Java 25 已测试兼容。

其他信息请查看 [Release](./release.md)。

```shell
gradlew build
gradlew runClient
gradlew runServer
gradlew syncVersion
```
