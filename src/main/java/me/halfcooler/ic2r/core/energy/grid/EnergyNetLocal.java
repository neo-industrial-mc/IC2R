...
public class EnergyNetLocal
{
	...
	void addTile(IEnergyTile ioTile, BlockPos pos)
	{
		...
		if ((prev = this.gridAdditionsMap.put(ioTile, change)) != null)
		{
			...
			{
				IC2R.log.warn(LogCategory.EnergyNet, "Tile %s was attempted to be queued twice for addition.", Util.toString(ioTile, this.getWorld(), pos));
			}
		} else
		{
	...
	void removeTile(IEnergyTile ioTile, BlockPos pos)
	{
		...
		if (addition != null)
		{
			...
			{
				IC2R.log.debug(LogCategory.EnergyNet, "Removing tile %s by cancelling a pending addition.", Util.toString(ioTile, this.getWorld(), pos));
			}
		...
		} else
		{
			...
			if (tile != null)
			{
				...
				if (EnergyNetSettings.logGridUpdatesVerbose)
				{
					IC2R.log.debug(LogCategory.EnergyNet, "Disabled tile %s.", Util.toString(ioTile, this.getWorld(), pos));
				}
			} else if (EnergyNetSettings.logGridUpdatesVerbose)
			{
				IC2R.log.warn(LogCategory.EnergyNet, "Missing tile %s.", Util.toString(ioTile, this.getWorld(), pos));
			}
		}
	}
	...
	void requeueAddition(GridChange change)
	{
		...
		if (EnergyNetSettings.logGridUpdatesVerbose)
		{
			IC2R.log.debug(LogCategory.EnergyNet, "Re-queued addition of %s (chunk not ready).", Util.toString(change.ioTile, this.getWorld(), change.pos));
		}
	}
