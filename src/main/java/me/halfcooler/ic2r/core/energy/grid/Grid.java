...
public class Grid
{
	...
	void add(Node node, Collection<Node> neighbors)
	{
		...
		{
			IC2R.log.debug(LogCategory.EnergyNet, "%d Add %s to %s neighbors: %s.", this.uid, node, this, neighbors);
		}
	...
	void remove(Node node)
	{
		if (EnergyNetSettings.logGridUpdatesVerbose)
		{
			IC2R.log.debug(LogCategory.EnergyNet, "%d Remove Node %s from %s with %d nodes.", this.uid, node, this, this.nodes.size());
		}
		...
		while (it.hasNext())
		{
			...
			if (neighbor.links.isEmpty() && neighbor.tile.removeExtraNode(neighbor))
			{
				if (EnergyNetSettings.logGridUpdatesVerbose)
				{
					IC2R.log.debug(LogCategory.EnergyNet, "%d Removing isolated extra node %s.", this.uid, neighbor);
				}
		...
		} else if (linkCount > 1 && node.nodeType == NodeType.Conductor)
		{
			...
			if (EnergyNetSettings.logGridUpdatesVerbose)
			{
				IC2R.log.debug(LogCategory.EnergyNet, "%d Neighbor connectivity (%d links, %d new grids):", this.uid, linkCount, gridCount);
				...
				for (int i = 0; i < linkCount; i++)
				{
					...
					if (nodes != null)
					{
						IC2R.log.debug(LogCategory.EnergyNet, "%d %d: %s: %s (%d).", this.uid, i, node.links.get(i).getNeighbor(node), nodes, nodes.size());
					} else
					{
						IC2R.log.debug(LogCategory.EnergyNet, "%d %d: %s contained in %d.", this.uid, i, node.links.get(i).getNeighbor(node), mapping[i]);
					}
				}
			}
				...
				if (connectedNodes != null)
				{
					...
					for (Node cNode : connectedNodes)
					{
						...
						if (needsExtraNode)
						{
							...
							if (EnergyNetSettings.logGridUpdatesVerbose)
							{
								IC2R.log.debug(LogCategory.EnergyNet, "%s Create extra Node %d for %s in grid %d.", this.uid, extraNode.uid, cNode, grid.uid);
							}
						...
						if (EnergyNetSettings.logGridUpdatesVerbose)
						{
							IC2R.log.debug(LogCategory.EnergyNet, "%d Move Node %s to grid %d.", this.uid, cNode, grid.uid);
						}
	...
	void merge(Grid grid, Map<Node, Node> nodeReplacements)
	{
		...
		if (EnergyNetSettings.logGridUpdatesVerbose)
		{
			IC2R.log.debug(LogCategory.EnergyNet, "%d Merge %s -> %s.", this.uid, grid, this);
		}
		...
		for (Node node : grid.nodes.values())
		{
			...
			if (node.nodeType != NodeType.Conductor)
			{
				for (Node node2 : node.tile.nodes)
				{
					if (node2.nodeType == node.nodeType && node2.getGrid() == this)
					{
						if (EnergyNetSettings.logGridUpdatesVerbose)
						{
							IC2R.log.debug(LogCategory.EnergyNet, "%d Merge Node %s -> %s.", this.uid, node, node2);
						}
			...
			if (!found)
			{
				if (EnergyNetSettings.logGridUpdatesVerbose)
				{
					IC2R.log.debug(LogCategory.EnergyNet, "%d Add Node %s.", this.uid, node);
				}
		...
		if (EnergyNetSettings.logGridUpdatesVerbose)
		{
			IC2R.log.debug(LogCategory.EnergyNet, "Remove %s.", grid);
		}
	...
	void dumpGraph()
	{
		...
		try (FileWriter out = new FileWriter("graph_" + this.uid + "_raw.txt"))
		{
			...
			{
				IC2R.log.debug(LogCategory.EnergyNet, e, "Graph saving failed.");
			}
