...
class ChangeHandler
{
	static boolean prepareSync(EnergyNetLocal enet, GridChange change)
	{
		...
		if (EnergyNet.instance.getWorld(ioTile) != world)
		{
			...
			{
				IC2R.log.warn(LogCategory.EnergyNet, "Tile %s had the wrong world in grid update (%s)", Util.toString(ioTile, enet.getWorld(), pos), type);
			}
		...
		} else if (type != GridChange.Type.REMOVAL && !EnergyNet.instance.getPos(ioTile).equals(pos))
		{
			if (EnergyNetSettings.logGridUpdateIssues)
			{
				IC2R.log.warn(LogCategory.EnergyNet, "Tile %s has the wrong position in grid update (%s)", Util.toString(ioTile, enet.getWorld(), pos), type);
			}
		...
		} else if (type != GridChange.Type.REMOVAL && !world.isLoaded(pos))
		{
			if (EnergyNetSettings.logGridUpdateIssues)
			{
				IC2R.log.warn(LogCategory.EnergyNet, "Tile %s was unloaded in grid update (%s)", Util.toString(ioTile, enet.getWorld(), pos), type);
			}
		...
		} else if (type != GridChange.Type.REMOVAL && ioTile instanceof BlockEntity && ((BlockEntity) ioTile).isRemoved())
		{
			if (EnergyNetSettings.logGridUpdateIssues)
			{
				IC2R.log.warn(LogCategory.EnergyNet, "Tile %s was invalidated in grid update (%s)", Util.toString(ioTile, enet.getWorld(), pos), type);
			}
		...
		} else
		{
			...
			{
				IC2R.log.debug(LogCategory.EnergyNet, "Considering tile %s for grid update (%s)", Util.toString(ioTile, enet.getWorld(), pos), type);
			}
	...
	static void applyAddition(EnergyNetLocal enet, IEnergyTile ioTile, BlockPos pos, List<IEnergyTile> subTiles, Collection<GridChange> pendingChanges)
	{
		if (enet.registeredIoTiles.containsKey(ioTile))
		{
			if (EnergyNetSettings.logGridUpdateIssues)
			{
				IC2R.log.warn(LogCategory.EnergyNet, "Tile %s is already registered", Util.toString(ioTile, enet.getWorld(), pos));
			}
		} else
		{
			...
			if (EnergyNetSettings.logGridUpdatesVerbose)
			{
				IC2R.log.debug(LogCategory.EnergyNet, "Adding tile %s.", Util.toString(ioTile, enet.getWorld(), pos));
			}
	...
	private static void addTileToGrids(EnergyNetLocal enet, Tile tile)
	{
		...
		for (Node node : tile.nodes)
		{
			if (EnergyNetSettings.logGridUpdatesVerbose)
			{
				IC2R.log.debug(LogCategory.EnergyNet, "Adding node %s.", node);
			}
			...
			if (neighbors.isEmpty())
			{
				if (EnergyNetSettings.logGridUpdatesVerbose)
				{
					IC2R.log.debug(LogCategory.EnergyNet, "Creating new grid for %s.", node);
				}
			...
			} else
			{
				switch (node.nodeType)
				{
					case Conductor:
						...
						for (Node neighbor : neighbors)
						{
							if (neighbor.nodeType == NodeType.Conductor || neighbor.links.isEmpty())
							{
								if (EnergyNetSettings.logGridUpdatesVerbose)
								{
									IC2R.log.debug(LogCategory.EnergyNet, "Using %s for %s with neighbors %s.", neighbor.getGrid(), node, neighbors);
								}
						...
						if (grid == null)
						{
							if (EnergyNetSettings.logGridUpdatesVerbose)
							{
								IC2R.log.debug(LogCategory.EnergyNet, "Creating new grid for %s with neighbors %s.", node, neighbors);
							}
								...
								if (neighbor.nodeType != NodeType.Conductor && !neighbor.links.isEmpty())
								{
									...
									for (int i = 0; i < it.previousIndex(); i++)
									{
										...
										if (neighbor2.tile == neighbor.tile && neighbor2.nodeType == neighbor.nodeType && neighbor2.getGrid() == grid)
										{
											if (EnergyNetSettings.logGridUpdatesVerbose)
											{
												IC2R.log.debug(LogCategory.EnergyNet, "Using neighbor node %s instead of %s.", neighbor2, neighbors);
											}
									...
									if (!found)
									{
										if (EnergyNetSettings.logGridUpdatesVerbose)
										{
											IC2R.log.debug(LogCategory.EnergyNet, "Creating new extra node for neighbor %s.", neighbor);
										}
					...
					case Source:
						...
						if (EnergyNetSettings.logGridUpdatesVerbose)
						{
							IC2R.log.debug(LogCategory.EnergyNet, "Neighbor groups detected for %s: %s.", node, neighborGroups);
						}
						...
						for (int i = 0; i < neighborGroups.size(); i++)
						{
							...
							} else
							{
								if (EnergyNetSettings.logGridUpdatesVerbose)
								{
									IC2R.log.debug(LogCategory.EnergyNet, "Creating new extra node for %s.", node);
								}
	...
	static void applyRemoval(EnergyNetLocal enet, IEnergyTile ioTile, BlockPos pos)
	{
		...
		if (tile == null)
		{
			if (EnergyNetSettings.logGridUpdateIssues)
			{
				IC2R.log.warn(LogCategory.EnergyNet, "Tile %s removal without registration", Util.toString(ioTile, enet.getWorld(), pos));
			}
		} else
		{
			if (EnergyNetSettings.logGridUpdatesVerbose)
			{
				IC2R.log.debug(LogCategory.EnergyNet, "Removing tile %s.", Util.toString(ioTile, enet.getWorld(), pos));
			}
