...
public class WindSim
{
	...
	private int windStrength = 5 + IC2R.random.nextInt(20);
	private int windDirection = IC2R.random.nextInt(360);

	public WindSim(Level world)
	{
		this.world = world;
		this.windHeightCoefficients = calculateCoefficients(IC2R.getWorldMaxHeight(world), IC2R.getSeaLevel(world));
	}
	...
	public void updateWind()
	{
		if ((this.windTicker++ & 127) == 0)
		{
			...
			if (IC2R.random.nextInt(100) < upChance)
			{
				this.windStrength++;
			} else if (IC2R.random.nextInt(100) < downChance)
			{
				this.windStrength--;
			}

			switch (IC2R.random.nextInt(3))
			{
				case 0:
