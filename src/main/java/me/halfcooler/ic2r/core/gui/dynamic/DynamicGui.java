...
public class DynamicGui<T extends Container> extends GuiDefaultBackground<DynamicContainer<T>>
{
	...
	private void initializeWidgets(Inventory playerInventory, GuiParser.ParentNode parentNode)
	{
		for (GuiParser.Node rawNode : parentNode.getNodes())
		{
			switch (rawNode.getType())
			{
				...
				case energygauge:
				{
					if (!(this.menu.base instanceof Ic2rTileEntity)
						|| !((Ic2rTileEntity) this.menu.base).hasComponent(Energy.class))
					{
					...
					parentNode.addElement(this, new EnergyGauge(this, node.x, node.y, (Ic2rTileEntity) this.menu.base, node.style));
					break;
				}
				case gauge:
				{
				...
				{
					if (!(this.menu.base instanceof Ic2rTileEntity)
						|| !((Ic2rTileEntity) this.menu.base).hasComponent(Fluids.class))
					{
					...
					Fluids fluids = ((Ic2rTileEntity) this.menu.base).getComponent(Fluids.class);
				...
				case fluidslot:
				{
					...
					parentNode.addElement(
						...
						TankFluidSlot.createFluidSlot(
							this, node.x, node.y, ((Ic2rTileEntity) this.menu.base).getComponent(Fluids.class).getFluidTank(node.name)
						)
					);
				}
			}
	...
	protected IClickHandler createEventSender(int event, String eventString)
	{
		if (this.isHandHeldGUI())
		{
			...
			if (eventString == null)
			{
				IC2R.log.warn(LogCategory.General, "HandHand inventory given numeric event rather than string");
			...
			return button ->
			{
				IC2R.network.get(false).sendContainerEvent(DynamicGui.this.menu, eventName);
