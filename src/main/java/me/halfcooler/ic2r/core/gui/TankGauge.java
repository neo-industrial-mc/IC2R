...
public class TankGauge extends GuiElement<TankGauge>
{
	...
	private TankGauge(Ic2rGui<?> gui, int x, int y, int width, int height, Ic2rFluidTank tank, TankGauge.TankGuiStyle style)
	{
	...
	public static TankGauge createNormal(Ic2rGui<?> gui, int x, int y, Ic2rFluidTank tank)
	{
		return new TankGauge(gui, x, y, 20, 55, tank, TankGauge.TankGuiStyle.Normal);
	}

	public static TankGauge createPlain(Ic2rGui<?> gui, int x, int y, int width, int height, Ic2rFluidTank tank)
	{
		return new TankGauge(gui, x, y, width, height, tank, TankGauge.TankGuiStyle.Plain);
	}

	public static TankGauge createBorderless(Ic2rGui<?> gui, int x, int y, Ic2rFluidTank tank, boolean mirrored)
	{
	...
	protected List<Component> getToolTip()
	{
		...
		if (fs != null && !fs.isEmpty())
		{
			...
			if (fluid != null)
			{
				ret.add(Component.literal(SideProxyClient.envProxy.getFluidName(fs) + ": " + fs.getAmountMb() + " " + Component.translatable("ic2r.generic.text.mb").getString()));
			} else
			{
		...
		{
			ret.add(Component.translatable("ic2r.generic.text.empty"));
		}
