...
public final class CommandIc2rc
{
	...
	public static void register(CommandDispatcher<CommandSourceStack> dispatcher)
	{
		dispatcher.register(
			Commands.literal("ic2c")
				...
				.then(Commands.literal("currentItem").executes(ctx -> CommandIc2r.cmdCurrentItem(ctx.getSource())))
		);
	}

	private static int cmdRightClick(CommandSourceStack source, int x, int y, int z, CommandIc2rc.ClickSide side) throws CommandSyntaxException
	{
		...
		CommandIc2r.msg(source, "Right click executed.");
		return 1;
	}

	private static CommandIc2rc.ClickSide parseSide(String name) throws CommandSyntaxException
	{
		try
		{
			return CommandIc2rc.ClickSide.valueOf(name);
