...
public class UuGraph
{
	...
	public static void build(boolean reset)
	{
		...
		IC2R.log.debug(LogCategory.Uu, "%d UU recipe transformations fetched after %d ms.", transformations.size(), (System.nanoTime() - startTime) / 1000000L);
		calculation = IC2R.threadPool.submit(() -> UuGraph.processRecipes(transformations));
	}
	...
	public static ItemStack find(ItemStack stack)
	{
		...
		{
			LeanItemStack search = new LeanItemStack(stack.getItem(), StackUtil.getTag(stack), StackUtil.getSize(stack));
	...
	private static void finishCalculation()
	{
		if (calculation != null)
		{
			...
			{
				IC2R.log.warn(LogCategory.Uu, e, "Calculation failed.");
				nodes.clear();
				itemNodes.clear();
			}
