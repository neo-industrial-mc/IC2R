...
class TeUpdate
{
	static final boolean debug = System.getProperty("ic2r.network.debug.teupdate") != null;
		...
		while (buffer.hasAvailable())
		{
			...
			for (int i = 0; i < fieldCount; i++)
			{
				...
				if (fieldName.equals("teBlk"))
				{
					...
					teData.teType = (Ic2rTileEntityBlock) BuiltInRegistries.BLOCK.get(identifier);
				} else
				{
			...
			if (teData.hasClass())
			{
				Class<? extends Ic2rTileEntity> teClass = teData.teType.getTeClass();
		...
		IC2R.sideProxy.requestTick(false, () ->
		{
			...
			if (world != null && Util.getDimId(world).equals(worldId))
			{
				for (TeUpdateDataClient.TeData update : updateData.getTes())
				{
					...
					{
						IC2R.log.warn(LogCategory.Network, t, "TE update at %s failed.", Util.formatPosition(world, update.pos));
					}
				}
			}
		});
	...
	private static void printDebugOutput(ResourceLocation dimensionId, TeUpdateDataClient data)
	{
		...
		IC2R.log.info(LogCategory.Network, "Received TE Update:\n" + out);
	}
	...
	private static void apply(TeUpdateDataClient.TeData update, Level world)
	{
		if (!world.hasChunkAt(update.pos))
		{
			if (debug)
			{
				IC2R.log.info(LogCategory.Network, "Skipping update at %s, chunk not loaded.", Util.formatPosition(world, update.pos));
			}
		} else
		{
			...
			} else
			{
				...
				if (update.hasClass() && (te == null || te.getClass() != update.teType.getTeClass() || te.isRemoved() || te.getLevel() != world))
				{
					if (debug)
					{
						IC2R.log.info(LogCategory.Network, "Instantiating %s with %s.", Util.formatPosition(world, update.pos), update.teType.getTeClass().getName());
					}
				...
				} else
				{
					if (te == null)
					{
						if (debug)
						{
							IC2R.log.info(LogCategory.Network, "Can't apply update at %s, no te and no teClass.", Util.formatPosition(world, update.pos));
						}
					...
					if (te.isRemoved() || te.getLevel() != world)
					{
						if (debug)
						{
							IC2R.log.warn(LogCategory.Network, "Can't apply update at %s, invalid te and no teClass.", Util.formatPosition(world, update.pos));
						}
					...
					if (debug)
					{
						IC2R.log.info(LogCategory.Network, "TE class at %s unchanged.", Util.formatPosition(world, update.pos));
					}
				}
