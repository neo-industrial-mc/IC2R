...
public enum SubPacketType
{
	...
	public static SubPacketType read(GrowingBuffer in, boolean simulating)
	{
		...
		if (id >= 0 && id < values.length)
		{
			...
			{
				IC2R.log.warn(LogCategory.Network, "Invalid sub packet type %s for side %s", ret.name(), simulating ? "server" : "client");
				return null;
			} else
			{
				return ret;
		...
		{
			IC2R.log.warn(LogCategory.Network, "Invalid sub packet type: %d", id);
			return null;
		}
	}
