...
public class NetworkManagerClient extends NetworkManager
{
	...
	private static void processChatPacket(GrowingBuffer buffer)
	{
		...
		IC2R.sideProxy.requestTick(false, () ->
		{
			...
			{
				IC2R.sideProxy.messagePlayer(null, line);
			}
		});
	}
...
final Object teDeferred = DataEncoder.decodeDeferred(is, BlockEntity.class);
					...
					case ExplosionEffect:
					{
						...
						Ic2rExplosion.Type type = DataEncoder.decodeEnum(is, Ic2rExplosion.Type.class);
						IC2R.sideProxy.requestTick(false, () ->
						{
					...
					case TileEntityBlockComponent:
					{
						...
						IC2R.sideProxy.requestTick(false, () ->
						{
							...
							if (Util.getDimId(world).equals(dimensionId))
							{
								...
								if (teRaw instanceof Ic2rTileEntity)
								{
									TileEntityComponent component = ((Ic2rTileEntity) teRaw).getComponent(componentCls);
									if (component != null)
									{
	...
	protected final void sendC2SPacket(GrowingBuffer buffer)
	{
		...
		if (handler != null)
		{
			PacketDistributor.sendToServer(Ic2rRawPayload.fromByteBuf(makePacket(buffer, true)));
		}
	}
}
