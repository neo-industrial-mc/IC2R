...
public class AdvRecipeGameTests {
    ...
    public static void hiddenRecipeRetainsDisplayIngredients(GameTestHelper helper) {
        Object loadedRecipe = helper
            .getLevel()
            .getServer()
            .getRecipeManager()
            .byKey(IC2R.getIdentifier("shaped/gunpowder"))
        ...
        helper.assertTrue(recipe.getIngredients().isEmpty(), "hidden ingredients should remain absent from the vanilla recipe book");
        ...
        for (int slot = 0; slot < displayIngredients.size(); slot++) {
            ItemStack expected = new ItemStack(slot % 2 == 0 ? Items.REDSTONE : Ic2rItems.COAL_DUST);
            helper.assertTrue(displayIngredients.get(slot).test(expected), "JEI display ingredient should match slot " + slot);
        }
