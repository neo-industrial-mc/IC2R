...
public class ContainerIndustrialWorkbench extends ContainerFullInv<TileEntityIndustrialWorkbench>
{
	...
	public ContainerIndustrialWorkbench(int syncId, Inventory playerInventory, TileEntityIndustrialWorkbench tileEntity)
	{
		...
		this.outputs[0] = this.addSlot(new Ic2rCraftingResultSlot(this.player, this.craftMatrix, this.craftResult, 0, 124, 61)
		{
			protected void checkTakeAchievements(@NotNull ItemStack stack)
			{
				...
				{
					IC2R.network.get(false).sendContainerEvent(ContainerIndustrialWorkbench.this, "craft");
				} else
				{
		...
		this.outputs[1] = this.addSlot(new Ic2rCraftingResultSlot(this.player, tileEntity.leftCrafting.crafting, tileEntity.leftCrafting.resultInv, 0, 69, 17));
		...
		this.outputs[2] = this.addSlot(new Ic2rCraftingResultSlot(this.player, tileEntity.rightCrafting.crafting, tileEntity.rightCrafting.resultInv, 0, 153, 17));
	...
	private CraftingRecipe getRecipe(CraftingContainer inventory)
	{
		...
		return server == null ? null : server.getRecipeManager().getRecipeFor(RecipeType.CRAFTING, inventory.asCraftInput(), world).map(net.minecraft.world.item.crafting.RecipeHolder::value).orElse(null);
	}
					...
					&& isValidTargetSlot(targetSlot, sourceItemStack, run == 1, false))
				{
					...
					if (StackUtil.isEmpty(sourceItemStack))
					{
						if (isOutput)
						{
							...
							Ic2rCraftingResultSlot outputSlot = (Ic2rCraftingResultSlot) craftingSlot;
