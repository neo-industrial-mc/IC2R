	...
	public TradingMarket(Level world)
	{
		...
		if (Util.inDev())
		{
			this.watchers.add(new TradingMarket.MarketWatcher()
			{
				...
				public void onAdd(BlockPos pos)
				{
					IC2R.log.info(LogCategory.Block, "Market registration at " + Util.formatPosition(world.getBlockEntity(pos)));
				}

				@Override
				public void onRemove(BlockPos pos)
				{
					IC2R.log.info(LogCategory.Block, "Market removal at " + Util.formatPosition(world.getBlockEntity(pos)));
				}
			});
		}
	}
