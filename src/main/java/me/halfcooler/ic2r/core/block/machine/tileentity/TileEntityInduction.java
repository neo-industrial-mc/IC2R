...
public class TileEntityInduction extends TileEntityElectricMachine implements IHasGui, IUpgradableBlock, IGuiValueProvider
{
	...
	{
		super(Ic2rBlockEntities.INDUCTION_FURNACE, pos, state, 10000, 2);
	...
	protected void updateEntityServer()
	{
		...
		if (newActive && this.progress != 0)
		{
			if (!canOperate || this.energy.getEnergy() < 15.0)
			{
				...
				newActive = false;
				IC2R.network.get(true).initiateTileEntityEvent(this, 1, true);
				this.shutdown(true);
			}
		} else if (canOperate)
		{
		...
		} else
		{
			if (needsInvUpdate)
			{
				IC2R.network.get(true).initiateTileEntityEvent(this, 3, true);
			}
	...
	{
		return Ic2rSoundEvents.MACHINE_FURNACE_INDUCTION_START;
	}

	@Override
	public SoundEvent getStopSoundEvent()
	{
		return Ic2rSoundEvents.MACHINE_FURNACE_INDUCTION_STOP;
	}

	@Override
	public SoundEvent getLoopingSoundEvent()
	{
		return Ic2rSoundEvents.MACHINE_FURNACE_INDUCTION_LOOP;
	}
