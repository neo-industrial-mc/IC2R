...
import me.halfcooler.ic2r.core.item.DamageHandler;
	...
	public ItemStack consume(int amount, boolean simulate, boolean consumeContainers)
	{
		...
		for (int i = 0; i < this.size(); i++)
		{
			...
			if (StackUtil.getSize(stack) >= 1
				...
				&& (StackUtil.getSize(stack) == 1 || consumeContainers || !IC2R.envProxy.hasRecipeRemainder(stack)))
			{
				...
				if (!simulate)
				{
					...
					{
						if (!consumeContainers && IC2R.envProxy.hasRecipeRemainder(stack))
						{
							ItemStack container = IC2R.envProxy.getRecipeRemainder(stack);
	...
	public int damage(int amount, boolean simulate, LivingEntity src)
	{
		...
		for (int i = 0; i < this.size() && amount > 0; i++)
		{
			...
			if (!StackUtil.isEmpty(stack))
			{
				...
				if (this.accepts(stack) && stack.isDamageableItem() && (target == null || item == target.getItem() && ItemStack.isSameItemSameComponents(stack, target)))
				{
					if (target == null)
					{
						target = stack.copy();
