...
public abstract class MachineRecipeHelper<RI, RO> implements IMachineRecipeManager<RI, RO, ItemStack>
{
	...
	public MachineRecipeResult<RI, RO, ItemStack> apply(ItemStack input, boolean acceptTest)
	{
		...
		if (IC2R.envProxy.hasRecipeRemainder(input)
			&& !StackUtil.isEmpty(adjustedInput = IC2R.envProxy.getRecipeRemainder(input))
			&& !acceptTest
	...
	protected void removeCachedRecipes(RI input)
	{
		...
		if (items != null)
		{
			for (Item item : items)
			{
				...
				if (recipes == null)
				{
					IC2R.log.warn(LogCategory.Recipe, "Inconsistent recipe cache, the entry for the item " + item + " is missing.");
				} else
				{
