		...
		if (!world.isClientSide && player != null && !player.isCrouching())
		{
			...
			if (te instanceof TileEntityCrop crop)
			{
				...
				if (ElectricItem.manager.use(StackUtil.get(player, context.getHand()), HandHeldCropAnalyzer.energyForLevel(2), player))
				{
					...
					player.sendSystemMessage(Component.translatable("ic2r.crop_analyzer.crop_name", Component.translatable(plant.getUnlocalizedName())));
					player.sendSystemMessage(Component.translatable("ic2r.crop.analyzer.crop_discovered_by", plant.getDiscoveredBy()));
					player.sendSystemMessage(Component.translatable("ic2r.crop_analyzer.crop_age", crop.getCurrentAge()));
					player.sendSystemMessage(Component.translatable("ic2r.crop_analyzer.crop_nutrients", crop.getStorageNutrients()));
					player.sendSystemMessage(Component.translatable("ic2r.crop_analyzer.crop_water", crop.getStorageWater()));
					player.sendSystemMessage(Component.translatable("ic2r.crop_analyzer.crop_weed_extra", crop.getStorageWeedEX()));
					player.sendSystemMessage(Component.translatable("ic2r.crop_analyzer.crop.growth_points", crop.getGrowthPoints(), plant.getGrowthDuration(crop)));
