...
public class ItemFrequencyTransmitter extends Item
{
	...
	public @NotNull InteractionResultHolder<ItemStack> use(@NotNull Level world, @NotNull Player player, @NotNull InteractionHand hand)
	{
		...
		if (IC2R.sideProxy.isSimulating())
		{
			...
			if (nbtData.getBoolean("targetSet") && !hadJustSet)
			{
				...
				IC2R.sideProxy.messagePlayer(player, "ic2r.frequency_transmitter.unlink_target");
			}
	...
	public @NotNull InteractionResult useOn(UseOnContext context)
	{
		...
		} else
		{
			...
			if (!targetSet)
			{
				...
				IC2R.sideProxy.messagePlayer(player, "ic2r.frequency_transmitter.link_target", target.getX(), target.getY(), target.getZ());
			} else if (tp.getBlockPos().equals(target))
			{
				IC2R.sideProxy.messagePlayer(player, "ic2r.frequency_transmitter.cannot_link_to_self");
			} else if (tp.hasTarget() && tp.getTarget().equals(target))
			{
				IC2R.sideProxy.messagePlayer(player, "ic2r.frequency_transmitter.link_already_established");
			} else
			{
	...
	public void appendHoverText(@NotNull ItemStack stack, Item.TooltipContext world, @NotNull List<Component> tooltip, @NotNull TooltipFlag advanced)
	{
		...
		{
			Ic2rTooltip.add(tooltip, Component.translatable("ic2r.frequency_transmitter.tooltip.target", nbtData.getInt("targetX"), nbtData.getInt("targetY"), nbtData.getInt("targetZ")));
		} else
		{
			Ic2rTooltip.add(tooltip, Component.translatable("ic2r.frequency_transmitter.tooltip.blank"));
		}
	}
}
