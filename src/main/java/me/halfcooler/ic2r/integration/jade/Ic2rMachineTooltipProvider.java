...
/**
 * Text lines for IC2R machines on Jade.
 * <ul>
 ...
 *   <li>Working / idle</li>
 * </ul>
 * Visibility of each line is controlled by {@code ic2r-client.toml} → {@code [jade.machine]}.
 * Item inventory and fluids stay on Jade's universal providers.
 */
public enum Ic2rMachineTooltipProvider implements IBlockComponentProvider, IServerDataProvider<BlockAccessor>
{
	INSTANCE;

	public static final ResourceLocation UID = IC2R.getIdentifier("machine");
	...
	{
		if (!(accessor.getBlockEntity() instanceof Ic2rTileEntity te))
		{
			return;
		}
	...
	public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config)
	{
		...
		if (data.getBoolean(KEY_STORAGE) && data.contains(KEY_REDSTONE_MODE) && JadeConfigHelper.redstoneMode().isVisible(details))
		{
			...
			{
				tooltip.add(Component.translatable("ic2r.EUStorage.gui.mod.redstone" + mode));
			}
		}

		if (data.contains(KEY_ACTIVE) && JadeConfigHelper.activeMode().isVisible(details))
		{
			tooltip.add(Component.translatable(data.getBoolean(KEY_ACTIVE) ? "ic2r.jade.working" : "ic2r.jade.idle"));
		}
	}
}
