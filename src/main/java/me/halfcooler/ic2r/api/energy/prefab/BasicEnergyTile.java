package me.halfcooler.ic2r.api.energy.prefab;
...
abstract class BasicEnergyTile implements ILocatable, IEnergyTile
{
	...
	public void onLoad()
	{
		if (!this.addedToEnet && !this.getWorldObj().isClientSide && Info.isIc2rAvailable())
		{
	...
	{
		if (stack != null && Info.isIc2rAvailable() && !this.getWorldObj().isClientSide)
		{
			double energyStored = this.getEnergyStored();
