...
public final class ElectrolyzerCategory extends IORecipeCategory<IElectrolyzerRecipeManager> implements IDrawable {
  ...
  public ElectrolyzerCategory(IGuiHelper guiHelper) {
    super((ITeBlock)TeBlock.electrolyzer, Recipes.electrolyzer);
    SlotPosition pos = new SlotPosition(78, 0);
    this.elements.add(new Tuple.T2(getFluidSlot(guiHelper), pos));
    this.inputSlots = Collections.singletonList(pos);
    List<SlotPosition> tempOutput = new ArrayList<>(5);
    for (int i = 0; i < 5; i++) {
      this.elements.add(new Tuple.T2(getFluidSlot(guiHelper), pos = new SlotPosition(36 + 21 * i + 0, 45)));
      tempOutput.add(pos);
    } 
    ...
    this.elements.add(new Tuple.T2(guiHelper.createDrawable(energyStyle.texture, energyStyle.uBgInactive, energyStyle.vBgInactive, energyStyle.bgWidth, energyStyle.bgHeight), pos));
    this.progress.add(new Tuple.T2(guiHelper.createAnimatedDrawable(drawableProperties(guiHelper, energyStyle), 300, getDirection(energyStyle.reverse, energyStyle.vertical), true), new SlotPosition(12, 28)));
    this.progress.add(new Tuple.T2(guiHelper.createAnimatedDrawable(drawableProperties(guiHelper, GuiElectrolyzer.ElectrolyzerGauges.THREE_TANK.properties), 100, IDrawableAnimated.StartDirection.TOP, false), new SlotPosition(60, 20)));
  }
  ...
  public void setRecipe(IRecipeLayout recipeLayout, IRecipeWrapper recipeWrapper, IIngredients ingredients) {
    ...
    List<SlotPosition> inputSlots = getInputSlotPos();
