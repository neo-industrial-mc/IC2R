...
public class SpecializedOps {
  public static DenseMatrix64F createReflector(RowD1Matrix64F u) {
    if (!MatrixFeatures.isVector((D1Matrix64F)u))
      throw new IllegalArgumentException("u must be a vector"); 
    double norm = NormOps.fastNormF((D1Matrix64F)u);
    double gamma = -2.0D / norm * norm;
    DenseMatrix64F Q = CommonOps.identity(u.getNumElements());
    CommonOps.multAddTransB(gamma, u, u, (RowD1Matrix64F)Q);
    return Q;
  }
  ...
  public static DenseMatrix64F createReflector(DenseMatrix64F u, double gamma) {
    if (!MatrixFeatures.isVector((D1Matrix64F)u))
      throw new IllegalArgumentException("u must be a vector"); 
    DenseMatrix64F Q = CommonOps.identity(u.getNumElements());
    CommonOps.multAddTransB(-gamma, (RowD1Matrix64F)u, (RowD1Matrix64F)u, (RowD1Matrix64F)Q);
    return Q;
  }
  ...
  public static double diffNormF(D1Matrix64F a, D1Matrix64F b) {
    ...
    return NormOps.normF((D1Matrix64F)diff);
  }
  ...
  public static DenseMatrix64F[] splitIntoVectors(RowD1Matrix64F A, boolean column) {
    ...
    for (int i = 0; i < w; i++) {
      ...
      if (column) {
        subvector(A, 0, i, o, false, 0, (RowD1Matrix64F)a);
      } else {
        subvector(A, i, 0, o, true, 0, (RowD1Matrix64F)a);
      } 
      ret[i] = a;
    } 
    return ret;
  ...
  public static DenseMatrix64F pivotMatrix(DenseMatrix64F ret, int[] pivots, int numPivots, boolean transposed) {
    if (ret == null) {
      ...
      CommonOps.fill((D1Matrix64F)ret, 0.0D);
    } 
    if (transposed) {
      for (int i = 0; i < numPivots; i++)
        ret.set(pivots[i], i, 1.0D); 
    } else {
      for (int i = 0; i < numPivots; i++)
        ret.set(i, pivots[i], 1.0D); 
    } 
    return ret;
  }
