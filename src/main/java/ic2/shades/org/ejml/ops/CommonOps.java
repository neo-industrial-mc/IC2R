...
public class CommonOps {
  public static void mult(RowD1Matrix64F a, RowD1Matrix64F b, RowD1Matrix64F c) {
    if (b.numCols == 1) {
      MatrixVectorMult.mult(a, (D1Matrix64F)b, (D1Matrix64F)c);
  ...
  public static void multTransA(RowD1Matrix64F a, RowD1Matrix64F b, RowD1Matrix64F c) {
    if (b.numCols == 1) {
      if (a.numCols >= EjmlParameters.MULT_COLUMN_SWITCH) {
        MatrixVectorMult.multTransA_reorder(a, (D1Matrix64F)b, (D1Matrix64F)c);
      } else {
        MatrixVectorMult.multTransA_small(a, (D1Matrix64F)b, (D1Matrix64F)c);
      } 
  ...
  public static void multAdd(RowD1Matrix64F a, RowD1Matrix64F b, RowD1Matrix64F c) {
    if (b.numCols == 1) {
      MatrixVectorMult.multAdd(a, (D1Matrix64F)b, (D1Matrix64F)c);
    } else if (b.numCols >= EjmlParameters.MULT_COLUMN_SWITCH) {
  ...
  public static void multAddTransA(RowD1Matrix64F a, RowD1Matrix64F b, RowD1Matrix64F c) {
    if (b.numCols == 1) {
      if (a.numCols >= EjmlParameters.MULT_COLUMN_SWITCH) {
        MatrixVectorMult.multAddTransA_reorder(a, (D1Matrix64F)b, (D1Matrix64F)c);
      } else {
        MatrixVectorMult.multAddTransA_small(a, (D1Matrix64F)b, (D1Matrix64F)c);
      } 
    } else if (a.numCols >= EjmlParameters.MULT_COLUMN_SWITCH || b.numCols >= EjmlParameters.MULT_COLUMN_SWITCH) {
  ...
  public static boolean solve(DenseMatrix64F a, DenseMatrix64F b, DenseMatrix64F x) {
    ...
    if (!linearSolverSafe.setA((Matrix64F)a))
      return false; 
    linearSolverSafe.solve((Matrix64F)b, (Matrix64F)x);
    return true;
  }
  
  public static void transpose(DenseMatrix64F mat) {
    if (mat.numCols == mat.numRows) {
      TransposeAlgs.square((RowD1Matrix64F)mat);
    } else {
  ...
  public static DenseMatrix64F transpose(DenseMatrix64F A, DenseMatrix64F A_tran) {
    ...
    if (A.numRows > EjmlParameters.TRANSPOSE_SWITCH && A.numCols > EjmlParameters.TRANSPOSE_SWITCH) {
      TransposeAlgs.block((RowD1Matrix64F)A, (RowD1Matrix64F)A_tran, EjmlParameters.BLOCK_WIDTH);
    } else {
      TransposeAlgs.standard((RowD1Matrix64F)A, (RowD1Matrix64F)A_tran);
    } 
    return A_tran;
  }
  ...
  public static double det(DenseMatrix64F mat) {
    ...
    if (numCol <= 6) {
      if (numCol >= 2)
        return UnrolledDeterminantFromMinor.det((RowD1Matrix64F)mat); 
      return mat.get(0);
    } 
  ...
  public static boolean invert(DenseMatrix64F mat) {
    if (mat.numCols <= 5) {
      ...
      if (mat.numCols >= 2) {
        ...
        mat.set(0, 1.0D / mat.get(0));
      } 
    } else {
      LUDecompositionAlt_D64 alg = new LUDecompositionAlt_D64();
      LinearSolverLu solver = new LinearSolverLu((LUDecompositionBase_D64)alg);
      if (solver.setA(mat)) {
        solver.invert(mat);
      } else {
        return false;
  ...
  public static void pinv(DenseMatrix64F A, DenseMatrix64F invA) {
    ...
    if (solver.modifiesA())
      A = A.copy(); 
    if (!solver.setA((Matrix64F)A))
      throw new IllegalArgumentException("Invert failed, maybe a bug?"); 
    solver.invert((Matrix64F)invA);
  }
  ...
  public static void setIdentity(RowD1Matrix64F mat) {
    ...
    Arrays.fill(mat.data, 0, mat.getNumElements(), 0.0D);
    int index = 0;
  ...
  public static void extract(ReshapeMatrix64F src, int srcY0, int srcY1, int srcX0, int srcX1, ReshapeMatrix64F dst, int dstY0, int dstX0) {
    ...
    if (src instanceof DenseMatrix64F && dst instanceof DenseMatrix64F) {
      ...
      ImplCommonOps_Matrix64F.extract((Matrix64F)src, srcY0, srcX0, (Matrix64F)dst, dstY0, dstX0, h, w);
    } 
  }
  ...
  public static void extractDiag(DenseMatrix64F src, DenseMatrix64F dst) {
    ...
    if (!MatrixFeatures.isVector((D1Matrix64F)dst))
  ...
  public static DenseMatrix64F rref(DenseMatrix64F A, int numUnknowns, DenseMatrix64F reduced) {
    ...
    rrefGaussJordanRowPivot.setTolerance(elementMaxAbs((D1Matrix64F)A) * UtilEjml.EPS * Math.max(A.numRows, A.numCols));
    reduced.set((D1Matrix64F)A);
    rrefGaussJordanRowPivot.reduce((Matrix64F)reduced, numUnknowns);
    return reduced;
  }
}
