...
public class BlockMatrixOps {
         ...
         for (int i = 0; i < dst.numRows; i += dst.blockLength) {
            ...
            for (int j = 0; j < dst.numCols; j += dst.blockLength) {
               ...
               for (int l = 0; l < blockWidth; l++) {
                  ...
                  for (int k = 0; k < blockHeight; rowDst += blockWidth) {
   ...
   private static void transposeBlock(BlockMatrix64F A, BlockMatrix64F A_tran, int indexA, int indexC, int width, int height) {
      for (int i = 0; i < height; i++) {
         ...
         for (int end = rowIndexA + width; rowIndexA < end; rowIndexA++) {
   ...
   public static boolean isEquals(BlockMatrix64F A, BlockMatrix64F B) {
      return A.blockLength != B.blockLength ? false : MatrixFeatures.isEquals(A, B);
   }

   public static boolean isEquals(BlockMatrix64F A, BlockMatrix64F B, double tol) {
      return A.blockLength != B.blockLength ? false : MatrixFeatures.isEquals(A, B, tol);
   }
   ...
   public static boolean blockAligned(int blockLength, D1Submatrix64F A) {
      if (A.col0 % blockLength != 0) {
         return false;
      } else if (A.row0 % blockLength != 0) {
         return false;
      } else {
         return A.col1 % blockLength != 0 && A.col1 != A.original.numCols ? false : A.row1 % blockLength == 0 || A.row1 == A.original.numRows;
      }
   }
}
