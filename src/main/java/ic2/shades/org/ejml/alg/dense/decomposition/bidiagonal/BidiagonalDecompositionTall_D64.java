...
public class BidiagonalDecompositionTall_D64 implements BidiagonalDecomposition<DenseMatrix64F> {
  ...
  public DenseMatrix64F getU(DenseMatrix64F U, boolean transpose, boolean compact) {
    ...
    if (compact) {
      ...
      DenseMatrix64F Q = (DenseMatrix64F)this.decompQRP.getQ((Matrix64F)U, false);
      DenseMatrix64F U1 = (DenseMatrix64F)this.decompBi.getU(null, false, true);
      ...
      CommonOps.insert((ReshapeMatrix64F)tmp, (ReshapeMatrix64F)Q, 0, 0);
    } 
    if (transpose)
  ...
  public DenseMatrix64F getV(DenseMatrix64F V, boolean transpose, boolean compact) {
    return (DenseMatrix64F)this.decompBi.getV((ReshapeMatrix64F)V, transpose, compact);
  }
  
  public boolean decompose(DenseMatrix64F orig) {
    if (!this.decompQRP.decompose((Matrix64F)orig))
      return false; 
    this.m = orig.numRows;
    this.n = orig.numCols;
    this.min = Math.min(this.m, this.n);
    this.B.reshape(this.min, this.n, false);
    this.decompQRP.getR((Matrix64F)this.B, true);
    ...
    return this.decompBi.decompose((Matrix64F)this.B);
  }
