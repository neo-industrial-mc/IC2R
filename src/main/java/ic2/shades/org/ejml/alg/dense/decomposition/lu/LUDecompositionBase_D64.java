...
public abstract class LUDecompositionBase_D64 implements LUDecomposition<DenseMatrix64F> {
  ...
  public DenseMatrix64F getLower(DenseMatrix64F lower) {
    ...
    if (lower == null) {
      ...
      CommonOps.fill((D1Matrix64F)lower, 0.0D);
    } 
    int i;
    for (i = 0; i < numCols; i++) {
      lower.set(i, i, 1.0D);
  ...
  public DenseMatrix64F getUpper(DenseMatrix64F upper) {
    ...
    if (upper == null) {
      ...
      if (upper.numCols != numCols || upper.numRows != numRows)
        throw new IllegalArgumentException("Unexpected matrix dimension"); 
      CommonOps.fill((D1Matrix64F)upper, 0.0D);
    } 
  ...
  public double computeDeterminant() {
    ...
    int i;
    for (i = 0; i < total; i += this.n + 1)
      ret *= this.dataLU[i]; 
    return ret;
  }
  
  public double quality() {
    return SpecializedOps.qualityTriangular(true, (D1Matrix64F)this.LU);
  }
