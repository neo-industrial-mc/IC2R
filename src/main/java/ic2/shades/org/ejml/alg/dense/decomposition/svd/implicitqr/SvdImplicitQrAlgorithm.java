...
public class SvdImplicitQrAlgorithm {
  ...
  private void performDynamicStep() {
    if (this.findingZeros) {
      if (this.steps > 6) {
        ...
        performImplicitSingleStep(scale, 0.0D, false);
      } 
    } else {
      double scale = computeBulgeScale();
  ...
  public void performImplicitSingleStep(double scale, double lambda, boolean byAngle) {
    createBulge(this.x1, lambda, scale, byAngle);
    for (int i = this.x1; i < this.x2 - 1 && this.bulge != 0.0D; i++) {
  ...
  protected void createBulge(int x1, double p, double scale, boolean byAngle) {
    ...
    if (this.Vt != null)
      updateRotator(this.Vt, x1, x1 + 1, this.c, this.s); 
  }
  ...
  protected void removeBulgeLeft(int x1, boolean notLast) {
    ...
    if (this.Ut != null)
      updateRotator(this.Ut, x1, x1 + 1, this.c, this.s); 
  }
  ...
  protected void removeBulgeRight(int x1) {
    ...
    if (this.Vt != null)
      updateRotator(this.Vt, x1 + 1, x1 + 2, this.c, this.s); 
  }
  ...
  private void pushRight(int row) {
    ...
    for (int i = 0; i < end && this.bulge != 0.0D; i++)
  ...
  private void rotatorPushRight(int m) {
    ...
    if (this.Ut != null)
      updateRotator(this.Ut, m, m + 1, this.c, this.s); 
  }
  ...
  private void rotatorPushRight2(int m, int offset) {
    ...
    if (this.Ut != null)
      updateRotator(this.Ut, m, m + offset, this.c, this.s); 
  }
  
  public void exceptionShift() {
    this.numExceptional++;
    double mag = 0.05D * this.numExceptional;
    if (mag > 1.0D)
      mag = 1.0D; 
    double angle = 6.283185307179586D * (this.rand.nextDouble() - 0.5D) * mag;
    performImplicitSingleStep(0.0D, angle, true);
  ...
  private SimpleMatrix createQ(int x1, int x2, double c, double s, boolean transposed) {
    SimpleMatrix Q = SimpleMatrix.identity(this.N);
    Q.set(x1, x1, c);
    if (transposed) {
      Q.set(x1, x2, s);
      Q.set(x2, x1, -s);
  ...
  private SimpleMatrix createB() {
    SimpleMatrix B = new SimpleMatrix(this.N, this.N);
  ...
  public void printMatrix() {
    ...
    for (j = 0; j < this.N - 1; j++) {
      System.out.printf("%5.2f ", new Object[] { Double.valueOf(this.off[j]) });
    } 
    System.out.println();
    System.out.print("    Diag[ ");
    for (j = 0; j < this.N; j++) {
      System.out.printf("%5.2f ", new Object[] { Double.valueOf(this.diag[j]) });
    } 
    System.out.println();
  }
