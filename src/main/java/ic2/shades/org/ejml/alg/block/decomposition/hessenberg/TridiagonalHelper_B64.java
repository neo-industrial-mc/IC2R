...
public class TridiagonalHelper_B64 {
  ...
  public static void computeW_row(int blockLength, D1Submatrix64F Y, D1Submatrix64F W, double[] beta, int betaIndex) {
    ...
    CommonOps.fill(W.original, 0.0D);
    ...
    for (int i = 1; i < min; i++) {
      ...
      for (int j = 0; j < i; j++) {
        ...
        BlockVectorOps.add_row(blockLength, W, i, 1.0D, W, j, b * yv, W, i, 1, Y.col1 - Y.col0);
      } 
      BlockHouseHolder.add_row(blockLength, Y, i, b, W, i, 1.0D, W, i, 1, Y.col1 - Y.col0);
    } 
  }
  ...
  public static void applyReflectorsToRow(int blockLength, D1Submatrix64F A, D1Submatrix64F V, int row) {
    ...
    for (int i = 0; i < row; i++) {
      ...
      double u_row = (i + 1 == row) ? 1.0D : dataA[indexU];
      double v_row = dataV[indexV];
      double before = A.get(i, i + 1);
      A.set(i, i + 1, 1.0D);
      BlockVectorOps.add_row(blockLength, A, row, 1.0D, V, i, u_row, A, row, row, A.col1 - A.col0);
      BlockVectorOps.add_row(blockLength, A, row, 1.0D, A, i, v_row, A, row, row, A.col1 - A.col0);
      A.set(i, i + 1, before);
    } 
  }
  ...
  public static void computeY(int blockLength, D1Submatrix64F A, D1Submatrix64F V, int row, double gamma) {
    ...
    for (int i = 0; i < row; i++) {
      ...
      BlockVectorOps.add_row(blockLength, V, row, 1.0D, A, i, dot_v_u, V, row, row + 1, A.col1 - A.col0);
      BlockVectorOps.add_row(blockLength, V, row, 1.0D, V, i, dot_u_u, V, row, row + 1, A.col1 - A.col0);
    } 
  ...
  public static double innerProdRowSymm(int blockLength, D1Submatrix64F A, int rowA, D1Submatrix64F B, int rowB, int zeroOffset) {
    ...
    if (offset + B.col0 >= B.col1)
      return 0.0D; 
    if (offset < rowB) {
      double d = B.get(offset, rowB);
      d += BlockVectorOps.dot_row_col(blockLength, A, rowA, B, rowB, offset + 1, rowB);
      d += BlockVectorOps.dot_row(blockLength, A, rowA, B, rowB, rowB, A.col1 - A.col0);
      return d;
    } 
  ...
  public static void computeRowOfV(int blockLength, D1Submatrix64F A, D1Submatrix64F V, int row, double gamma) {
    ...
    A.set(row, row + 1, 1.0D);
    BlockVectorOps.add_row(blockLength, V, row, 1.0D, A, row, -0.5D * gamma * val, V, row, row + 1, A.col1 - A.col0);
