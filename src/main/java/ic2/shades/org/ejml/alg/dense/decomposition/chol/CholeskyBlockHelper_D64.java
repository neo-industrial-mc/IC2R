...
class CholeskyBlockHelper_D64
{
    ...
    public CholeskyBlockHelper_D64(final int widthMax) {
        this.L = new DenseMatrix64F(widthMax, widthMax);
        this.el = this.L.data;
    }
    
    public boolean decompose(final DenseMatrix64F mat, final int indexStart, final int n) {
        final double[] m = mat.data;
        double div_el_ii = 0.0;
        for (int i = 0; i < n; ++i) {
            for (int j = i; j < n; ++j) {
                double sum = m[indexStart + i * mat.numCols + j];
                for (int iEl = i * n, jEl = j * n, end = iEl + i; iEl < end; ++iEl, ++jEl) {
