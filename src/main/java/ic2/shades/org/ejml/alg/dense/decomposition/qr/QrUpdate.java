...
public class QrUpdate {
  ...
  private void setQR(DenseMatrix64F Q, DenseMatrix64F R, int growRows) {
    ...
    if (this.m + growRows > this.maxRows || this.n > this.maxCols)
      if (this.autoGrow) {
        declareInternalData(this.m + growRows, this.n);
      } else {
  ...
  private void updateInsertQ(int rowIndex) {
    ...
    for (int k = 0; k < rowIndex; k++) {
      for (int m = 0; m < this.m_m; m++) {
        ...
        for (int n = 0; n < this.m; n++)
          sum += this.Qm.data[k * this.m + n] * this.U_tran.data[m * this.m_m + n + 1]; 
        this.Q.data[k * this.m_m + m] = sum;
      } 
    } 
    ...
    for (int i = rowIndex + 1; i < this.m_m; i++) {
      for (int m = 0; m < this.m_m; m++) {
        double sum = 0.0D;
        for (int n = 0; n < this.m; n++)
          sum += this.Qm.data[(i - 1) * this.m + n] * this.U_tran.data[m * this.m_m + n + 1]; 
        this.Q.data[i * this.m_m + m] = sum;
      } 
    } 
  }
  ...
  private void applyFirstGivens(double[] row) {
    ...
    CommonOps.setIdentity((RowD1Matrix64F)this.U_tran);
  ...
  private void computeRemoveGivens(int selectedRow) {
    ...
    for (int j = this.m - 2; j >= 0; j--) {
      double c, s, xi = this.Q.data[selectedRow * this.m + j];
      double r = xi * xi + xj * xj;
      if (r != 0.0D) {
        r = Math.sqrt(r);
        c = xi / r;
