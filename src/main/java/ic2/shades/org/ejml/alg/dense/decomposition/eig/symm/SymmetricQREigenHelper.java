...
public class SymmetricQREigenHelper {
  ...
  protected void performImplicitSingleStep(double lambda, boolean byAngle) {
    if (this.x2 - this.x1 == 1) {
      createBulge2by2(this.x1, lambda, byAngle);
    } else {
      createBulge(this.x1, lambda, byAngle);
      for (int i = this.x1; i < this.x2 - 2 && this.bulge != 0.0D; i++)
        removeBulge(i); 
      if (this.bulge != 0.0D)
  ...
  protected void createBulge(int x1, double p, boolean byAngle) {
    ...
    this.diag[x1] = this.c2 * a11 + 2.0D * this.cs * a12 + this.s2 * a22;
    this.diag[x1 + 1] = this.c2 * a22 - 2.0D * this.cs * a12 + this.s2 * a11;
    this.off[x1] = a12 * (this.c2 - this.s2) + this.cs * (a22 - a11);
    this.off[x1 + 1] = this.c * a23;
    this.bulge = this.s * a23;
    if (this.Q != null)
      updateQ(x1, x1 + 1, this.c, this.s); 
  }
  ...
  private void computeRotation(double run, double rise) {
    if (Math.abs(rise) > Math.abs(run)) {
      double k = run / rise;
      double bottom = 1.0D + k * k;
      double bottom_sq = Math.sqrt(bottom);
      this.s2 = 1.0D / bottom;
      this.c2 = k * k / bottom;
      this.cs = k / bottom;
      this.s = 1.0D / bottom_sq;
      this.c = k / bottom_sq;
    } else {
      double t = rise / run;
      double bottom = 1.0D + t * t;
      double bottom_sq = Math.sqrt(bottom);
      this.c2 = 1.0D / bottom;
      this.s2 = t * t / bottom;
      this.cs = t / bottom;
      this.c = 1.0D / bottom_sq;
      this.s = t / bottom_sq;
    } 
  }
  ...
  protected void removeBulge(int x1) {
    ...
    computeRotation(a12, this.bulge);
    this.diag[x1 + 1] = this.c2 * a22 + 2.0D * this.cs * a23 + this.s2 * a33;
    this.diag[x1 + 2] = this.c2 * a33 - 2.0D * this.cs * a23 + this.s2 * a22;
    ...
    if (this.Q != null)
      updateQ(x1 + 1, x1 + 2, this.c, this.s); 
  }
  ...
  public void exceptionalShift() {
    ...
    if (mag > 1.0D)
      mag = 1.0D; 
    double theta = 2.0D * (this.rand.nextDouble() - 0.5D) * mag;
