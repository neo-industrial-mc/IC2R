...
public class BidiagonalDecompositionRow_D64 implements BidiagonalDecomposition<DenseMatrix64F>
{
    ...
    public BidiagonalDecompositionRow_D64(final int numElements) {
    ...
    public static DenseMatrix64F handleB(DenseMatrix64F B, final boolean compact, final int m, final int n, final int min) {
        final int w = (n > m) ? (min + 1) : min;
        if (compact) {
            if (B == null) {
    ...
    public static DenseMatrix64F handleU(DenseMatrix64F U, final boolean transpose, final boolean compact, final int m, final int n, final int min) {
        if (compact) {
            if (transpose) {
                if (U == null) {
    ...
    @Override
    public DenseMatrix64F getV(DenseMatrix64F V, final boolean transpose, final boolean compact) {
    ...
    public static DenseMatrix64F handleV(DenseMatrix64F V, final boolean transpose, final boolean compact, final int m, final int n, final int min) {
        final int w = (n > m) ? (min + 1) : min;
        if (compact) {
            if (transpose) {
                if (V == null) {
    ...
    protected void computeU(final int k) {
        final double[] b = this.UBV.data;
        double max = 0.0;
        for (int i = k; i < this.m; ++i) {
            final double[] u = this.u;
            final int n = i;
            final double n2 = b[i * this.n + k];
            u[n] = n2;
            double val = n2;
            val = Math.abs(val);
            if (val > max) {
        ...
        if (max > 0.0) {
            final double tau = QrHelperFunctions.computeTauAndDivide(k, this.m, this.u, max);
            final double nu = this.u[k] + tau;
    ...
    protected void computeV(final int k) {
        final double[] b = this.UBV.data;
        final int row = k * this.n;
        final double max = QrHelperFunctions.findMax(b, row + k + 1, this.n - k - 1);
        if (max > 0.0) {
            final double tau = QrHelperFunctions.computeTauAndDivide(k + 1, this.n, b, row, max);
            final double nu = b[row + k + 1] + tau;
