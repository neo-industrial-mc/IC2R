...
public class WatchedDoubleStepQREigenvector
{
    ...
    public boolean process(final WatchedDoubleStepQREigen implicit, final DenseMatrix64F A, final DenseMatrix64F Q_h) {
        ...
        this.numSplits = 0;
        this.onscript = true;
        return this.findQandR() && this.extractVectors(Q_h);
    }
    
    public boolean extractVectors(final DenseMatrix64F Q_h) {
        UtilEjml.memset(this.eigenvectorTemp.data, 0.0);
        boolean triangular = true;
        for (int i = 0; i < this.N; ++i) {
            final Complex64F c = this.implicit.eigenvalues[this.N - i - 1];
        ...
        if (Q_h != null) {
            ...
            for (int j = 0; j < this.N; ++j) {
                final DenseMatrix64F v = this.eigenvectors[j];
                if (v != null) {
    ...
    private void solveEigenvectorDuplicateEigenvalue(final double real, final int first, final boolean isTriangle) {
        ...
        for (int i = first; i < this.N; ++i) {
            ...
            if (c.isReal() && Math.abs(c.real - real) / scale < 100.0 * UtilEjml.EPS) {
                ...
                final DenseMatrix64F v = new DenseMatrix64F(this.N, 1);
                CommonOps.multTransA(this.Q, this.eigenvectorTemp, v);
                NormOps.normalizeF(this.eigenvectors[this.N - i - 1] = v);
                this.eigenvectorTemp.data[i] = 0.0;
            }
        }
    }
    
    private void solveUsingTriangle(final double real, final int index, final DenseMatrix64F r) {
    ...
    private void solveWithLU(final double real, final int index, final DenseMatrix64F r) {
        final DenseMatrix64F A = new DenseMatrix64F(index, index);
    ...
    private void checkSplitPerformImplicit() {
        ...
        if (this.onscript) {
            if (this.implicit.steps > this.implicit.exceptionalThreshold / 2) {
                ...
                final Complex64F a = this.origEigenvalues[this.indexVal];
                if (a.isReal()) {
