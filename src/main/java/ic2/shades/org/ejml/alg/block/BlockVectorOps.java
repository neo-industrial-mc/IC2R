...
public class BlockVectorOps
{
	...
	public static void add_row(
		int blockLength, D1Submatrix64F A, int rowA, double alpha, D1Submatrix64F B, int rowB, double beta, D1Submatrix64F C, int rowC, int offset, int end
	)
	{
	...
	public static double dot_row_col(int blockLength, D1Submatrix64F A, int rowA, D1Submatrix64F B, int colB, int offset, int end)
	{
		...
		for (int i = startI; i < end; i += blockLength)
		{
			...
			if (i == startI)
			{
				...
				for (int j = offset; j < segment; indexB += widthB)
				{
					total += dataB[indexB] * dataA[indexA++];
					j++;
				}
			...
			{
				for (int j = 0; j < segment; indexB += widthB)
				{
					total += dataB[indexB] * dataA[indexA++];
					j++;
				}
