...
public class FermenterRecipeManager implements IFermenterRecipeManager {
  private final Map<Fluid, IFermenterRecipeManager.FermentationProperty> fluidMap =
      new IdentityHashMap<>();
  ...
  public void addRecipe(Fluid input, int inputAmount, int heat, Fluid output, int outputAmount) {
    ...
    this.fluidMap.put(
        input,
        new IFermenterRecipeManager.FermentationProperty(inputAmount, heat, output, outputAmount));
  }
