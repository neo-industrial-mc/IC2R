...
public class MatterAmplifierRecipeManager
  ...
  public boolean addRecipe(
      IRecipeInput input, Integer output, CompoundTag metadata, boolean replace) {
    if (output <= 0) {
  ...
  public MachineRecipeResult<IRecipeInput, Integer, ItemStack> apply(
      ItemStack input, boolean acceptTest) {
    MachineRecipe<IRecipeInput, Integer> recipe = this.getRecipe(input, acceptTest);
    return recipe == null
        ? null
        : recipe.getResult(StackUtil.copyShrunk(input, recipe.getInput().getAmount()));
  }
  ...
  private MachineRecipe<IRecipeInput, Integer> getRecipe(ItemStack stack, boolean acceptTest) {
    for (MachineRecipe<IRecipeInput, Integer> recipe : this.recipes) {
      if (recipe.getInput().matches(stack)
          && (acceptTest || recipe.getInput().getAmount() <= StackUtil.getSize(stack))) {
        return recipe;
      }
    }
