...
public class AdvShapelessRecipe implements CraftingRecipe
{
	...
	{
		private final MapCodec<AdvShapelessRecipe> codec = new MapCodec<>()
		{
			@Override
			...
			public <T> DataResult<AdvShapelessRecipe> decode(DynamicOps<T> ops, MapLike<T> input)
			{
				...
				{
					return DataResult.error(() -> "Failed to parse IC2 shapeless recipe: " + e.getMessage());
				}
			}

			@Override
			public <T> RecordBuilder<T> encode(AdvShapelessRecipe recipe, DynamicOps<T> ops, RecordBuilder<T> prefix)
			{
				return prefix.withErrorsFrom(DataResult.error(() -> "IC2 shapeless recipes cannot be encoded"));
			}
		};
		private final StreamCodec<RegistryFriendlyByteBuf, AdvShapelessRecipe> streamCodec = StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);
		...
		private static AdvShapelessRecipe fromNetwork(RegistryFriendlyByteBuf buf)
		{
			...
			return new AdvShapelessRecipe(null, inputs, ItemStack.STREAM_CODEC.decode(buf), buf.readBoolean(), buf.readBoolean());
		}
