...
public abstract class MachineRecipeHelper<RI, RO>
  ...
  protected boolean consumeContainer(
      ItemStack input, ItemStack container, MachineRecipe<RI, RO> recipe) {
    return false;
  }
  ...
  public MachineRecipeResult<RI, RO, ItemStack> apply(ItemStack input, boolean acceptTest) {
    ...
    if (IC2.envProxy.hasRecipeRemainder(input)
      ...
      adjustedInput =
          StackUtil.copyWithSize(input, StackUtil.getSize(input) - recipeInput.getAmount());
    }
      ...
      public Iterator<MachineRecipe<RI, RO>> iterator() {
        return new Iterator<>() {
          private final Iterator<MachineRecipe<RI, RO>> recipeIt =
              MachineRecipeHelper.this.recipes.values().iterator();
  ...
  private void addToCache(Item item, MachineRecipe<RI, RO> recipe) {
    List<MachineRecipe<RI, RO>> recipes =
        this.recipeCache.computeIfAbsent(item, k -> new ArrayList<>());
