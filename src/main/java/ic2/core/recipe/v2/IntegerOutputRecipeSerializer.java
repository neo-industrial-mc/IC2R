...
public class IntegerOutputRecipeSerializer implements RecipeSerializer<RecipeHolder<IRecipeInput, Integer>>
{
	...
	private final MapCodec<RecipeHolder<IRecipeInput, Integer>> codec = new MapCodec<>()
	{
		@Override
		...
		public <T> DataResult<RecipeHolder<IRecipeInput, Integer>> decode(DynamicOps<T> ops, MapLike<T> input)
		{
			...
			{
				return DataResult.error(() -> "Failed to parse IC2 integer output recipe: " + e.getMessage());
			}
		}

		@Override
		public <T> RecordBuilder<T> encode(RecipeHolder<IRecipeInput, Integer> recipe, DynamicOps<T> ops, RecordBuilder<T> prefix)
		{
			return prefix.withErrorsFrom(DataResult.error(() -> "IC2 integer output recipes cannot be encoded"));
		}
	};
	...
	private RecipeHolder<IRecipeInput, Integer> fromJson(JsonObject json)
	{
		...
		return new RecipeHolder<>(new MachineRecipe<>(input, output, meta), null, this, this.recipeType);
	}
	...
	private RecipeHolder<IRecipeInput, Integer> fromNetwork(RegistryFriendlyByteBuf buf)
	{
		...
		{
			return new RecipeHolder<>(new MachineRecipe<>(RecipeIo.readInput(buf), RecipeIo.readIntegerOutput(buf), buf.readNbt()), null, this, this.recipeType);
		}
	}
