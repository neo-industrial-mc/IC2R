...
public class BasicMachineRecipeSerializer implements RecipeSerializer<RecipeHolder<IRecipeInput, Collection<ItemStack>>>
{
	...
	private final MapCodec<RecipeHolder<IRecipeInput, Collection<ItemStack>>> codec = new MapCodec<>()
	{
		...
		{
			return prefix.withErrorsFrom(DataResult.error(() -> "IC2 machine recipes cannot be encoded"));
		}
	};
	...
	private RecipeHolder<IRecipeInput, Collection<ItemStack>> fromNetwork(RegistryFriendlyByteBuf buf)
	{
		...
		{
			return new RecipeHolder<>(new MachineRecipe<>(RecipeIo.readInput(buf), RecipeIo.readOutput(buf), buf.readNbt()), null, this, this.recipeType);
		}
	}
