...
public class CannerEnrichRecipeSerializer
  ...
  private final MapCodec<RecipeHolder<ICannerEnrichRecipeManager.Input, Ic2FluidStack>> codec =
      new MapCodec<>() {
        @Override
        public <T> Stream<T> keys(DynamicOps<T> ops) {
          return Stream.of("input_ingredient", "additive_ingredient", "result")
              .map(ops::createString);
        }

        @Override
        public <T> DataResult<RecipeHolder<ICannerEnrichRecipeManager.Input, Ic2FluidStack>> decode(
          ...
          try {
            ...
            return DataResult.error(
                () -> "Failed to parse IC2 canner enrich recipe: " + e.getMessage());
          }
        }

        @Override
        public <T> RecordBuilder<T> encode(
            RecipeHolder<ICannerEnrichRecipeManager.Input, Ic2FluidStack> recipe,
            DynamicOps<T> ops,
            RecordBuilder<T> prefix) {
          return prefix.withErrorsFrom(
              DataResult.error(() -> "IC2 canner enrich recipes cannot be encoded"));
        }
      };
  private final StreamCodec<
          RegistryFriendlyByteBuf, RecipeHolder<ICannerEnrichRecipeManager.Input, Ic2FluidStack>>
  ...
  private RecipeHolder<ICannerEnrichRecipeManager.Input, Ic2FluidStack> fromJson(JsonObject json) {
    Ic2FluidStack input =
        RecipeIo.parseFluidStack(GsonHelper.getAsJsonObject(json, "input_ingredient"));
    ...
    return new RecipeHolder<>(
        new MachineRecipe<>(new ICannerEnrichRecipeManager.Input(input, additive), result),
        null,
        this,
        Ic2RecipeTypes.CANNER_ENRICH);
  }

  private RecipeHolder<ICannerEnrichRecipeManager.Input, Ic2FluidStack> fromNetwork(
