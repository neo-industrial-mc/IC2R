...
{
	private final MapCodec<RecipeHolder<ICannerBottleRecipeManager.Input, ItemStack>> codec = new MapCodec<>()
	{
		@Override
		public <T> Stream<T> keys(DynamicOps<T> ops)
		{
			return Stream.of("container_ingredient", "fill_ingredient", "result").map(ops::createString);
		}
		...
		public <T> DataResult<RecipeHolder<ICannerBottleRecipeManager.Input, ItemStack>> decode(DynamicOps<T> ops, MapLike<T> input)
		{
			...
			{
				return DataResult.error(() -> "Failed to parse IC2 canner bottle recipe: " + e.getMessage());
			}
		}

		@Override
		public <T> RecordBuilder<T> encode(RecipeHolder<ICannerBottleRecipeManager.Input, ItemStack> recipe, DynamicOps<T> ops, RecordBuilder<T> prefix)
		{
			return prefix.withErrorsFrom(DataResult.error(() -> "IC2 canner bottle recipes cannot be encoded"));
		}
	};
	...
	private RecipeHolder<ICannerBottleRecipeManager.Input, ItemStack> fromJson(JsonObject json)
	{
		...
		return new RecipeHolder<>(new MachineRecipe<>(new ICannerBottleRecipeManager.Input(container, fill), output), null, this, Ic2RecipeTypes.CANNER_BOTTLE);
	}

	private RecipeHolder<ICannerBottleRecipeManager.Input, ItemStack> fromNetwork(RegistryFriendlyByteBuf buf)
	{
	...
	private void toNetwork(RegistryFriendlyByteBuf buf, RecipeHolder<ICannerBottleRecipeManager.Input, ItemStack> recipe)
	{
