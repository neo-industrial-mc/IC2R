...
public class ParticleUtil {
   ...
   public static void showFlames(World world, BlockPos pos, EnumFacing facing) {
      if (world.rand.nextInt(8) == 0) {
         ...
         double y = pos.getY() + world.rand.nextFloat() * 0.625;
         ...
         if (mc.gameSettings.particleSetting <= 1 && (mc.gameSettings.particleSetting != 1 || rnd.nextInt(3) != 0)) {
            if (!(mc.player.getDistanceSq(x, y, z) > 1024.0)) {
               ...
               for (int i = 0; i < count; i++) {
                  ...
                  ParticleDigging particle = newParticleBlockDust(world, x, y, z, mx, my, mz, state);
                  ensureTexture(world, pos, particle.init(), state);
   ...
   public static void spawnBlockRunParticles(World world, BlockPos pos, double x, double y, double z, double xSpeed, double zSpeed, ITeBlock block) {
      ...
      ParticleDigging particle = newParticleDigging(world, x, y, z, xSpeed, 1.5, zSpeed, state);
      ensureTexture(world, pos, particle.init(), state);
   ...
   public static void spawnBlockHitParticles(TileEntityBlock te, EnumFacing side, boolean checkTexture) {
      ...
      double x = pos.getX()
         + world.rand.nextDouble() * (aabb.maxX - aabb.minX - offset * 2.0)
         + offset
         + aabb.minX;
      double y = pos.getY()
         + world.rand.nextDouble() * (aabb.maxY - aabb.minY - offset * 2.0)
         + offset
         + aabb.minY;
      double z = pos.getZ()
         + world.rand.nextDouble() * (aabb.maxZ - aabb.minZ - offset * 2.0)
         + offset
         + aabb.minZ;
      switch (side) {
         case DOWN:
      ...
      ParticleDigging particle = newParticleDigging(world, x, y, z, 0.0, 0.0, 0.0, te.getBlockState());
   ...
   public static void spawnBlockBreakParticles(TileEntityBlock te) {
      ...
      for (int x = 0; x < 4; x++) {
         for (int y = 0; y < 4; y++) {
            for (int z = 0; z < 4; z++) {
               double xOffset = (x + 0.5) / 4.0;
               double yOffset = (y + 0.5) / 4.0;
               double zOffset = (z + 0.5) / 4.0;
               ParticleDigging particle = newParticleDigging(
                  world,
                  pos.getX() + xOffset,
                  pos.getY() + yOffset,
                  pos.getZ() + zOffset,
   ...
   public static ParticleBlockDust newParticleBlockDust(
      World world, double xCoord, double yCoord, double zCoord, double xSpeed, double ySpeed, double zSpeed, IBlockState state
   ) {
      try {
   ...
   public static ParticleDigging newParticleDigging(
      World world, double xCoord, double yCoord, double zCoord, double xSpeed, double ySpeed, double zSpeed, IBlockState state
   ) {
      try {
   ...
   private static Constructor<ParticleBlockDust> getParticleBlockDustCtor() {
      try {
         Constructor<ParticleBlockDust> ret = ParticleBlockDust.class
            .getDeclaredConstructor(World.class, double.class, double.class, double.class, double.class, double.class, double.class, IBlockState.class);
   ...
   private static Constructor<ParticleDigging> getParticleDiggingCtor() {
      try {
         Constructor<ParticleDigging> ret = ParticleDigging.class
            .getDeclaredConstructor(World.class, double.class, double.class, double.class, double.class, double.class, double.class, IBlockState.class);
         ret.setAccessible(true);
         return ret;
      } catch (Exception e) {
