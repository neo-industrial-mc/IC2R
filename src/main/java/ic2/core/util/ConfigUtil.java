...
public class ConfigUtil {
  ...
  private static IRecipeInput asRecipeInput(String str, boolean checkAmount) throws ParseException {
    ...
    for (int i = 1; i < parts.length; i++) {
      ...
      if (tmp.startsWith("@")) {
        if (i + 1 < parts.length && parts[i + 1].equals("*")) {
          meta = Integer.valueOf(32767);
          i++;
        } else {
          meta = Integer.valueOf(Integer.parseInt(tmp.substring(1)));
        } 
      } else if (tmp.startsWith("*")) {
        if (!checkAmount)
          throw new ParseException("We do not support amount here.", -1); 
        amount = Integer.valueOf(Integer.parseInt(tmp.substring(1)));
      } 
    } 
    if (itemName.startsWith("OreDict:")) {
      ...
      if (meta == null)
        return Recipes.inputFactory.forOreDict(itemName.substring("OreDict:".length()), amount.intValue()); 
      return Recipes.inputFactory.forOreDict(itemName.substring("OreDict:".length()), amount.intValue(), meta.intValue());
    } 
    if (itemName.startsWith("Fluid:")) {
      if (amount == null)
        amount = Integer.valueOf(1000); 
      return Recipes.inputFactory.forFluidContainer(FluidRegistry.getFluid(itemName.substring("Fluid:".length())), amount.intValue());
    } 
    return null;
  }
