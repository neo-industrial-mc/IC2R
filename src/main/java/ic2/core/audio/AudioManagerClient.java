...
public final class AudioManagerClient extends AudioManager {
  ...
  public void initialize() {
    ...
    IC2.log.debug(LogCategory.Audio, "Using %d audio sources.", new Object[] { Integer.valueOf(this.maxSourceCount) });
    ...
    if (player == null) {
      ...
      if (!isPaused && !this.singleSoundQueue.isEmpty()) {
        ...
        for (Iterator<Map.Entry<String, FutureSound>> iterator = this.singleSoundQueue.entrySet().iterator(); iterator.hasNext(); ) {
          ...
          if (((FutureSound)entry.getValue()).isCancelled()) {
      ...
      for (it = this.objectToAudioSourceMap.entrySet().iterator(); it.hasNext(); ) {
        ...
        if (((WeakObject)entry.getKey()).get() == null) {
          it.remove();
  ...
  public void onSoundPlayed(PlaySoundEvent event) {
    ...
    if ((category == SoundCategory.NEUTRAL && event.getName().endsWith(".hit")) || (category == SoundCategory.BLOCKS && event
      ...
      if (stack != null && stack.func_77973_b() instanceof IHitSoundOverride) {
        ...
        RayTraceResult mop = getMovingObjectPositionFromPlayer(world, (EntityPlayer)player, false);
        ...
        if (mop != null && mop.field_72313_a == RayTraceResult.Type.BLOCK && pos.equals(mop.func_178782_a())) {
          ...
          if (event.getSound().func_184365_d() == SoundCategory.NEUTRAL) {
            replace = ((IHitSoundOverride)stack.func_77973_b()).getHitSoundForBlock(player, world, pos, stack);
          } else {
            replace = ((IHitSoundOverride)stack.func_77973_b()).getBreakSoundForBlock(player, world, pos, stack);
          } 
          if (replace != null) {
