...
public class BoatEntityRenderer extends EntityRenderer<AbstractBoatEntity> {
                    ...
                    type ->
                        Pair.of(
                            ResourceLocation.fromNamespaceAndPath(
                                modId, this.getTextureLocation(type, chest)),
                            this.createModel(ctx, chest))));
  }

  private BoatModel createModel(Context ctx, boolean chest) {
    ModelLayerLocation entityModelLayer =
        chest
            ? ModelLayers.createChestBoatModelName(Type.OAK)
            : ModelLayers.createBoatModelName(Type.OAK);
  ...
  public void render(
    ...
    if (h > 0.0F) {
      matrixStack.mulPose(
          Axis.XP.rotationDegrees(Mth.sin(h) * h * j / 10.0F * boatEntity.getHurtDir()));
    }
    ...
    if (!Mth.equal(boatEntity.getBubbleAngle(g), 0.0F)) {
      matrixStack.mulPose(
          new Quaternionf()
              .rotateAxis(
                  boatEntity.getBubbleAngle(g) * ((float) Math.PI / 180F), 1.0F, 0.0F, 1.0F));
    }

    Pair<ResourceLocation, BoatModel> pair =
        this.texturesAndModels.get(boatEntity.getOverrideBoatType());
    ...
    VertexConsumer vertexConsumer =
        vertexConsumerProvider.getBuffer(boatEntityModel.renderType(identifier));
    boatEntityModel.renderToBuffer(
        matrixStack,
        vertexConsumer,
        i,
    ...
    if (!boatEntity.isUnderWater()) {
      ...
      boatEntityModel
          .waterPatch()
          .render(matrixStack, vertexConsumer2, i, OverlayTexture.NO_OVERLAY);
    }
  ...
  public ResourceLocation getTextureLocation(AbstractBoatEntity boatEntity) {
    return (ResourceLocation)
        this.texturesAndModels.get(boatEntity.getOverrideBoatType()).getFirst();
  }
}
