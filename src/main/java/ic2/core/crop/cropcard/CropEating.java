...
public class CropEating extends Ic2CropCard {
  ...
  public boolean canGrow(ICropTile crop) {
    return crop.getCurrentAge() < 2
        ...
        : crop.isBlockBelow(Blocks.LAVA)
            && crop.getCurrentAge() < this.getMaxAge()
            && crop.getLightLevel() > 10;
  }
  ...
  public boolean canBeHarvested(ICropTile crop) {
    return crop.getCurrentAge() >= this.getOptimalHarvestAge(crop)
        && crop.getCurrentAge() < this.getMaxAge();
  }
        ...
        for (LivingEntity entity : list) {
          if (!(entity instanceof Player) || !((Player) entity).getAbilities().instabuild) {
            entity.setDeltaMovement(
                (xcentered - entity.getX()) * 0.5,
                Math.min(entity.getDeltaMovement().y(), -0.05),
                (zcentered - entity.getZ()) * 0.5);
            entity.hurt(
                Ic2DamageSource.create(crop.getWorldObj(), "crop_eating"),
                (crop.getCurrentAge() + 1) * 2.0F);
            ...
            crop.getWorldObj()
                .playSound(
                    ...
                    SoundSource.BLOCKS,
                    1.0F,
                    IC2.random.nextFloat() * 0.1F + 0.9F);
  ...
  public int getGrowthDuration(ICropTile crop) {
    ...
    if (IC2.envProxy.biomeHasType(biome, EnvProxy.BiomeType.SWAMP)
        || IC2.envProxy.biomeHasType(biome, EnvProxy.BiomeType.MOUNTAIN)) {
      multiplier /= 1.5F;
    }
