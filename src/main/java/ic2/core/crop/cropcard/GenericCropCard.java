...
public class GenericCropCard extends Ic2CropCard {
  ...
  public ItemStack[] getGains(ICropTile crop) {
    if (this.drops != null && this.drops.length > 0) {
      ...
      if (this.specialDrops != null && this.specialDrops.length > 0) {
        ...
        if (roulette < this.specialDrops.length
            && !StackUtil.isEmpty(this.specialDrops[roulette])) {
  ...
  public int getGrowthDuration(ICropTile cropTile) {
    return this.growthSpeed < 200
        ? this.properties.tier() * 200
        : this.properties.tier() * this.growthSpeed;
  }
  ...
  public boolean canGrow(ICropTile crop) {
    if (this.rootRequirements != null
        && !this.rootRequirements.isEmpty()
        && crop.getCurrentAge() == this.maxSize - 1) {
  ...
  public List<ResourceLocation> getTexturesLocation() {
    ...
    for (int size = 1; size <= this.getMaxAge(); size++) {
      ret.add(
          ResourceLocation.fromNamespaceAndPath("ic2", "blocks/crop/" + this.getId() + "_" + size));
    }
  ...
  public GenericCropCard register() {
    ...
    for (GenericCropCard.BaseSeed baseSeed : this.baseSeeds) {
      Crops.instance.registerBaseSeed(
          baseSeed.seed, this, baseSeed.size, baseSeed.growth, baseSeed.gain, baseSeed.resistance);
    }
  ...
  public GenericCropCard addBaseSeed(
      ItemStack seed, int size, int growth, int gain, int resistance) {
