...
public class LiquidHeatExchangerManager implements ILiquidHeatExchangerManager {
  ...
  private final Map<Fluid, ILiquidHeatExchangerManager.HeatExchangeProperty> map =
      new IdentityHashMap<>();
  ...
  public void addFluid(Fluid input, Fluid output, int huPerMB) {
    if (this.map.containsKey(input)) {
      this.displayError(
          "The fluid " + input + " does already have a HeatExchangerProperty assigned.");
    } else if (huPerMB == 0) {
      this.displayError(
          "A mod tried to register a Fluid for the HeatExchanging recipe, without having an Energy value. Ignoring...");
    } else {
      if (this.heatup) {
        if (FluidHandler.getTemperature(input) >= FluidHandler.getTemperature(output)) {
          this.displayError(
              "Cannot heat up a warm liquid into a cold one. " + input + " -> " + output);
        }
      } else if (FluidHandler.getTemperature(input) <= FluidHandler.getTemperature(output)) {
        this.displayError(
            "Cannot cool down a cold liquid into a warm one. " + input + " -> " + output);
      }

      this.map.put(
          input, new ILiquidHeatExchangerManager.HeatExchangeProperty(output, Math.abs(huPerMB)));
    }
  }
  ...
  public class SingleDirectionManager implements ILiquidAcceptManager {
    ...
    public boolean acceptsFluid(Fluid fluid) {
      ...
      ILiquidHeatExchangerManager.HeatExchangeProperty property =
          LiquidHeatExchangerManager.this.getHeatExchangeProperty(fluid);
    ...
    public Set<Fluid> getAcceptedFluids() {
      ...
      for (Entry<Fluid, ILiquidHeatExchangerManager.HeatExchangeProperty> e :
          LiquidHeatExchangerManager.this.map.entrySet()) {
