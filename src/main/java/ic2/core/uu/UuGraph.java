...
public class UuGraph {
  ...
  public static void build(boolean reset) {
    ...
    IC2.log.debug(
        LogCategory.Uu,
        "%d UU recipe transformations fetched after %d ms.",
  ...
  public static ItemStack find(ItemStack stack) {
    ...
    if (exactNode != null) {
      ...
      LeanItemStack search =
          new LeanItemStack(stack.getItem(), StackUtil.getTag(stack), StackUtil.getSize(stack));
  ...
  private static void processRecipes(List<RecipeTransformation> transformations) {
    ...
    IC2.log.debug(
        LogCategory.Uu,
        "UU graph built with %d nodes after %d ms.",
        nodes.size(),
        (System.nanoTime() - startTime) / 1000000L);
  }
        ...
        for (UuGraph.NodeTransform nt : this.provides) {
          for (UuGraph.Node node : nt.out) {
            ...
            if (outputSize <= 0) {
              IC2.log.warn(
                  LogCategory.Uu,
                  "UU update: Invalid output size %d in recipe transform %s, expected %s.",
                  outputSize,
                  nt.transform,
                  node.stack);
              assert false;
