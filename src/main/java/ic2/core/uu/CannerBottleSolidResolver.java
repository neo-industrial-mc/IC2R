...
public class CannerBottleSolidResolver implements IRecipeResolver {
  ...
  public List<RecipeTransformation> getTransformations() {
    ...
    for (MachineRecipe<ICannerBottleRecipeManager.Input, ItemStack> recipe : (Iterable<MachineRecipe<ICannerBottleRecipeManager.Input, ItemStack>>)Recipes.cannerBottle.getRecipes()) {
      try {
        ...
        inputs.add(fill);
        ret.add(new RecipeTransformation(0.0D, inputs, RecipeUtil.convertOutputs(Collections.singletonList(recipe.getOutput()))));
