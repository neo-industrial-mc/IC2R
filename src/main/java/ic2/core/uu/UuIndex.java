...
public class UuIndex {
  ...
  public void refresh(boolean reset) {
    ...
    if (worldScanList.isEmpty()) {
      IC2.log.info(
          LogCategory.Uu,
          "Loading predefined UU world scan values, run /ic2 uu-world-scan <small|medium|large> to calibrate them for your world.");
  ...
  private void parseUuEntry(String entry, String category) {
    ...
    if (stack == null) {
      IC2.log.warn(
          LogCategory.Uu, "UU %s config: Can't find ItemStack for %s.", category, itemName);
      return;
    }
    ...
    try {
      ...
      IC2.log.warn(
          LogCategory.Uu, "UU %s config: Invalid value %s for %s.", category, valueStr, itemName);
    }
  }
    ...
    try (BufferedReader reader =
        new BufferedReader(
            new InputStreamReader(
                IC2.class.getResourceAsStream("/assets/ic2/config/uu_scan_values.ini"),
      ...
      while ((line = reader.readLine()) != null) {
        ...
        if (stack != null) {
          try {
            ...
            IC2.log.warn(
                LogCategory.Uu,
                "UU scan default config: Invalid value %s for %s.",
                valueStr,
                itemName);
          }
        } else {
          IC2.log.warn(
              LogCategory.Uu, "UU scan default config: Can't find ItemStack for %s.", itemName);
        }
      }
    } catch (Exception e) {
