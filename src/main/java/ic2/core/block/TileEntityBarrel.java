...
public class TileEntityBarrel extends TileEntityBlock {
   ...
   protected boolean onActivated(EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
      ...
      if (side.getAxis() != Axis.Y && !this.getActive() && StackUtil.consume(player, hand, StackUtil.sameStack(ItemName.treetap.getItemStack()), 1)) {
         ...
         if (!this.opened) {
            if (this.type == 0 || this.type == 1) {
               ...
               FluidStack fs;
               if (space >= 1000
                  && (fs = LiquidUtil.drainContainer(player, hand, FluidRegistry.WATER, space, FluidContainerOutputMode.InPlacePreferred, true)) != null
                  && fs.amount >= 1000) {
               ...
               if (StackUtil.checkItemEquality(stack, ItemName.crop_res.getItemStack(CropResItemType.hops))) {
                  this.type = 1;
                  int amount = StackUtil.getSize(stack);
                  if (player.isSneaking()) {
                     amount = 1;
      ...
      if (!this.opened) {
         ...
         if (this.type == 1) {
            ...
            if (this.hopsCount <= 0) {
               ...
               ratio = (float)this.hopsCount / this.wheatCount;
            }
            ...
            if (this.boozeAmount <= 0) {
               ...
               ratio = (float)(this.hopsCount + this.wheatCount) / this.boozeAmount;
            }
