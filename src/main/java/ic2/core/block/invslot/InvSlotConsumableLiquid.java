...
public class InvSlotConsumableLiquid extends InvSlotConsumable {
  ...
  public boolean accepts(ItemStack stack) {
    ...
    if (this.opType == InvSlotConsumableLiquid.OpType.Drain
        || this.opType == InvSlotConsumableLiquid.OpType.Both) {
    ...
    return (this.opType == InvSlotConsumableLiquid.OpType.Fill
            || this.opType == InvSlotConsumableLiquid.OpType.Both)
  ...
  public int fill(Ic2FluidStack fs, MutableObject<ItemStack> output, boolean simulate) {
    ...
    if (this.opType != InvSlotConsumableLiquid.OpType.Fill
        && this.opType != InvSlotConsumableLiquid.OpType.Both) {
      return 0;
    }
    ...
    LiquidUtil.FluidOperationResult result =
        LiquidUtil.fillContainer(stack, fs, FluidContainerOutputMode.EmptyFullToOutput);
    if (result == null) {
      return 0;
    }
  ...
  public boolean transferFromTank(
    ...
    LiquidUtil.FluidOperationResult result =
        LiquidUtil.fillContainerComplete(
            stack, tankFluid.copy(), FluidContainerOutputMode.EmptyFullToOutput);
    if (result == null) {
      return false;
    }
