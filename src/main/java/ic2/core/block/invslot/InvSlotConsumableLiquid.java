...
public class InvSlotConsumableLiquid extends InvSlotConsumable
{
	...
	public boolean accepts(ItemStack stack)
	{
		...
		if (this.opType == OpType.Drain || this.opType == OpType.Both)
		{
			...
			if (stack.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY, null))
			{
				...
				IFluidHandlerItem handler = singleStack.getCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY, null);
				if (handler != null)
		...
		return (this.opType == OpType.Fill || this.opType == OpType.Both) &&
			LiquidUtil.isFillableFluidContainer(stack, getPossibleFluids());
	}
	...
	public boolean processIntoTank(IFluidTank tank, InvSlotOutput outputSlot)
	{
		...
		if (transferToTank(tank, output, true) && (StackUtil.isEmpty(output.getValue()) || outputSlot.canAdd(output.getValue())))
		{
	...
	public boolean processFromTank(IFluidTank tank, InvSlotOutput outputSlot)
	{
		...
		MutableObject<ItemStack> output = new MutableObject<>();
		boolean wasChange = false;
		if (transferFromTank(tank, output, true) && (StackUtil.isEmpty(output.getValue()) || outputSlot.canAdd(output.getValue())))
		{
