...
public class InvSlotUpgrade extends InvSlot {
  ...
  public <T extends IInventorySlotHolder<?> & IUpgradableBlock> InvSlotUpgrade(
      T base, String name, int count) {
    super(base, name, InvSlot.Access.NONE, count);
    this.resetRates();
  }

  public static InvSlotUpgrade createUnchecked(
      IInventorySlotHolder<?> base, String name, int count) {
    return new InvSlotUpgrade((IInventorySlotHolder<?> & IUpgradableBlock) base, name, count);
  }

  @Override
  public boolean accepts(ItemStack stack) {
    return stack.getItem() instanceof IUpgradeItem item
        && item.isSuitableFor(stack, ((IUpgradableBlock) this.base).getUpgradableProperties());
  }
  ...
  public void onChanged() {
      ...
      if (!StackUtil.isEmpty(stack) && this.accepts(stack)) {
        ...
        if (all || upgrade instanceof IRedstoneSensitiveUpgrade) {
          ...
          if (redUpgrade.modifiesRedstoneInput(stack, block)) {
            newRedstoneModifiers.add(
                new InvSlotUpgrade.UpgradeRedstoneModifier(redUpgrade, stack, block));
          }
        }
      }
    }
    ...
    this.redstoneModifiers =
        newRedstoneModifiers.isEmpty() ? Collections.emptyList() : newRedstoneModifiers;
  }
  ...
  public int getOperationsPerTick(int defaultOperationLength) {
    return defaultOperationLength == 0
        ? 64
  ...
  private double getStackOpLen(int defaultOperationLength) {
    return ((double) defaultOperationLength + this.extraProcessTime)
        * 64.0
        * this.processTimeMultiplier;
  }
  ...
  public int getEnergyDemand(int defaultEnergyDemand) {
    return Process.applyModifier(
        defaultEnergyDemand, this.extraEnergyDemand, this.energyDemandMultiplier);
  }

  public int getEnergyStorage(
      int defaultEnergyStorage, int defaultOperationLength, int defaultEnergyDemand) {
    ...
    for (ItemStack stack : this) {
      if (!StackUtil.isEmpty(stack) && this.accepts(stack)) {
        ...
        if (upgrade instanceof IRemoteAccessUpgrade) {
          existingRange =
              ((IRemoteAccessUpgrade) upgrade)
                  .getRangeAmplification(stack, (IUpgradableBlock) this.base, existingRange);
        }
      }
    }
  ...
  public boolean tickNoMark() {
    ...
    for (int i = 0; i < this.size(); i++) {
      ...
      if (!StackUtil.isEmpty(stack)
          && stack.getItem() instanceof IUpgradeItem
  ...
  private record UpgradeRedstoneModifier(
      IRedstoneSensitiveUpgrade upgrade, ItemStack stack, IUpgradableBlock block)
      implements Redstone.IRedstoneModifier {
    private UpgradeRedstoneModifier(
        IRedstoneSensitiveUpgrade upgrade, ItemStack stack, IUpgradableBlock block) {
