...
public class InvSlotOutput extends InvSlot {
  ...
  public InvSlotOutput(
      IInventorySlotHolder<?> base1, String name1, int count, InvSlot.InvSide side) {
          ...
          for (int pass = 0; pass < 2; pass++) {
            for (int i = 0; i < this.size(); i++) {
              ...
              if (!StackUtil.isEmpty(existingStack)) {
                space =
                    Math.min(space, existingStack.getMaxStackSize())
                        - StackUtil.getSize(existingStack);
              }

              if (space > 0) {
                if (pass == 0
                    && !StackUtil.isEmpty(existingStack)
                    && StackUtil.checkItemEqualityStrict(stack, existingStack)) {
                  if (space >= amount) {
