...
public abstract class InvSlotConsumable extends InvSlot {
  ...
  public InvSlotConsumable(
      IInventorySlotHolder<?> base,
      String name,
      InvSlot.Access access,
      int count,
      InvSlot.InvSide preferredSide) {
  ...
  public boolean canOutput() {
    return super.canOutput()
        || this.access != InvSlot.Access.NONE && !this.isEmpty() && !this.accepts(this.get());
  }
  ...
  public ItemStack consume(int amount, boolean simulate, boolean consumeContainers) {
    ...
    for (int i = 0; i < this.size(); i++) {
      ...
      if (StackUtil.getSize(stack) >= 1
          ...
          && (StackUtil.getSize(stack) == 1
              || consumeContainers
              || !IC2.envProxy.hasRecipeRemainder(stack))) {
        ...
        if (!simulate) {
          if (StackUtil.getSize(stack) == currentAmount) {
            if (!consumeContainers && IC2.envProxy.hasRecipeRemainder(stack)) {
              ...
              if (container != null
                  && container.isDamageableItem()
                  && DamageHandler.getDamage(container) > DamageHandler.getMaxDamage(container)) {
                container = null;
              }
    ...
    for (int i = 0; i < this.size() && amount > 0; i++) {
      ...
      if (!StackUtil.isEmpty(stack)) {
        ...
        if (this.accepts(stack)
            ...
            && (target == null
                || item == target.getItem() && ItemStack.isSameItemSameComponents(stack, target))) {
          if (target == null) {
            target = stack.copy();
          }
