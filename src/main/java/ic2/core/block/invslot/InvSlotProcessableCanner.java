...
public class InvSlotProcessableCanner extends InvSlotProcessable<Object, Object, Object> {
  ...
  protected Object getInput(ItemStack fill) {
    ...
    return switch (((TileEntityCanner) this.base).getMode()) {
      ...
      case BottleLiquid ->
          new IFillFluidContainerRecipeManager.Input(container, this.getTankFluid());
  ...
  protected void setInput(Object rawInput) {
    ...
    switch (((TileEntityCanner) this.base).getMode()) {
      case BottleSolid:
        {
          ICannerBottleRecipeManager.RawInput input =
              (ICannerBottleRecipeManager.RawInput) rawInput;
      ...
      case EnrichLiquid:
        {
          ICannerEnrichRecipeManager.RawInput input =
              (ICannerEnrichRecipeManager.RawInput) rawInput;
          this.put(input.additive());
          tank.drainMbUnchecked(
              input.fluid() == null
                  ? tank.getFluidAmount()
                  : tank.getFluidAmount() - input.fluid().getAmountMb(),
              false);
          break;
        }
      case BottleLiquid:
        {
          IFillFluidContainerRecipeManager.Input input =
              (IFillFluidContainerRecipeManager.Input) rawInput;
          canInputSlot.put(input.container());
          tank.drainMbUnchecked(
              input.fluid() == null
                  ? tank.getFluidAmount()
  ...
  @Override
  protected MachineRecipeResult<Object, Object, Object> getOutputFor(
      Object input, boolean forAccept) {
      ...
      case BottleSolid ->
          (MachineRecipeResult)
              Recipes.cannerBottle
                  .get(world)
                  .apply((ICannerBottleRecipeManager.RawInput) input, forAccept);
      case EnrichLiquid ->
          (MachineRecipeResult)
              Recipes.cannerEnrich
                  .get(world)
                  .apply((ICannerEnrichRecipeManager.RawInput) input, forAccept);
      case BottleLiquid ->
          (MachineRecipeResult)
      ...
      case EmptyLiquid ->
          (MachineRecipeResult)
              Recipes.emptyFluidContainer.apply(
                  (ItemStack) input,
                  this.getTankFluid() == null ? null : this.getTankFluid().getFluid(),
                  FluidContainerOutputMode.EmptyFullToOutput,
                  forAccept);
    };
  }
