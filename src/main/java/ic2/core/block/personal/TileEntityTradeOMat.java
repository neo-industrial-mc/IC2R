...
public class TileEntityTradeOMat extends TileEntityInventory
  ...
  protected void onLoaded() {
    ...
    if (this.level == null) {
      IC2.log.error(
          LogCategory.Block,
          "World object is null while trade-o-mat block entity at: \""
              + this.worldPosition
              + "\" is loaded");
    } else {
  ...
  protected InteractionResult onActivated(
    ...
    if (!this.isWireless()
        && StackUtil.consume(
            player, hand, StackUtil.sameItem(Ic2Items.REMOTE_INTERFACE_UPGRADE), 1)) {
      if (this.level == null) {
        IC2.log.error(
            LogCategory.Block,
            "World object is null while trade-o-mat block entity at: \""
  ...
  public void updateStock() {
    ...
    if (StackUtil.isEmpty(offer)) {
      ...
      this.stock =
          StackUtil.fetch(this, StackUtil.copyWithSize(offer, Integer.MAX_VALUE), true)
              / StackUtil.getSize(offer);
    }
  }
  ...
  public ContainerBase<?> createServerScreenHandler(int syncId, Player player) {
    return this.permitsAccess(player.getGameProfile())
        ? new ContainerTradeOMatOpen(
            syncId, player.getInventory(), this, this.canToggleInfinite(player))
