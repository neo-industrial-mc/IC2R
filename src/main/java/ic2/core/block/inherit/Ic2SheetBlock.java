...
public class Ic2SheetBlock extends Block {
  public static final com.mojang.serialization.MapCodec<Ic2SheetBlock> CODEC =
      simpleCodec(Ic2SheetBlock::new);
  ...
  private static final Direction[] positiveHorizontalFacings =
    ...
    if (state.getBlock() == Ic2Blocks.RESIN_SHEET) {
      ...
      return !(state.getBlock() == Ic2Blocks.WOOL_SHEET
              ...
              || !entityContext.getEntity().isShiftKeyDown()
                  && !(entityContext.getEntity().getY()
                      < pos.getY() + 0.125 - entityContext.getEntity().maxUpStep())
          ? aabb
          : Shapes.empty();
    }
  }
