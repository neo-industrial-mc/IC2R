...
public final class ChunkLoadAwareBlockHandler
{
	...
	/**
	 * Called at the start of each server world tick to flush deferred chunk loads.
	 */
	public static void onWorldTick(Level world)
	{
		...
		for (ChunkPos cp : toProcess)
		{
			if (!world.getChunkSource().hasChunk(cp.x, cp.z))
			{
				continue;
			}

			processChunk(world.getChunk(cp.x, cp.z), true);
		}
	}
