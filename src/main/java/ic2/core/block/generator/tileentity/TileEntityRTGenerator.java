...
public class TileEntityRTGenerator extends TileEntityBaseGenerator {
  public final InvSlotConsumable fuelSlot =
      new InvSlotConsumableItemStack(this, "fuel", 6, new ItemStack(Ic2Items.RTG_PELLET));

  public TileEntityRTGenerator(BlockPos pos, BlockState state) {
    super(
        Ic2BlockEntities.RT_GENERATOR,
        pos,
        state,
        Math.round(
            16.0F * (float) IC2Config.balance.energy.generator.radioisotope.get().floatValue()),
        1,
        20000);
  ...
  public boolean gainEnergy() {
    ...
    this.energy.addEnergy(
        Math.pow(2.0, counter - 1)
            * (float) IC2Config.balance.energy.generator.radioisotope.get().floatValue());
    return true;
  }
