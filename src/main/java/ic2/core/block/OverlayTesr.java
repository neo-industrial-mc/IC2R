...
public class OverlayTesr extends TileEntitySpecialRenderer<TileEntityBlock> {
  public void render(TileEntityBlock te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
    ...
    float zScale = 1.001F;
    GlStateManager.translate((float)(x + 0.5D), (float)(y + 0.5D), (float)(z + 0.5D));
    ...
    renderer.getBlockModelRenderer().renderModel((IBlockAccess)te.getWorld(), renderer
        .getModelForState(state), state, te
        .getPos(), wr, true);
