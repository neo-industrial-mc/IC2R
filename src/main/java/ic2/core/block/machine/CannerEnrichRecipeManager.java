...
public class CannerEnrichRecipeManager implements ICannerEnrichRecipeManager {
  private final List<MachineRecipe<ICannerEnrichRecipeManager.Input, Ic2FluidStack>> recipes =
      new ArrayList<>();

  public boolean addRecipe(
      ICannerEnrichRecipeManager.Input input,
      Ic2FluidStack output,
    ...
    for (ItemStack stack : input.additive().getInputs()) {
      MachineRecipe<ICannerEnrichRecipeManager.Input, Ic2FluidStack> recipe =
          this.getRecipe(input.fluid(), stack, true);
      if (recipe != null) {
        if (!replace) {
          return false;
        }
  ...
  public void addRecipe(Ic2FluidStack fluid, IRecipeInput additive, Ic2FluidStack output) {
    if (!this.addRecipe(
        new ICannerEnrichRecipeManager.Input(fluid, additive), output, null, false)) {
      throw new RuntimeException(
          "ambiguous recipe: [" + fluid + "+" + additive.getInputs() + " -> " + output + "]");
    }
  }

  public MachineRecipeResult<
          ICannerEnrichRecipeManager.Input, Ic2FluidStack, ICannerEnrichRecipeManager.RawInput>
      apply(ICannerEnrichRecipeManager.RawInput input, boolean acceptTest) {
    MachineRecipe<ICannerEnrichRecipeManager.Input, Ic2FluidStack> recipe =
    ...
    return recipe.getResult(
        new ICannerEnrichRecipeManager.RawInput(
            remainingFluid,
            StackUtil.copyShrunk(input.additive(), recipe.getInput().additive().getAmount())));
  }

  private MachineRecipe<ICannerEnrichRecipeManager.Input, Ic2FluidStack> getRecipe(
      ...
      for (MachineRecipe<ICannerEnrichRecipeManager.Input, Ic2FluidStack> recipe : this.recipes) {
        if ((fluid == null
                || fluid.hasExactFluid(recipe.getInput().fluid())
                    && (acceptTest
                        || recipe.getInput().fluid().getAmountMb() <= fluid.getAmountMb()))
            && (additive == null
  ...
  public RecipeOutput getOutputFor(
    ...
    MachineRecipeResult<
            ICannerEnrichRecipeManager.Input, Ic2FluidStack, ICannerEnrichRecipeManager.RawInput>
        result = this.apply(new ICannerEnrichRecipeManager.RawInput(fluid, additive), acceptTest);
    if (result == null) {
      return null;
    }
  ...
  @Override
  public Iterable<? extends MachineRecipe<ICannerEnrichRecipeManager.Input, Ic2FluidStack>>
      getRecipes() {
    return this.recipes;
  }
