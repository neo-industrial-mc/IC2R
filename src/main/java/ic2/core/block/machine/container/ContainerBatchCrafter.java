...
public class ContainerBatchCrafter extends ContainerElectricMachine<TileEntityBatchCrafter> {
  ...
  public ContainerBatchCrafter(
      int syncId, Inventory playerInventory, TileEntityBatchCrafter tileEntity) {
      ...
      for (int x = 0; x < 3; x++) {
        this.addSlot(
            new SlotHologramSlot(
                tileEntity.craftingGrid,
                x + y * 3,
                30 + x * 18,
                17 + y * 18,
                1,
                index -> {
                  if (ContainerBatchCrafter.this.base.hasLevel()
                      && !ContainerBatchCrafter.this.base.getLevel().isClientSide) {
    ...
    for (int slot = 0; slot < 9; slot++) {
      this.indexToSlot.put(
          slot,
          this.addSlot(new SlotInvSlot(tileEntity.ingredientsRow[slot], 0, 8 + slot * 18, 84))
              .index);
  ...
  protected ItemStack handlePlayerSlotShiftClick(Player player, ItemStack sourceItemStack) {
    Tuple.T2<List<ItemStack>, ? extends IntCollection> changes =
        StackUtil.balanceStacks(
            this.base.ingredients,
            this.base.acceptPredicate,
            StackUtil.getSlotsFromInv(this.base.ingredients),
            Collections.singleton(sourceItemStack));
