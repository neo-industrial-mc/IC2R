...
public class ContainerMagnetizer extends ContainerElectricMachine<TileEntityMagnetizer> {
  ...
  public ContainerMagnetizer(EntityPlayer player, TileEntityMagnetizer base1) {
    ...
    this.player = player;
    for (int i = 0; i < 4; i++)
      addSlotToContainer((Slot)new SlotInvSlot((InvSlot)base1.upgradeSlot, i, 152, 8 + i * 18)); 
    addSlotToContainer((Slot)new SlotArmor(player.inventory, EntityEquipmentSlot.FEET, 45, 26));
  }
}
