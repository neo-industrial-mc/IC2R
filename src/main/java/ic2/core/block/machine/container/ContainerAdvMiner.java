...
public class ContainerAdvMiner extends ContainerElectricMachine<TileEntityAdvMiner>
{
	public ContainerAdvMiner(EntityPlayer player, TileEntityAdvMiner tileEntity1)
	{
		...
		addSlotToContainer(new SlotInvSlot(tileEntity1.scannerSlot, 0, 8, 26));
		for (int i = 0; i < 4; i++)
			addSlotToContainer(new SlotInvSlot(tileEntity1.upgradeSlot, i, 152, 26 + i * 18));
		for (int row = 0; row < 3; row++)
		{
			for (int col = 0; col < 5; col++)
				addSlotToContainer(new SlotInvSlot(tileEntity1.filterSlot, col + row * 5, 36 + col * 18, 44 + row * 18));
		}
	}
