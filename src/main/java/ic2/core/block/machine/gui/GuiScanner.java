...
public class GuiScanner extends GuiIC2<ContainerScanner>
{
	public GuiScanner(final ContainerScanner container)
	{
		...
		addElement(EnergyGauge.asBolt(this, 12, 25, container.base));
		addElement((new CustomButton(this, 102, 49, 12, 12, 176, 57, background, createEventSender(0))).withEnableHandler(() -> (container.base.getState() == TileEntityScanner.State.COMPLETED || container.base.getState() == TileEntityScanner.State.TRANSFER_ERROR || container.base.getState() == TileEntityScanner.State.FAILED)).withTooltip("ic2.Scanner.gui.button.delete"));
		addElement((new CustomButton(this, 143, 49, 24, 12, 176, 69, background, createEventSender(1))).withEnableHandler(() -> (container.base.getState() == TileEntityScanner.State.COMPLETED || container.base.getState() == TileEntityScanner.State.TRANSFER_ERROR)).withTooltip("ic2.Scanner.gui.button.save"));
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY)
	{
		...
		if (scanningloop > 0)
			drawTexturedModalRect(this.guiLeft + 30, this.guiTop + 20, 176, 14, scanningloop, 43);
	}
