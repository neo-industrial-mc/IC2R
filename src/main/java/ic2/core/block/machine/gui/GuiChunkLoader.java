...
public class GuiChunkLoader extends GuiIC2<ContainerChunkLoader>
{
	...
	{
		super(container, 250);
		addElement(EnergyGauge.asBolt(this, 12, 125, container.base));
	}
	...
	protected void drawForegroundLayer(int mouseX, int mouseY)
	{
		...
		ImmutableSet<ChunkPos> immutableSet = this.container.base.getLoadedChunks();
	...
	private void drawChunkAt(int x, int y, ChunkPos chunkPos)
	{
		...
		for (int cx = 0; cx < 16; cx++)
		{
			...
			for (int cz = 0; cz < 16; cz++)
			{
				...
				drawColoredRect(x + cx, y + cz, 1, 1, getColor(state, world, worldPos));
			}
		}
	}
		...
		if (mouseButton == 0)
		{
			...
			for (int i = -4; i <= 4; i++)
			{
				for (int j = -4; j <= 4; j++)
				{
					...
					{
						changeChunk(new ChunkPos(mainChunk.x + i, mainChunk.z + j));
						return;
					}
				}
			}
	...
	{
		ChunkPos mainChunk = ChunkLoaderLogic.getChunkCoords(this.container.base.getPos());
		IC2.network.get(false).initiateClientTileEntityEvent(this.container.base, chunk.x - mainChunk.x + 8 & 0xF | (chunk.z - mainChunk.z + 8 & 0xF) << 4);
	}
}
