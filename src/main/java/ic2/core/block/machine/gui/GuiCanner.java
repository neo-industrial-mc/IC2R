...
public class GuiCanner extends Ic2Gui<ContainerCanner> {
  ...
  public GuiCanner(ContainerCanner container, Inventory playerInventory, Component title) {
        ...
        new CycleHandler(
            ...
            new INumericValueHandler() {
              ...
              public void onChange(int value) {
                IC2.network
                    .get(false)
                    .initiateClientTileEntityEvent(GuiCanner.this.menu.base, value);
              }
            });
    this.addElement(
        new CustomButton(this, 63, 81, 50, 14, cycleHandler, texture, cycleHandler)
            .withTooltip(
                (Supplier<String>)
                    () ->
    ...
    this.addElement(
        new CustomButton(
                this, 77, 64, 22, 13, this.createEventSender(TileEntityCanner.eventSwapTanks))
    ...
    if (RecipeButton.canUse()) {
      for (final TileEntityCanner.Mode mode : TileEntityCanner.Mode.values) {
        this.addElement(
            new RecipeButton(this, 74, 22, 23, 14, new String[] {"canner_" + mode})
                .withEnableHandler(() -> GuiCanner.this.menu.base.getMode() == mode));
      }
    }
  }
