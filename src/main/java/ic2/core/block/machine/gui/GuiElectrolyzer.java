...
public class GuiElectrolyzer extends GuiIC2<ContainerElectrolyzer>
{
	public static class ElectrolyzerFluidTank implements IFluidTank
	{
		...
		{
			return (this.fluid == null) ? null : this.fluid.getRight();
		}

		public FluidStack getFluid()
		{
			return (this.fluid == null) ? null : this.fluid.getLeft();
		}

		public int getFluidAmount()
		{
			return (this.fluid == null) ? null : this.fluid.getLeft().amount;
		}
	...
	public GuiElectrolyzer(ContainerElectrolyzer container)
	{
		...
		addElement(FluidSlot.createFluidSlot(this, 78, 16, container.base.getInput()));
		ElectrolyzerFluidTank[] tanks = new ElectrolyzerFluidTank[5];
		for (int i = 0; i < 5; i++)
		{
			final ElectrolyzerFluidTank tank = new ElectrolyzerFluidTank();
			addElement(new FluidSlot(this, 36 + 21 * i, 61, 18, 18, tank)
			{
		...
		for (ElectrolyzerGauges gauge : ElectrolyzerGauges.values())
			addElement(last = (new CustomGauge(this, 36 + gauge.offset, 36, container.base, gauge.properties)).withEnableHandler(getEnableHandler(controller, gauge.ordinal() + 1)));
		if (RecipeButton.canUse())
		{
			assert last != null;
			addElement(new RecipeButton(last, new String[] { TeBlock.electrolyzer.getName() }));
		}
	}
