...
public abstract class GuiWeightedDistributor<T extends ContainerBase<? extends IWeightedDistributor>> extends GuiIC2<T> {
  ...
  public GuiWeightedDistributor(T container, int height) {
    super((ContainerBase)container, height);
    this.buttons = new StickyVanillaButton[5][6];
    for (int y = 0; y < 5; y++) {
      TextProvider.ITextProvider text;
      final int row = y;
      for (int col = 0; col < 6; col++) {
        final EnumFacing facing = EnumFacing.func_82600_a(facingOffset(col));
        addElement(
            
            (GuiElement)(this.buttons[y][col] = (new StickyVanillaButton(this, 63 + col * 18, 17 + y * 18, 16, 16, new IClickHandler() {
                ...
                public void onClick(MouseButton mouse) {
                  ...
                  for (int i = 0, aim = GuiWeightedDistributor.buttonOffset(facing.func_176745_a()); i < GuiWeightedDistributor.this.buttons.length; i++) {
                  ...
                  if (!switchingOff) {
                    ...
                    for (int j = 0, k = GuiWeightedDistributor.buttonOffset(facing.func_176745_a()); j < switches.length; j++)
                  ...
                  for (StickyVanillaButton[] switches : GuiWeightedDistributor.this.buttons) {
                    for (int j = 0; j < switches.length; j++) {
                      if (switches[j].isOn()) {
                        priorities.add(EnumFacing.func_82600_a(GuiWeightedDistributor.facingOffset(j)));
                        break;
                      } 
                    } 
                  } 
      ...
      addElement((GuiElement)Text.create(this, 8, 21 + y * 18, text, 4210752, false));
    } 
    int end = 0;
    for (EnumFacing side : ((IWeightedDistributor)((ContainerBase)container).base).getPriority())
      this.buttons[end++][buttonOffset(side.func_176745_a())].setOn(true); 
  }
