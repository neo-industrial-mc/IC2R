...
public class TileEntitySteamGenerator extends TileEntityInventory
  ...
  private TileEntitySteamGenerator.outputType outputFluid =
      TileEntitySteamGenerator.outputType.NONE;
  ...
  protected void updateEntityServer() {
    ...
    this.systemHeat =
        Math.max(
            this.systemHeat, BiomeUtil.getBiomeTemperature(this.getLevel(), this.worldPosition));
    if (this.isCalcified()) {
      if (this.getActive()) {
  ...
  private boolean work() {
    ...
    if (!this.waterTank.isEmpty() && this.inputMB > 0) {
      ...
      if (reqHeat > 1.0E-4F) {
        ...
        if (this.heatInput <= heatReq) {
          ...
          if (this.pressure == 0 && this.systemHeat < 99.9999F) {
            ...
            this.outputFluid =
                hasDistilledWater
                    ? TileEntitySteamGenerator.outputType.DISTILLEDWATER
                    : TileEntitySteamGenerator.outputType.WATER;
            int transferred =
                LiquidUtil.distribute(this, Ic2FluidStack.create(inputFluid, maxAmount), false);
            if (transferred > 0) {
      ...
      int activeAmount =
          Math.min(maxAmount, (int) ((remainingHuInput + availableSystemHu) / hUneeded));
      ...
      int transferred =
          LiquidUtil.distribute(this, Ic2FluidStack.create(output, this.outputMB), false);
  ...
  private void heatup(float heatinput) {
    ...
    if (this.systemHeat > 500.0F) {
      ...
      new Ic2Explosion(world, null, this.worldPosition, 10, 0.01F, Ic2Explosion.Type.Heat)
          .doExplosion();
    }
  }
  ...
  private void cooldown(float cool) {
    ...
    this.systemHeat =
        Math.max(
            ...
            BiomeUtil.getBiomeTemperature(this.getLevel(), this.worldPosition));
  }
  ...
  public int gaugeLiquidScaled(int i, int tank) {
    if (tank == 0) {
      return this.waterTank.getFluidAmount() <= 0
          ? 0
          : this.waterTank.getFluidAmount() * i / this.waterTank.getCapacity();
    } else {
      return 0;
    }
  }
