...
public class TileEntityTerra extends TileEntityElectricMachine {
  public final InvSlotConsumableClass tfbpSlot =
      new InvSlotConsumableClass(this, "tfbp", 1, ITerraformingBP.class);
  ...
  protected InteractionResult onActivated(
    ...
    if (!player.isShiftKeyDown() && !world.isClientSide) {
      ...
      ItemStack stack =
          StackUtil.consumeAndGet(
              player,
              hand,
              (Predicate<ItemStack>)
                  input -> {
                    Item item = input.getItem();
                    return item instanceof ITerraformingBP
                        && ((ITerraformingBP) item)
                            .canInsert(input, player, world, TileEntityTerra.this.worldPosition);
                  },
              1);
      if (!StackUtil.isEmpty(stack)) {
