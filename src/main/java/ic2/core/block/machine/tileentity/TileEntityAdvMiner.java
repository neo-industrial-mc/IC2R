...
public class TileEntityAdvMiner extends TileEntityElectricMachine
  ...
  protected void loadAdditional(
    ...
    if (nbt.contains("mineTargetX")) {
      this.mineTarget =
          new BlockPos(
              nbt.getInt("mineTargetX"), nbt.getInt("mineTargetY"), nbt.getInt("mineTargetZ"));
    }
  ...
  private boolean work() {
    ...
    if (!StackUtil.isEmpty(scanner) && ElectricItem.manager.canUse(scanner, 64.0)) {
      ...
      if (this.mineTarget == null) {
        this.mineTarget =
            new BlockPos(
                this.worldPosition.getX() - range - 1,
      ...
      MutableBlockPos scanPos =
          new MutableBlockPos(
              this.mineTarget.getX(), this.mineTarget.getY(), this.mineTarget.getZ());
      ...
      do {
        if (scanPos.getX() < this.worldPosition.getX() + range) {
          ...
          scanPos =
              new MutableBlockPos(
                  this.worldPosition.getX() - range, scanPos.getY(), scanPos.getZ() + 1);
        } else {
          scanPos =
              new MutableBlockPos(
    ...
    if (!this.scannerSlot.isEmpty()) {
      this.energy.useEnergy(
          ElectricItem.manager.charge(
              this.scannerSlot.get(),
              this.energy.getEnergy(),
              this.energy.getSinkTier(),
              false,
              false));
    }
  }
  ...
  public void doMine(BlockPos pos, BlockState state) {
    Level world = this.getLevel();
    StackUtil.distributeDrops(
        this, new ArrayList<>(StackUtil.getDrops(world, pos, state, null, 0, this.silkTouch)));
      ...
      if (!StackUtil.isEmpty(cardStack) && cardStack.getItem() instanceof ItemMiningFilterCard) {
        ...
        if (nbt != null) {
          ...
          for (int i = 0; i < items.size(); i++) {
            cardFilter.add(
                ItemStack.parseOptional(this.getLevel().registryAccess(), items.getCompound(i)));
          }
  ...
  private boolean evaluateFilter(
      List<ItemStack> drops, Iterable<ItemStack> filterItems, boolean isBlacklist) {
    if (isBlacklist) {
