...
@NotClassic
public class TileEntityPatternStorage extends TileEntityInventory
  ...
  public TileEntityPatternStorage(BlockPos pos, BlockState state) {
    ...
    this.diskSlot =
        new InvSlotConsumableId(
            this, "SaveSlot", InvSlot.Access.IO, 1, InvSlot.InvSide.ANY, Ic2Items.CRYSTAL_MEMORY);
  }
  ...
  public void readContents(CompoundTag nbt) {
    ...
    for (int i = 0; i < patternList.size(); i++) {
      ...
      ItemStack Item = ItemStack.parseOptional(net.minecraft.core.RegistryAccess.EMPTY, contentTag);
      ...
      case 2:
        if (this.index >= 0 && this.index < this.patterns.size() && !this.diskSlot.isEmpty()) {
          ...
          if (crystalMemory.getItem() instanceof ItemCrystalMemory) {
            ((ItemCrystalMemory) crystalMemory.getItem())
                .writeContentsTag(crystalMemory, this.patterns.get(this.index));
          }
        }
        break;
      case 3:
        if (!this.diskSlot.isEmpty()) {
          ...
          if (crystalMemory.getItem() instanceof ItemCrystalMemory) {
            ItemStack record =
                ((ItemCrystalMemory) crystalMemory.getItem()).readItemStack(crystalMemory);
            if (record != null) {
