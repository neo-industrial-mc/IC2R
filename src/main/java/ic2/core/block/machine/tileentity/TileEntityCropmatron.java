...
public class TileEntityCropmatron extends TileEntityElectricMachine
  ...
  public TileEntityCropmatron(BlockPos pos, BlockState state) {
    ...
    this.exTank =
        this.fluids.addTankInsert("exTank", 2000, Fluids.fluidPredicate(Ic2Fluids.WEED_EX.still()));
    this.fertilizerSlot =
        new InvSlotConsumableItemStack(this, "fertilizer", 7, new ItemStack(Ic2Items.FERTILIZER));
  ...
  public void scan() {
    ...
    if (this.getLevel().getBlockEntity(scan) instanceof TileEntityCrop crop) {
      if (!this.fertilizerSlot.isEmpty()
          && this.fertilizerSlot.consume(1, true, false) != null
          && crop.applyFertilizer(false)) {
      ...
      if (!this.waterTank.isEmpty()
          && (amount = crop.applyHydration(this.waterTank.getFluidAmount(), false)) > 0) {
        this.waterTank.drainMb(amount, false);
        this.energy.useEnergy(10.0);
      }

      if (!this.exTank.isEmpty()
          && (amount = crop.applyWeedEx(this.exTank.getFluidAmount(), false, false, false)) > 0) {
  ...
  private boolean tryHydrateFarmland(BlockPos pos) {
    ...
    if (state.getBlock() == Blocks.FARMLAND
        && (hydration = (Integer) state.getValue(FarmBlock.MOISTURE)) < 7) {
      ...
      world.setBlock(
          pos, (BlockState) state.setValue(FarmBlock.MOISTURE, hydration + drainAmount), 2);
      return true;
    } else {
      return false;
    }
