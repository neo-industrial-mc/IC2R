...
@NotClassic
public class TileEntityCondenser extends TileEntityElectricMachine
    ...
    if (this.outputTank.getCapacity() - this.outputTank.getFluidAmount() >= 100) {
      if (this.progress >= 10000) {
        this.outputTank.fillMbUnchecked(
            Ic2FluidStack.create(Ic2Fluids.DISTILLED_WATER.still(), 100), false);
        this.progress -= 10000;
      }
      ...
      if (this.inputTank.getFluidAmount() > 0) {
        ...
        if (this.energy.useEnergy(vents * 2)) {
          this.progress =
              this.progress + this.inputTank.drainMbUnchecked(drain, false).getAmountMb();
        }
      }
    }
  }
