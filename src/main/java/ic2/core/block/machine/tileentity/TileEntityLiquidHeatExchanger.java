...
public class TileEntityLiquidHeatExchanger extends TileEntityHeatSourceInventory
  ...
  public TileEntityLiquidHeatExchanger(BlockPos pos, BlockState state) {
    ...
    this.inputTank =
        this.fluids.addTankInsert(
            "inputTank", 2000, Fluids.fluidPredicate(Recipes.liquidCooldownManager));
    this.outputTank = this.fluids.addTankExtract("outputTank", 2000);
    this.heatexchangerslots =
        new InvSlotConsumableItemStack(
            this, "heatExchanger", 10, new ItemStack(Ic2Items.HEAT_CONDUCTOR));
    ...
    IC2.envProxy.runAfterRegistryInit(
        () -> {
          ...
          addHeatupRecipe(
              ...
              Math.round(
                  IC2Config.balance.energy.fluidConversion.heatExchangerWater.get().floatValue()));
        });
  }
  ...
  protected int fillHeatBuffer(int bufferSpace) {
    if (bufferSpace > 0) {
      ...
      if (Recipes.liquidCooldownManager.acceptsFluid(fluidInputTank)) {
        ILiquidHeatExchangerManager.HeatExchangeProperty hep =
            Recipes.liquidCooldownManager.getHeatExchangeProperty(fluidInputTank);
      ...
      if (this.outputTank.getFluidAmount() > 0
          && !this.outputTank.getFluidStack().hasExactFluid(fluidOutput)) {
        return 0;
      }
      ...
      if (drainCoolant != null) {
        ...
        this.outputTank.fillMbUnchecked(
            Ic2FluidStack.create(fluidOutput, drainCoolant.getAmountMb()), false);
