...
public class TileEntityLiquidHeatExchanger extends TileEntityHeatSourceInventory implements IHasGui, IUpgradableBlock {
  ...
  public TileEntityLiquidHeatExchanger() {
    ...
    this.inputTank = (FluidTank)this.fluids.addTankInsert("inputTank", 2000, Fluids.fluidPredicate((ILiquidAcceptManager)Recipes.liquidCooldownManager));
  ...
  public static void init() {
    Recipes.liquidCooldownManager = (ILiquidHeatExchangerManager)new LiquidHeatExchangerManager(false);
    Recipes.liquidHeatupManager = (ILiquidHeatExchangerManager)new LiquidHeatExchangerManager(true);
  ...
  public ContainerBase<TileEntityLiquidHeatExchanger> getGuiContainer(EntityPlayer player) {
    return (ContainerBase<TileEntityLiquidHeatExchanger>)new ContainerLiquidHeatExchanger(player, this);
  }
  ...
  protected int fillHeatBuffer(int bufferspace) {
    if (bufferspace > 0) {
      ...
      if (fluidOutput == null)
        return 0; 
      if (this.outputTank.getFluidAmount() > 0 && 
        !this.outputTank.getFluid().getFluid().equals(fluidOutput))
        return 0; 
