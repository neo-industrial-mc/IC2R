...
public class TileEntityRTHeatGenerator extends TileEntityHeatSourceInventory implements IHasGui {
  public static final float outputMultiplier =
      2.0F * (float) IC2Config.balance.energy.heatGenerator.radioisotope.get().floatValue();
  public final InvSlotConsumable fuelSlot =
      new InvSlotConsumableItemStack(this, "fuelSlot", 6, new ItemStack(Ic2Items.RTG_PELLET));
  ...
  protected int fillHeatBuffer(int maxAmount) {
    return maxAmount >= this.getMaxHeatEmittedPerTick()
        ? this.getMaxHeatEmittedPerTick()
        : maxAmount;
  }
  ...
  @Override
  public ContainerBase<TileEntityRTHeatGenerator> createServerScreenHandler(
