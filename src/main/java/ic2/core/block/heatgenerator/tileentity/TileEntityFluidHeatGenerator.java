...
public class TileEntityFluidHeatGenerator extends TileEntityHeatSourceInventory implements IHasGui {
  ...
  public TileEntityFluidHeatGenerator(BlockPos pos, BlockState state) {
    ...
    this.fluidSlot =
        new InvSlotConsumableLiquidByManager(this, "fluidSlot", 1, Recipes.fluidHeatGenerator);
    ...
    if ((float) IC2Config.balance.energy.generator.semiFluidBiogas.get().floatValue() > 0.0F) {
      addFuel(
          ...
          Math.round(
              32.0F
                  * (float)
                      IC2Config.balance.energy.heatGenerator.semiFluidBiogas.get().floatValue()));
    }
  }
  ...
  @Override
  public ContainerBase<TileEntityFluidHeatGenerator> createServerScreenHandler(
  ...
  protected int calcHeatProduction() {
    if (!this.fluidTank.isEmpty() && this.getFluidfromTank() != null) {
      IFluidHeatManager.BurnProperty property =
          Recipes.fluidHeatGenerator.getBurnProperty(this.getFluidfromTank());
      if (property != null) {
  ...
  public int gaugeLiquidScaled(int i) {
    return this.fluidTank.getFluidAmount() <= 0
        ? 0
        : this.fluidTank.getFluidAmount() * i / this.fluidTank.getCapacity();
  }
