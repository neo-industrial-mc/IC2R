...
public class EntityDynamite extends Entity implements IProjectile {
  ...
  public EntityDynamite(World world, EntityLivingBase owner) {
    ...
    setLocationAndAngles(eyePos.x, eyePos.y, eyePos.z, owner.rotationYaw, owner.rotationPitch);
    ...
    shoot(this.motionX, this.motionY, this.motionZ, 1.0F, 1.0F);
  }
  ...
  public void shoot(double x, double y, double z, float velocity, float inaccuracy) {
    ...
    this.prevRotationPitch = this.rotationPitch = (float)(Math.atan2(y, hLen) * 180.0D / Math.PI);
  ...
  public void setVelocity(double x, double y, double z) {
    ...
    if (this.prevRotationPitch == 0.0F && this.prevRotationYaw == 0.0F) {
      ...
      this.prevRotationYaw = this.rotationYaw = (float)(Math.atan2(x, z) * 180.0D / Math.PI);
      this.prevRotationPitch = this.rotationPitch = (float)(Math.atan2(y, h) * 180.0D / Math.PI);
  ...
  public void onUpdate() {
    ...
    if (this.fuse-- <= 0) {
      ...
      world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, this.posX, this.posY + 0.5D, this.posZ, 0.0D, 0.0D, 0.0D, new int[0]);
    } 
    if (this.inGround) {
      this.ticksInGround++;
    ...
    if (isInWater()) {
      ...
      for (int i1 = 0; i1 < 4; i1++) {
        float f6 = 0.25F;
        world.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX - this.motionX * f6, this.posY - this.motionY * f6, this.posZ - this.motionZ * f6, this.motionX, this.motionY, this.motionZ, new int[0]);
      } 
      f3 = 0.75F;
    } 
    this.motionX *= f3;
