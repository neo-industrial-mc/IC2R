...
public class SlotHologramSlot extends Slot {
  ...
  public SlotHologramSlot(
      ItemStack[] stacks,
      int index,
      int x,
      int y,
      int stackSizeLimit,
      SlotHologramSlot.ChangeCallback changeCallback) {
  ...
  public ItemStack slotClick(
      int button, ClickType clickType, Player player, AbstractContainerMenu screenHandler) {
    if (Util.inDev() && player.getCommandSenderWorld().isClientSide) {
      System.out.printf(
          "button=%d clickType=%s stack=%s%n", button, clickType, screenHandler.getCarried());
    }
    ...
    if (clickType == ClickType.PICKUP && (button == 0 || button == 1)) {
      ...
      if (!StackUtil.isEmpty(playerStack)) {
        ...
        if (curSize == 0) {
          ...
          if (Util.inDev()) {
            System.out.println(
                "add " + extraSize + " to " + slotStack + " -> " + (curSize + extraSize));
          }

          this.stacks[this.index] = StackUtil.incSize(slotStack, extraSize);
        } else {
          this.stacks[this.index] =
              StackUtil.copyWithSize(playerStack, Math.min(StackUtil.getSize(playerStack), limit));
        }
