...
public interface IHasGui extends Container {
  static Component getBeName(BlockEntity be) {
    ...
    return Component.translatable(
        String.format("container.%s.%s", id.getNamespace(), id.getPath().replace('/', '.')));
  }
  ...
  static Component getItemName(Item item, Integer subGuiId) {
    ...
    return Component.translatable(
        String.format(
            ...
            id.getNamespace(),
            id.getPath().replace('/', '.'),
            subGuiId != null ? String.format(".%d", subGuiId) : ""));
  }
  ...
  default boolean openManagedBe(Player player, InteractionHand hand) {
    ...
    buffer.flip();
    return IC2.envProxy.openHandledScreen(
        player,
        new MenuProvider() {
          public AbstractContainerMenu createMenu(
              int syncId, Inventory playerInventory, Player playerx) {
