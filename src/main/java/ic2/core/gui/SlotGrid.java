...
public class SlotGrid extends GuiElement<SlotGrid> {
  ...
  public SlotGrid(
    ...
    super(
        ...
        x - border,
        y - border,
        xCount * style.width + 2 * border + (xCount - 1) * spacing,
        yCount * style.height + 2 * border + (yCount - 1) * spacing);
    this.style = style;
    this.border = border;
    ...
    public static void registerVarient(String name, SlotStyle newSlotStyle) {
      ...
      if (old != null) {
        throw new RuntimeException(
            "Duplicate slot instance for name! " + name + " -> " + old + " and " + newSlotStyle);
      }
    }
