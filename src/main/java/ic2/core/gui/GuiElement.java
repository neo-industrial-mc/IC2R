...
public abstract class GuiElement<T extends GuiElement<T>> {
  ...
  private static final Map<Class<?>, Set<GuiElement.ImplementedMethod>> IMPLEMENTED_METHOD_CACHE =
      new IdentityHashMap<>();
      ...
      for (Class<?> curCls = cls; curCls != GuiElement.class; curCls = curCls.getSuperclass()) {
        for (Method method : curCls.getDeclaredMethods()) {
          if ((method.getModifiers() & 10) == 0) {
            GuiElement.ImplementedMethod implMethod =
                GuiElement.ImplementedMethod.LOOKUP.get(method.getName());
            if (implMethod != null
                && !ret.contains(implMethod)
                && !method.isAnnotationPresent(GuiElement.SkippedMethod.class)) {
              ret.add(implMethod);
            }
          }
        }
