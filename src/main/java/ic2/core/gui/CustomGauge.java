...
public class CustomGauge extends Gauge<CustomGauge> {
  ...
  public CustomGauge(
      ...
      int x,
      int y,
      CustomGauge.IGaugeRatioProvider provider,
      Gauge.GaugeProperties properties) {
    super(gui, x, y, properties);
    this.provider = provider;
  }

  public static CustomGauge asFuel(
      Ic2Gui<?> gui, int x, int y, CustomGauge.IGaugeRatioProvider provider) {
    return new CustomGauge(gui, x, y, provider, Gauge.GaugeStyle.Fuel.properties);
  }

  public static CustomGauge create(
      Ic2Gui<?> gui,
      int x,
      int y,
      CustomGauge.IGaugeRatioProvider provider,
