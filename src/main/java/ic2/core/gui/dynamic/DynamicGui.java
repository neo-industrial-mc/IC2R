...
public class DynamicGui<T extends Container> extends GuiDefaultBackground<DynamicContainer<T>> {
  ...
  public static <T extends Container> DynamicGui<T> create(
      DynamicContainer<T> container, Inventory playerInventory, Component title) {
        ...
        case button:
          {
            ...
            if (node.type != GuiParser.ButtonNode.ButtonType.RECIPE
              ...
              throw new RuntimeException(
                  "Invalid base "
                      + ((DynamicContainer<?>) this.menu).base
                      + " for button elements");
            }
              ...
              case VANILLA:
                button =
                    new VanillaButton(
                        ...
                        node.width,
                        node.height,
                        this.createEventSender(node.eventID, node.eventName));
              case CUSTOM:
              default:
                break;
              case TRANSPARENT:
            ...
            if (button != null) {
              String text =
                  node.text.get(
                      this.menu.base,
                      Collections.singletonMap("name", TextProvider.of(this.title)));
              if (node.icon == null) {
        ...
        case energygauge:
          {
            if (!(this.menu.base instanceof Ic2TileEntity)
              ...
              throw new RuntimeException(
                  "invalid base "
                      + ((DynamicContainer<?>) this.menu).base
                      + " for energygauge elements");
            }

            GuiParser.EnergyGaugeNode node = (GuiParser.EnergyGaugeNode) rawNode;
            parentNode.addElement(
                this,
                new EnergyGauge(this, node.x, node.y, (Ic2TileEntity) this.menu.base, node.style));
            break;
          }
        case gauge:
          {
            if (!(this.menu.base instanceof IGuiValueProvider)) {
              throw new RuntimeException(
                  "invalid base " + ((DynamicContainer<?>) this.menu).base + " for gauge elements");
            }

            GuiParser.GaugeNode node = (GuiParser.GaugeNode) rawNode;
            final boolean isActiveLinked = node.activeLinked;
            if (isActiveLinked
                && !(this.menu.base instanceof IGuiValueProvider.IActiveGuiValueProvider)) {
              throw new RuntimeException(
                  "Invalid base "
                      + ((DynamicContainer<?>) this.menu).base
                      + " for active linked gauge elements");
            }
            ...
            parentNode.addElement(
                ...
                new LinkedGauge(
                    ...
                    node.x,
                    node.y,
                    (IGuiValueProvider) this.menu.base,
                    node.name,
                    node.style) {
                  @Override
                  protected boolean isActive(double ratio) {
                    return isActiveLinked
                        ? ((IGuiValueProvider.IActiveGuiValueProvider) DynamicGui.this.menu.base)
                            .isGuiValueActive(this.name)
        ...
        case playerinventory:
          {
            ...
            parentNode.addElement(
                this, new SlotGrid(this, node.x, node.y, 9, 3, node.style, 0, node.spacing));
            parentNode.addElement(
                this,
                new SlotGrid(
                    this, node.x, node.y + node.hotbarOffset, 9, 1, node.style, 0, node.spacing));
            if (node.showTitle) {
              parentNode.addElement(
                  this,
                  TextLabel.create(
                      ...
                      node.x + 1,
                      node.y - 10,
                      TextProvider.of(playerInventory.getName()),
                      4210752,
                      false));
            }
            break;
        ...
        case slotgrid:
          {
            if (!(this.menu.base instanceof IInventorySlotHolder)) {
              throw new RuntimeException(
                  "Invalid base " + ((DynamicContainer<?>) this.menu).base + " for slot elements");
            }
            ...
            if (size > node.offset) {
              ...
              parentNode.addElement(
                  this,
                  new SlotGrid(
                      this, node.x, node.y, dim.cols(), dim.rows(), node.style, 0, node.spacing));
            }
            break;
          }
        case text:
        ...
        case fluidtank:
          {
            if (!(this.menu.base instanceof Ic2TileEntity)
              ...
              throw new RuntimeException(
                  "invalid base " + ((DynamicContainer<?>) this.menu).base + " for tank elements");
            }
            ...
            parentNode.addElement(
                ...
                switch (node.type) {
                  case NORMAL ->
                      TankGauge.createNormal(this, node.x, node.y, fluids.getFluidTank(node.name));
                  case PLAIN ->
                      TankGauge.createPlain(
                          this,
                          node.x,
                          node.y,
                          node.width,
                          node.height,
                          fluids.getFluidTank(node.name));
                  case BORDERLESS ->
                      TankGauge.createBorderless(
                          this, node.x, node.y, fluids.getFluidTank(node.name), node.mirrored);
                });
            break;
          }
        case fluidslot:
          {
            ...
            parentNode.addElement(
                ...
                TankFluidSlot.createFluidSlot(
                    ...
                    node.x,
                    node.y,
                    ((Ic2TileEntity) this.menu.base)
                        .getComponent(Fluids.class)
                        .getFluidTank(node.name)));
          }
      }
  ...
  protected IClickHandler createEventSender(int event, String eventString) {
    if (this.isHandHeldGUI()) {
      ...
      if (eventString == null) {
        IC2.log.warn(
            LogCategory.General, "HandHand inventory given numeric event rather than string");
