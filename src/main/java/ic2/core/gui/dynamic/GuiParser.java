...
public class GuiParser {
  public static GuiParser.GuiNode parse(ResourceLocation id, Class<?> baseClass) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Error reading/parsing GUI definition " + id + " from " + fileLoc, e);
    } finally {
      try {
        if (is != null) {
          is.close();
  ...
  private static GuiParser.GuiNode parse(InputStream is, Class<?> baseClass)
  ...
  public abstract static class KeyOnlyNode extends GuiParser.ParentNode {
    ...
    static GuiParser.KeyOnlyNode getForKey(GuiParser.ParentNode parent, Attributes attributes)
        throws SAXException {
    ...
    @Override
    protected Collection<IEnableHandler> addHandlers(
        DynamicGui<?> gui, GuiElement<?> element, Collection<IEnableHandler> handlers) {
      handlers.add(
          this.inverted ? IEnableHandler.EnableHandlers.not(this::isKeyDown) : this::isKeyDown);
  ...
  public static class LogicalNode extends GuiParser.ParentNode {
    ...
    static GuiParser.LogicalNode getForValue(GuiParser.ParentNode parent, Attributes attributes)
        throws SAXException {
      return new GuiParser.LogicalNode(
          parent, XmlUtil.getAttr(attributes, "if"), attributes.getValue("inverted") != null);
    }
    ...
    protected Collection<IEnableHandler> addHandlers(
      ...
      if (!(gui.getContainer().base instanceof IGuiConditionProvider)) {
        throw new RuntimeException(
            "Invalid base " + gui.getContainer().base + " for conditional elements");
      }

      IEnableHandler handler =
          () -> ((IGuiConditionProvider) gui.getContainer().base).getGuiState(this.condition);
  ...
  public abstract static class ParentNode extends GuiParser.Node {
    ...
    void addElement(DynamicGui<?> gui, GuiElement<?> element) {
      ...
      if (!handlers.isEmpty()) {
        element.withEnableHandler(
            IEnableHandler.EnableHandlers.and(handlers.toArray(new IEnableHandler[0])));
      }
  ...
  private static class SaxHandler extends DefaultHandler {
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes)
      ...
      switch (type) {
        ...
        case tooltip:
          this.currentNode =
              new GuiParser.TooltipNode(this.parentNode, attributes.getValue("text"));
          this.parentNode.addNode(this.currentNode);
          this.parentNode = (GuiParser.ParentNode) this.currentNode;
          break;
        case button:
  ...
  public static class SlotGridNode extends GuiParser.Node {
    ...
    public GuiParser.SlotGridNode.SlotGridDimension getDimension(int totalSize) {
      ...
      if (!this.vertical) {
        if (this.cols > 0) {
          return new GuiParser.SlotGridNode.SlotGridDimension(
              Math.max(this.rows, (totalSize + this.cols - 1) / this.cols), this.cols);
        }

        if (this.rows > 0) {
          return new GuiParser.SlotGridNode.SlotGridDimension(
              this.rows, (totalSize + this.rows - 1) / this.rows);
        }
        ...
        if (this.rows > 0) {
          return new GuiParser.SlotGridNode.SlotGridDimension(
              this.rows, Math.max(this.cols, (totalSize + this.rows - 1) / this.rows));
        }

        if (this.cols > 0) {
          return new GuiParser.SlotGridNode.SlotGridDimension(
              (totalSize + this.cols - 1) / this.cols, this.cols);
        }
  ...
  public static class TextNode extends GuiParser.Node {
    ...
    TextNode(GuiParser.ParentNode parent, Attributes attributes) throws SAXException {
      ...
      String center =
          XmlUtil.getAttr(
              attributes, "center", this.align == TextLabel.TextAlignment.Center ? "x" : "");
