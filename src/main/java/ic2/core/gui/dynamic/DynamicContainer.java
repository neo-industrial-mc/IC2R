...
public class DynamicContainer<T extends Container> extends ContainerBase<T> {
  ...
  protected DynamicContainer(
      MenuType<DynamicContainer<T>> type,
      int syncId,
      Inventory playerInventory,
      T base,
      GuiParser.GuiNode guiNode) {
    super(type, syncId, playerInventory, base);
    this.guiNode = guiNode;
    this.initialize(playerInventory, guiNode, guiNode);
  }

  public static DynamicContainer<TileEntityInventory> create(
      int syncId, Inventory playerInventory, TileEntityInventory base) {
  ...
  private void initialize(
      Inventory playerInventory, GuiParser.GuiNode guiNode, GuiParser.ParentNode parentNode) {
              ...
              for (int col = 0; col < 9; col++) {
                this.addSlot(
                    new Slot(
                        playerInventory,
                        col + row * 9 + 9,
                        node.x + col * width + xOffset,
                        node.y + row * height + yOffset));
              }
            }
            ...
            for (int col = 0; col < 9; col++) {
              this.addSlot(
                  new Slot(
                      ...
                      col,
                      node.x + col * width + xOffset,
                      node.y + node.hotbarOffset + yOffset));
            }
            break;
          }
        case slot:
          {
            ...
            if (slot == null) {
              throw new RuntimeException(
                  "Invalid InvSlot name " + node.name + " for base " + this.base);
            }
        ...
        case slothologram:
          {
            if (!(this.base instanceof IHolographicSlotProvider)) {
              throw new RuntimeException(
                  "Invalid base " + this.base + " for holographic slot elements");
            }
            ...
            this.addSlot(
                new SlotHologramSlot(
                    ((IHolographicSlotProvider) this.base).getStacksForName(node.name),
                    node.index,
                    x,
                    y,
                    node.stackSizeLimit,
