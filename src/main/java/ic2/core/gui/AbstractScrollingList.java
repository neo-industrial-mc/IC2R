...
public abstract class AbstractScrollingList<
        T extends AbstractScrollingList<T, I>, I extends AbstractScrollingList.IListItem>
  ...
  protected AbstractScrollingList(
      Ic2Gui<?> gui, int x, int y, int width, int height, List<I> items) {
  ...
  public void drawBackground(GuiGraphics guiGraphics, int mouseX, int mouseY) {
    bindCommonTexture();
    this.gui.drawColoredRect(
        guiGraphics.pose(), this.x, this.y, this.width, this.height, -16777216);
    this.gui.drawColoredRect(
        guiGraphics.pose(), this.x + 1, this.y + 1, this.width - 7 - 1, this.height - 2, -6250336);
    this.gui.drawColoredRect(
        guiGraphics.pose(), this.x + this.width - 7 + 1, this.y + 1, 5, this.height - 2, -6250241);
    int scrollStart;
    int scrollHeight;
    if (this.items.size() * 11 - 1 <= this.height - 2) {
      ...
      scrollStart =
          (int)
              Util.lerp(
                  this.y + 1,
                  this.y + this.height - 1 - scrollHeight,
                  this.scroll / this.getMaxScroll());
    }

    this.gui.drawColoredRect(
        guiGraphics.pose(), this.x + this.width - 7 + 1, scrollStart, 5, scrollHeight, -16777216);
  }
  ...
  public void drawForeground(GuiGraphics guiGraphics, int mouseX, int mouseY) {
    ...
    for (I item : this.items) {
      ...
      if (currentY > 0) {
        ...
        this.gui.drawColoredRect(
            guiGraphics.pose(), xOffset, this.y - top + currentY - 1, this.width - 7, 1, -16777216);
        if (bottomGap == -1) {
          break;
        }
      }
  ...
  private void doScissor(int GUIwidth, int GUIheight) {
    ...
    GL11.glScissor(
        left * scale,
        window.getScreenHeight() - bottom * scale,
        viewWidth * scale,
        viewHeight * scale);
  }
          ...
          && mouseY
              <= this.scroll * this.getScrollScale()
                  + (this.items.size() * 11 - 1 <= this.height - 2
                      ? this.height - 2
                      : Math.max(
                          this.height - 2 - (this.items.size() * 11 - 1 - (this.height - 2)), 1))) {
  ...
  public boolean onMouseDrag(int mouseX, int mouseY, MouseButton button, boolean onThis) {
    if (this.scrolling) {
      ...
      float mouseMovement =
          this.scroll + (mouseY - this.mouseScrollOffset) * this.getInverseScrollScale();
      ...
      if (!Util.isSimilar(mouseMovement, this.scroll)) {
        if (!Util.isSimilar(startingScroll, this.scroll)) {
          this.mouseScrollOffset =
              (int)
                  (this.mouseScrollOffset + (this.scroll - startingScroll) * this.getScrollScale());
          assert this.mouseScrollOffset >= 0
              : "Left the scroll bar dragging from " + startingScroll + " to " + this.scroll;
        }
      } else {
        this.mouseScrollOffset = mouseY;
        if (mouseY == 0) {
