...
public class ScrollableSelectiveList
    extends AbstractScrollingList<
        ScrollableSelectiveList, ScrollableSelectiveList.ISelectableListItem> {
  protected ScrollableSelectiveList.ISelectableListItem currentSelected;

  public ScrollableSelectiveList(Ic2Gui<?> gui, int x, int y, int width, int height) {
    super(gui, x, y, width, height);
  }

  public ScrollableSelectiveList(
      Ic2Gui<?> gui,
      int x,
      int y,
      int width,
      int height,
      List<ScrollableSelectiveList.ISelectableListItem> items) {
  ...
  protected boolean onItemClick(
      ScrollableSelectiveList.ISelectableListItem item,
      MouseButton button,
      int mouseX,
      int mouseY) {
