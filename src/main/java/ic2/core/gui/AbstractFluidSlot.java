...
public abstract class AbstractFluidSlot extends GuiElement<TankFluidSlot> {
  ...
  public void drawBackground(GuiGraphics guiGraphics, int mouseX, int mouseY) {
    ...
    this.gui.drawTexturedRect(
        guiGraphics.pose(), this.x, this.y, this.width, this.height, 8.0, 160.0);
    if (fs != null && !fs.isEmpty()) {
      ...
      TextureAtlasSprite sprite =
          fluid != null
              ? getBlockTextureMap().getSprite(FluidHandler.getStillSpriteId(fluid))
              : null;
  ...
  protected List<Component> getToolTip() {
    ...
    if (fs != null && !fs.isEmpty()) {
      ...
      if (fluid != null) {
        ...
        String translateKey =
            FluidHandler.isGaseous(fluid) ? "ic2.generic.text.gas" : "ic2.generic.text.liquid";
        ret.add(
