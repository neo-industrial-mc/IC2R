...
public class ScrollableList extends GuiElement<ScrollableList>
{
	...
	public void drawBackground(int mouseX, int mouseY)
	{
		...
		} else
		{
			...
			scrollStart = (int) Util.lerp(this.y + 1, this.y + this.height - scrollHeight - 1, (float) this.scroll / this.getMaxScroll());
		}
	...
	public void drawForeground(int mouseX, int mouseY)
	{
		...
		for (ScrollableList.IListItem item : this.items)
		{
	...
	protected boolean onMouseClick(int mouseX, int mouseY, MouseButton button)
	{
		...
		if (!this.items.isEmpty() && mouseX > 0 && mouseY > 0)
		{
			if (mouseX < this.width - 7)
			{
				...
				&& mouseX < this.width - 1
				&& mouseY >= this.scroll
				&& mouseY <= this.scroll + (this.items.size() * 11 < this.height ? this.height : this.height - 2 - (this.items.size() * 11 - this.height)))
			{
	...
	public boolean onMouseDrag(int mouseX, int mouseY, MouseButton button, long timeFromLastClick, boolean onThis)
	{
		if (this.scrolling)
		{
			...
			if (mouseMovement != this.scroll)
			{
				...
				{
					this.mouseScrollOffset = this.mouseScrollOffset + (this.scroll - startingScroll);
