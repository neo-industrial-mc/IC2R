...
public class TextBox extends GuiElement<TextBox> {
  ...
  public TextBox(
      Ic2Gui<?> gui, int x, int y, int width, int height, String text, boolean drawBackground) {
  ...
  public boolean setText(String text, boolean forceLength) {
    ...
    if (this.validator.apply(text) && (text.length() <= this.maxTextLength || forceLength)) {
      ...
      this.text =
          text.length() <= this.maxTextLength ? text : text.substring(0, this.maxTextLength);
  ...
  public void drawBackground(GuiGraphics guiGraphics, int mouseX, int mouseY) {
    ...
    if (this.drawBackground) {
      this.gui.drawColoredRect(
          guiGraphics.pose(), this.x - 1, this.y - 1, this.width + 2, this.height + 2, -6250336);
      this.gui.drawColoredRect(
    ...
    if (!text.isEmpty()) {
      xPos =
          this.gui.drawString(
              ...
              xStartPos,
              yPos,
              validOffset ? text.substring(0, textOffset) : text,
              colour,
              true);
    }

    boolean inStringOrFull =
        this.cursor < this.text.length() || this.text.length() >= this.maxTextLength;
    int xCursorPos = xPos;
    if (!validOffset) {
  ...
  public String getSelectedText() {
    return this.text.substring(
        Math.min(this.cursor, this.selectionEnd), Math.max(this.cursor, this.selectionEnd));
  }
  ...
  protected void setSelectionPos(int position) {
    ...
    int maxPosition =
        this.gui.trimStringToWidth(this.text.substring(this.scrollOffset), width).length()
            + this.scrollOffset;
