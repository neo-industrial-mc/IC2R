...
public class PlayerHead extends ItemImage {
  private static final Map<GameProfile, ItemStack> IMAGE_MAKER =
    ...
    public ItemStack get() {
      try {
        Optional<GameProfile> resolved =
            SkullBlockEntity.fetchGameProfile(this.profile.getName()).get();
        GameProfile resolvedProfile = resolved.orElse(this.profile);
        return PlayerHead.IMAGE_MAKER.computeIfAbsent(
            resolvedProfile,
            p -> {
