...
public class VanillaButton extends Button<VanillaButton> {
  ...
  private static void drawVerticalPiece(
      Ic2Gui<?> gui, PoseStack matrices, int x, int y, int width, int height, int u, int v) {
    int minTop = 2;
    int minBottom = 3;
  ...
  public void drawBackground(GuiGraphics guiGraphics, int mouseX, int mouseY) {
    ...
    while (remainingWidth > 200 - minLeft) {
      ...
      drawVerticalPiece(
          this.gui, guiGraphics.pose(), cx, this.y, cWidth, this.height, u + minLeft, v);
      cx += cWidth;
      remainingWidth -= cWidth;
    }
