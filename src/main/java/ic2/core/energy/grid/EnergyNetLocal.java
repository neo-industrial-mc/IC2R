...
public class EnergyNetLocal {
  ...
  private final Set<IEnergyTile> ioTilesToNotify =
  ...
  IEnergyTile getSubTile(BlockPos pos) {
    ...
    for (GridChange change : this.gridChangesQueue) {
      if (change != QUEUE_DELAY_CHANGE) {
        for (IEnergyTile subtile :
            change.subTiles != null ? change.subTiles : Collections.singletonList(change.ioTile)) {
    ...
    if ((prev = this.gridAdditionsMap.put(ioTile, change)) != null) {
      ...
      if (EnergyNetSettings.logGridUpdateIssues) {
        IC2.log.warn(
            ...
            "Tile %s was attempted to be queued twice for addition.",
            Util.toString(ioTile, this.getWorld(), pos));
      }
    } else {
  ...
  void removeTile(IEnergyTile ioTile, BlockPos pos) {
    ...
    if (addition != null) {
      if (EnergyNetSettings.logGridUpdatesVerbose) {
        IC2.log.debug(
            LogCategory.EnergyNet,
            "Removing tile %s by cancelling a pending addition.",
            Util.toString(ioTile, this.getWorld(), pos));
      }
      ...
      if (tile != null) {
        ...
        IC2.log.warn(
            LogCategory.EnergyNet, "Missing tile %s.", Util.toString(ioTile, this.getWorld(), pos));
      }
    }
  }
