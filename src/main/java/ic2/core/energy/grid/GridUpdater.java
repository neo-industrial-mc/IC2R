        ...
        while (this.busy) {
          ...
          if (remaining <= 0L) {
            IC2.log.error(
                ...
                "GridUpdater.awaitCompletion() timed out after %dms! "
  ...
  public void run() {
    try {
      ...
      IC2.log.error(
          LogCategory.EnergyNet,
          t,
          "Unhandled exception/error in GridUpdater.run(), force-clearing busy flag.");
      this.forceClear();
    }
  }
  ...
  private void updateGrid() {
    ...
    try {
      ...
      while ((change = this.changes.poll()) != null) {
        switch (change.type) {
          case ADDITION:
            ChangeHandler.applyAddition(
                this.enet, change.ioTile, change.pos, change.subTiles, this.changes);
            break;
          case REMOVAL:
      ...
      IC2.log.error(
          LogCategory.EnergyNet,
          t,
          "Unhandled exception/error in GridUpdater.updateGrid(), force-clearing busy flag.");
      this.forceClear();
    }
  }
    ...
    public void run() {
      try {
        ...
        IC2.log.error(
            LogCategory.EnergyNet,
            t,
            "Unhandled exception/error in GridCalcTask.run() for grid %s.",
            this.grid);
      }
    ...
    public void run() {
      try {
        ...
        IC2.log.error(
            LogCategory.EnergyNet,
            t,
            "Unhandled exception/error in GridUpdateTask.run() for grid %s.",
            this.grid);
      }
