...
public class Grid {
  ...
  void add(Node node, Collection<Node> neighbors) {
    if (EnergyNetSettings.logGridUpdatesVerbose) {
      IC2.log.debug(
          LogCategory.EnergyNet, "%d Add %s to %s neighbors: %s.", this.uid, node, this, neighbors);
    }
  ...
  void remove(Node node) {
    if (EnergyNetSettings.logGridUpdatesVerbose) {
      IC2.log.debug(
          ...
          "%d Remove Node %s from %s with %d nodes.",
          this.uid,
          node,
          this,
          this.nodes.size());
    ...
    while (it.hasNext()) {
      ...
      if (neighbor.links.isEmpty() && neighbor.tile.removeExtraNode(neighbor)) {
        if (EnergyNetSettings.logGridUpdatesVerbose) {
          IC2.log.debug(
              LogCategory.EnergyNet, "%d Removing isolated extra node %s.", this.uid, neighbor);
        }
    ...
    if (linkCount == 0) {
      ...
      if (EnergyNetSettings.logGridUpdatesVerbose) {
        IC2.log.debug(
            LogCategory.EnergyNet,
            "%d Neighbor connectivity (%d links, %d new grids):",
            this.uid,
            linkCount,
            gridCount);
        ...
        for (int i = 0; i < linkCount; i++) {
          ...
          if (nodes != null) {
            IC2.log.debug(
                ...
                this.uid,
                i,
                node.links.get(i).getNeighbor(node),
                nodes,
                nodes.size());
          } else {
            IC2.log.debug(
                LogCategory.EnergyNet,
                "%d %d: %s contained in %d.",
                this.uid,
                i,
                node.links.get(i).getNeighbor(node),
                mapping[i]);
      ...
      for (int i = 1; i < linkCount; i++) {
        ...
        if (connectedNodes != null) {
          ...
          if (EnergyNetSettings.logGridUpdatesVerbose) {
            IC2.log.debug(
                LogCategory.EnergyNet,
                "%d Moving %d nodes from net %d to new grid %d.",
                this.uid,
                connectedNodes.size(),
                i,
                grid.uid);
          ...
          for (Node cNode : connectedNodes) {
            ...
            if (needsExtraNode) {
              ...
              if (EnergyNetSettings.logGridUpdatesVerbose) {
                IC2.log.debug(
                    LogCategory.EnergyNet,
                    "%s Create extra Node %d for %s in grid %d.",
                    this.uid,
                    extraNode.uid,
                    cNode,
                    grid.uid);
            ...
            if (EnergyNetSettings.logGridUpdatesVerbose) {
              IC2.log.debug(
                  LogCategory.EnergyNet, "%d Move Node %s to grid %d.", this.uid, cNode, grid.uid);
            }
      ...
      if (node.nodeType != NodeType.Conductor) {
        for (Node node2 : node.tile.nodes) {
          if (node2.nodeType == node.nodeType && node2.getGrid() == this) {
            if (EnergyNetSettings.logGridUpdatesVerbose) {
              IC2.log.debug(
                  LogCategory.EnergyNet, "%d Merge Node %s -> %s.", this.uid, node, node2);
            }
  ...
  GridInfo getInfo() {
    ...
    return new GridInfo(
        this.uid, this.nodes.size(), complexNodes, minX, minY, minZ, maxX, maxY, maxZ);
  }
  ...
  void dumpNodeInfo(Node node, PrintStream console, PrintStream chat) {
    ...
    chat.printf(
        ...
        " ",
        Util.formatPosition(
            EnergyNet.instance.getWorld(ioTile), EnergyNet.instance.getPos(ioTile)));
