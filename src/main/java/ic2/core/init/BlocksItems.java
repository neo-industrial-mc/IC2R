...
public class BlocksItems
{
	...
	{
		Info.POTION_RADIATION = IC2Potion.radiation = new IC2Potion("radiation", true, 5149489);
	}
	...
	private static void initBlocks()
	{
		...
		new Ic2Leaves();
		OreDictionary.registerOre("woodRubber", new BlockRubWood());
	...
	{
		ArmorMaterial bronzeArmorMaterial = EnumHelper.addArmorMaterial("IC2_BRONZE", "IC2_BRONZE", 15, new int[] { 2, 5, 6, 2 }, 9, null, 0.0F);
		ArmorMaterial alloyArmorMaterial = EnumHelper.addArmorMaterial("IC2_ALLOY", "IC2_ALLOY", 50, new int[] { 4, 7, 9, 4 }, 12, null, 2.0F);
		new ItemArmorAdvBatpack();
		new ItemArmorIC2(
			ItemName.alloy_chestplate, alloyArmorMaterial, "alloy", EntityEquipmentSlot.CHEST, ItemName.crafting.getItemStack(CraftingItemType.alloy)
		);
		...
		ItemMulti<NuclearResourceType> nuclearResource = new ItemNuclearResource();
		...
		new Tfbp();
		ToolMaterial bronzeToolMaterial = EnumHelper.addToolMaterial("IC2_BRONZE", 2, 350, 6.0F, 2.0F, 13)
			.setRepairItem(ItemName.ingot.getItemStack(IngotResourceType.bronze));
		...
		new ItemCable();
		ItemMulti<ItemUpgradeModule.UpgradeType> upgrade = new ItemUpgradeModule();
	...
	private static void registerIC2fluid(
		FluidName name, Material material, int color, int density, int viscosity, int luminosity, int temperature, boolean isGaseous
	)
	{
		...
		{
			Block block = new BlockIC2Fluid(name, fluid, material, color);
	...
	public static <T extends Block> void registerBlock(T item, ResourceLocation rl)
	{
		item.setRegistryName(rl);
		registerBlock(item);
	}

	public static <T extends Block> void registerBlock(T item)
	{
