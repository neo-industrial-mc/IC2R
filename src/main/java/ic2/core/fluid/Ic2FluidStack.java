...
public interface Ic2FluidStack {
  ...
  static Ic2FluidStack get(ItemStack stack) {
    ...
    return item instanceof Ic2FluidItem
        ? ((Ic2FluidItem) item).getFluidStack(stack)
        : FluidHandler.ENV_HANDLER.getFluidStack(stack);
  }

  static Ic2FluidStack[] getAll(ItemStack stack) {
    Item item = stack.getItem();
    return item instanceof Ic2FluidItem
        ? new Ic2FluidStack[] {((Ic2FluidItem) item).getFluidStack(stack)}
        : FluidHandler.ENV_HANDLER.getFluidStacks(stack);
  }
  ...
  default String getFluidTypeKey() {
    // Returns a string like: `fluid_type.ic2.xxx` for the fluid type, or null if the stack is
    // empty.
    if (isEmpty()) return null;

    return "fluid_type."
        + String.valueOf(BuiltInRegistries.FLUID.getKey(getFluid())).replace(':', '.');
  }
