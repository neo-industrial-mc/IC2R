...
public enum SubPacketType {
  ...
  public static SubPacketType read(GrowingBuffer in, boolean simulating) {
    ...
    if (id < 0 || id >= values.length) {
      IC2.log.warn(LogCategory.Network, "Invalid sub packet type: %d", new Object[] { Integer.valueOf(id) });
      return null;
    } 
    SubPacketType ret = values[id];
    if ((simulating && !ret.clientToServer) || (!simulating && !ret.serverToClient)) {
      IC2.log.warn(LogCategory.Network, "Invalid sub packet type %s for side %s", new Object[] { ret.name(), simulating ? "server" : "client" });
      return null;
    } 
    return ret;
  }
