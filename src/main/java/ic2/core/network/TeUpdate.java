          ...
          for (ServerPlayer player : playersInRange) {
            ...
            if (fieldCount != 0) {
              ...
              if (playerBuffer == null) {
                ...
                DataEncoder.encode(
                    playerBuffer, Util.getDimId(player.getCommandSenderWorld()), false);
              }
  ...
  static void receive(GrowingBuffer buffer) throws IOException {
    final ResourceLocation worldId =
        DataEncoder.getValue(
            DataEncoder.decode(buffer, DataEncoder.EncodedType.ResourceLocation), null);
  ...
  private static void apply(TeUpdateDataClient.TeData update, Level world) {
    if (!world.hasChunkAt(update.pos)) {
      if (debug) {
        IC2.log.info(
            LogCategory.Network,
            "Skipping update at %s, chunk not loaded.",
            Util.formatPosition(world, update.pos));
      }
    } else {
      ...
      if (update.teType != null && state.getBlock() != update.teType) {
        ...
        if (update.hasClass()
            && (te == null
                || te.getClass() != update.teType.getTeClass()
                || te.isRemoved()
                || te.getLevel() != world)) {
          if (debug) {
            IC2.log.info(
                LogCategory.Network,
                "Instantiating %s with %s.",
                Util.formatPosition(world, update.pos),
                update.teType.getTeClass().getName());
          }
          ...
          if (te == null) {
            if (debug) {
              IC2.log.info(
                  LogCategory.Network,
                  "Can't apply update at %s, no te and no teClass.",
                  Util.formatPosition(world, update.pos));
            }
          ...
          if (te.isRemoved() || te.getLevel() != world) {
            if (debug) {
              IC2.log.warn(
                  LogCategory.Network,
                  "Can't apply update at %s, invalid te and no teClass.",
                  Util.formatPosition(world, update.pos));
            }
