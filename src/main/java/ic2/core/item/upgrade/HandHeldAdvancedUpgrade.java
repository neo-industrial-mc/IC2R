...
public class HandHeldAdvancedUpgrade extends HandHeldInventory
    implements IHolographicSlotProvider, IGuiConditionProvider {
  ...
  @GuiSynced protected boolean meta;
  @GuiSynced protected boolean energy;
  @ClientModifiable protected NbtSettings nbt;
  ...
  private static ItemStack checkContainerStack(Player player, ItemStack containerStack) {
    if (!player.getCommandSenderWorld().isClientSide
        ...
        && ((ContainerHandHeldInventory<?>) player.containerMenu).base
            instanceof HandHeldUpgradeOption) {
      addMaintainedPlayer(player);
      return ((HandHeldInventory) ((ContainerHandHeldInventory<?>) player.containerMenu).base)
          .getContainerStack();
    } else {
  ...
  public ContainerBase<?> createServerScreenHandler(int syncId, Player player) {
    return DynamicHandHeldContainer.create(
        Ic2ScreenHandlers.ADVANCED_UPGRADE, syncId, player.getInventory(), this, this.getNode());
  }
  ...
  public ContainerBase<?> createClientScreenHandler(
    ...
    return DynamicHandHeldContainer.create(
        Ic2ScreenHandlers.ADVANCED_UPGRADE,
        syncId,
