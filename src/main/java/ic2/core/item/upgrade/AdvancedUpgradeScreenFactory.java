...
public final class AdvancedUpgradeScreenFactory
  ...
  public AbstractContainerScreen<DynamicContainer<HandHeldAdvancedUpgrade>> create(
      DynamicContainer<HandHeldAdvancedUpgrade> container,
      Inventory playerInventory,
      Component title) {
    final DynamicGui<HandHeldAdvancedUpgrade> gui =
        DynamicGui.create(container, playerInventory, title);
    if (Util.inDev()) {
      gui.addElement(
          new VanillaButton(
                  gui,
                  ...
                  50,
                  20,
                  new EnumCycleHandler<>(NbtSettings.VALUES, container.base.nbt) {
                    @Override
              ...
              .withTooltip(
                  new Supplier<>() {
                    private final String NBT =
                        Component.translatable("ic2.upgrade.advancedGUI.nbt").getString();

                    public String get() {
                      return Component.translatable(
                              "ic2.upgrade.advancedGUI.nbt.desc",
                              Component.translatable(container.base.nbt.name),
                              ChatFormatting.GRAY,
                              Component.translatable(container.base.nbt.name + ".desc", this.NBT))
                          .getString();
                    }
                  }));
    }
