...
public class HandHeldCropAnalyzer extends HandHeldInventory {
  ...
  public int getScannedLevel() {
    ...
    return !StackUtil.isEmpty(stack) && stack.getItem() instanceof ICropSeed
        ? ((ICropSeed) stack.getItem()).getScannedFromStack(stack)
        : -1;
  }
  ...
  public int getSeedGrowth() {
    ItemStack stack = this.getAnalyzedStack();
    return !StackUtil.isEmpty(stack) && stack.getItem() instanceof ICropSeed
        ? ((ICropSeed) stack.getItem()).getGrowthFromStack(stack)
        : -1;
  }
  ...
  public int getSeedGain() {
    ItemStack stack = this.getAnalyzedStack();
    return !StackUtil.isEmpty(stack) && stack.getItem() instanceof ICropSeed
        ? ((ICropSeed) stack.getItem()).getGainFromStack(stack)
        : -1;
  }
  ...
  public int getSeedResistance() {
    ItemStack stack = this.getAnalyzedStack();
    return !StackUtil.isEmpty(stack) && stack.getItem() instanceof ICropSeed
        ? ((ICropSeed) stack.getItem()).getResistanceFromStack(stack)
        : -1;
  }
  ...
  public void tryScan() {
    ...
    try {
      ...
      if (!StackUtil.isEmpty(output)
          || StackUtil.isEmpty(input)
          || !(input.getItem() instanceof ICropSeed seed)) {
        return;
      }
