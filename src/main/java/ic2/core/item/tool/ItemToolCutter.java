...
public class ItemToolCutter extends ItemToolCrafting implements IEnhancedOverlayProvider {
  ...
  public @NotNull InteractionResult useOn(UseOnContext context) {
    ...
    if (state.getBlock() instanceof CableBlock cable) {
      ...
      if (StackUtil.consumeFromPlayerInventory(player, request, 1, true)
          && cable.tryAddInsulation(state, world, pos)) {
  ...
  public void removeInsulation(
      Player player, InteractionHand hand, BlockState state, Level world, BlockPos pos) {
    CableBlock cable = (CableBlock) state.getBlock();
    if (cable.tryRemoveInsulation(state, world, pos, true)
        && StackUtil.damage(player, hand, StackUtil.sameItem(this), 3)) {
