...
public abstract class HandHeldInventory implements IHasGui {
  ...
  public HandHeldInventory(
      Player player, InteractionHand hand, ItemStack containerStack, int inventorySize) {
    ...
    if (IC2.sideProxy.isSimulating()) {
      ...
      for (int i = 0; i < contentList.size(); i++) {
        ...
        if (slot >= 0 && slot < this.inventory.length) {
          this.inventory[slot] =
              ItemStack.parseOptional(this.player.level().registryAccess(), slotNbt);
        }
      }
    }
  }
  ...
  public ItemStack removeItem(int index, int amount) {
    ...
    if (index >= 0
        ...
        && !StackUtil.isEmpty(stack = this.inventory[index])) {
      ItemStack ret;
      ...
      if (!this.cleared) {
        ...
        for (int i = 0; i < this.inventory.length; i++) {
          if (!StackUtil.isEmpty(this.inventory[i])) {
            ...
            // 1.21: ItemStack.save no longer writes into the passed tag, it returns the merged
            // result
        ...
        if (dropItself) {
          ...
          if (idx < -1) {
            IC2.log.warn(
                LogCategory.Item,
                "Handheld inventory saving failed for player "
