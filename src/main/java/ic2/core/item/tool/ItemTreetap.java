...
public class ItemTreetap extends Item implements IBoxable {
  ...
  public static boolean attemptExtract(
    ...
    RubberLogBlock.RubberWoodState rwState =
        (RubberLogBlock.RubberWoodState) state.getValue(RubberLogBlock.stateProperty);
    ...
    if (rwState.wet) {
      if (!world.isClientSide) {
        world.setBlockAndUpdate(
            pos, (BlockState) state.setValue(RubberLogBlock.stateProperty, rwState.getDry()));
        if (stacks != null) {
      ...
      if (!world.isClientSide) {
        if (rng.nextInt(5) == 0) {
          world.setBlockAndUpdate(
              ...
              (BlockState)
                  state.setValue(
                      RubberLogBlock.stateProperty, RubberLogBlock.RubberWoodState.plain));
          triggerToolUseEvent(world, pos, player, state, isElectric);
          ret = true;
        }
  ...
  private static void triggerToolUseEvent(
      Level world, BlockPos pos, Player player, BlockState state, boolean isElectric) {
    ...
    world.gameEvent(
        net.minecraft.core.registries.BuiltInRegistries.GAME_EVENT.wrapAsHolder(
            Ic2GameEvents.TOOL_USE),
        pos,
        Context.of(player, null));
  }
  ...
  private static void ejectResin(Level world, BlockPos pos, Direction side, int quantity) {
    ...
    for (int i = 0; i < quantity; i++) {
      ItemEntity entityitem =
          new ItemEntity(world, ejectX, ejectY, ejectZ, new ItemStack(Ic2Items.RESIN));
