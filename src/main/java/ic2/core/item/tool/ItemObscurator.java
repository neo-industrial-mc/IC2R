...
public class ItemObscurator extends BaseElectricItem implements IPlayerItemDataListener
{
	...
	private static final ThreadLocal<ItemObscurator.ExtractingVertexConsumer> testConsumers = new ThreadLocal<ItemObscurator.ExtractingVertexConsumer>()
	{
	...
	public EnumActionResult onItemUseFirst(EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ, EnumHand hand)
	{
		...
		} else if (player.isSneaking() && world.isRemote && ElectricItem.manager.canUse(stack, 20000.0))
		{
	...
	private boolean scanBlock(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumFacing side)
	{
		...
		{
			colorMultipliers[i] = IC2.platform.getColorMultiplier(state, world, pos, renderInfo.tints[i]);
		}
	...
	public static ItemObscurator.ObscuredRenderInfo getRenderInfo(IBlockState state, EnumFacing side)
	{
		...
		ItemObscurator.ExtractingVertexConsumer testConsumer = testConsumers.get();
