...
{
	private static final Map<EntityPlayer, ItemStack> playerArmorBuffer = new WeakHashMap<>();
	...
	{
		return stack != null
			&& EntityLiving.getSlotForItemStack(stack) == EntityEquipmentSlot.CHEST
			&& stack.hasTagCompound()
			&& stack.getTagCompound().getBoolean("hasIC2Jetpack");
	}
	...
	public double charge(ItemStack stack, double amount, int tier, boolean ignoreTransferLimit, boolean simulate)
	{
		...
		double charge = stack.hasTagCompound() ? stack.getTagCompound().getDouble("charge") : 0.0;
	...
	public void renderPost(RenderLivingEvent.Post<EntityLivingBase> event)
	{
		if (render != null)
		{
			ReflectionUtil.<List>getFieldValue(renderLayers, event.getRenderer()).remove(render);
		}
	}
}
