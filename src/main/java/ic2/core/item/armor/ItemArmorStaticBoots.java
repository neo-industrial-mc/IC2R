...
public class ItemArmorStaticBoots extends ItemArmorUtility {
  ...
  public void onArmorTick(World world, EntityPlayer player, ItemStack stack) {
    ...
    double distance = Math.sqrt(((compound.func_74762_e("x") - (int)player.field_70165_t) * (compound.func_74762_e("x") - (int)player.field_70165_t) + (compound.func_74762_e("z") - (int)player.field_70161_v) * (compound.func_74762_e("z") - (int)player.field_70161_v)));
    if (distance >= 5.0D) {
      compound.func_74768_a("x", (int)player.field_70165_t);
      compound.func_74768_a("z", (int)player.field_70161_v);
      ret = (ElectricItem.manager.charge((ItemStack)player.field_71071_by.field_70460_b.get(2), Math.min(3.0D, distance / 5.0D), 2147483647, true, false) > 0.0D);
    } 
    if (ret)
