...
public class ReactorGameTests
{
	...
	// a MOX rod scales its output with hull heat: at 50% heat each pulse is worth 4 * 0.5 + 1 = 3 output units
	@GameTest(template = EMPTY)
	public static void moxRodOutputScalesWithHullHeat(GameTestHelper helper)
	{
		...
		helper.succeedWhen(() ->
		{
			// the work tick adds the rod's own heat before computing output, so compare against the live hull heat
			float expected = 4.0F * ((float) reactor.getHeat() / reactor.getMaxHeat()) + 1.0F;
			helper.assertTrue(reactor.getReactorEnergyOutput() > 3.0F,
				"hot MOX rod should out-produce a cold rod, output is " + reactor.getReactorEnergyOutput());
			Ic2GameTestAssertions.assertNear(helper, reactor.getReactorEnergyOutput(), expected, "MOX rod output at ~50% hull heat");
		});
	}

	// multi-cell rods pulse themselves: a dual rod yields 4 output units and 24 heat, a quad 12 units and 96 heat
	@GameTest(template = EMPTY)
	public static void dualAndQuadRodsScaleOutputAndHeat(GameTestHelper helper)
	{
		...
		helper.succeedWhen(() ->
		{
			Ic2GameTestAssertions.assertNear(helper, dual.getReactorEnergyOutput(), 4.0, "dual rod output");
			Ic2GameTestAssertions.assertNear(helper, quad.getReactorEnergyOutput(), 12.0, "quad rod output");
			helper.assertTrue(dual.getHeat() >= 24 && dual.getHeat() % 24 == 0, "dual rod should add 24 hull heat per work tick, hull has " + dual.getHeat());
			helper.assertTrue(quad.getHeat() >= 96 && quad.getHeat() % 96 == 0, "quad rod should add 96 hull heat per work tick, hull has " + quad.getHeat());
		});
	}
	...
	public static void lithiumRodBreedsIntoTritium(GameTestHelper helper)
	{
		...
		// lithium needs 10000 breeding levels; at 3000 hull heat it gains heat/3000 = 1 per pulse, so start one short
		...
		helper.succeedWhen(() ->
		{
			...
			helper.assertTrue(stack != null && stack.getItem() == Ic2Items.TRITIUM_FUEL_ROD,
	...
	public static void iridiumReflectorBouncesWithoutWear(GameTestHelper helper)
	{
		...
		helper.succeedWhen(() ->
		{
			Ic2GameTestAssertions.assertNear(helper, reactor.getReactorEnergyOutput(), 2.0, "rod output with iridium reflector");
			ItemStack stack = reactor.getItemAt(1, 0);
			helper.assertTrue(stack != null && stack.getItem() == Ic2Items.IRIDIUM_NEUTRON_REFLECTOR,
	...
	public static void wornNeutronReflectorBreaks(GameTestHelper helper)
	{
		...
		{
			ItemStack stack = reactor.getItemAt(1, 0);
			helper.assertTrue(stack == null || stack.isEmpty(), "worn-out reflector should be destroyed, slot has " + stack);
			Ic2GameTestAssertions.assertNear(helper, reactor.getReactorEnergyOutput(), 1.0, "rod output after losing its reflector");
		});
	}
	...
	public static void overfilledCoolantCellShatters(GameTestHelper helper)
	{
		...
		{
			ItemStack stack = reactor.getItemAt(1, 0);
			helper.assertTrue(stack == null || stack.isEmpty(), "overfilled coolant cell should shatter, slot has " + stack);
		});
	}
	...
	public static void condensatorAbsorbsRodHeat(GameTestHelper helper)
	{
		...
		helper.succeedWhen(() ->
		{
			helper.assertTrue(condensator.getUseFraction(reactor.getItemAt(1, 0)) > 0.0, "condensator should hold the rod's heat");
			helper.assertValueEqual(reactor.getHeat(), 0, "hull heat with condensator");
		});
	}

	// the RSH/LZH coolant interfaces recharge nearly-full condensators in the reactor for a resource block and 1000 EU
	@GameTest(template = EMPTY)
	public static void rciRechargesCondensators(GameTestHelper helper)
	{
		TileEntityNuclearReactorElectric reactor = placeReactor(helper);
		// each RCI services the reactor behind its back face
		helper.setBlock(new BlockPos(2, 1, 1), Ic2Blocks.RCI_RSH.defaultBlockState().setValue(Ic2TileEntityBlock.anyFacingProperty, Direction.EAST));
		helper.setBlock(new BlockPos(0, 1, 1), Ic2Blocks.RCI_LZH.defaultBlockState().setValue(Ic2TileEntityBlock.anyFacingProperty, Direction.WEST));
		...
		helper.succeedWhen(() ->
		{
			Ic2GameTestAssertions.assertNear(helper, ((ItemReactorCondensator) Ic2Items.RSH_CONDENSATOR).getUseFraction(reactor.getItemAt(0, 0)), 0.0,
				"RSH condensator heat after recharge");
			Ic2GameTestAssertions.assertNear(helper, ((ItemReactorCondensator) Ic2Items.LZH_CONDENSATOR).getUseFraction(reactor.getItemAt(1, 0)), 0.0,
				"LZH condensator heat after recharge");
			helper.assertTrue(rsh.inputSlot.isEmpty(), "RSH interface should consume its redstone block");
			helper.assertTrue(lzh.inputSlot.isEmpty(), "LZH interface should consume its lapis block");
		});
	}
	...
	public static void reactorHeatExchangerDrainsHull(GameTestHelper helper)
	{
		...
		helper.succeedWhen(() ->
		{
			...
			helper.assertTrue(held >= 72 && held % 72 == 0, "exchanger should pull 72 hull heat per work tick, holds " + held);
			helper.assertValueEqual(reactor.getHeat(), 2000 - held, "hull heat after exchanger transfer");
		});
	}
	...
	public static void componentHeatVentCoolsNeighbors(GameTestHelper helper)
	{
		...
		helper.succeedWhen(() ->
		{
			...
			helper.assertValueEqual(westHeat, eastHeat, "both neighbors should cool at the same rate");
			helper.assertTrue(westHeat < 100 && (100 - westHeat) % 4 == 0, "spread vent should cool 4 heat per work tick, cell holds " + westHeat);
		});
	}

	// an overclocked heat vent pulls 36 hull heat per work tick but only vents 20, banking the difference
	@GameTest(template = EMPTY)
	public static void overclockedVentTradesStorageForHullCooling(GameTestHelper helper)
	{
		...
		helper.succeedWhen(() ->
		{
			...
			helper.assertTrue(drained >= 36 && drained % 36 == 0, "vent should drain 36 hull heat per work tick, drained " + drained);
			helper.assertValueEqual(held, drained / 36 * 16, "vent should bank 16 of every 36 drained heat");
		});
	}

	// a full 5x5x5 pressure vessel flips the reactor to fluid mode: off the energy net, boiling coolant into hot coolant
	...
	public static void pressureVesselBoilsCoolantIntoHotCoolant(GameTestHelper helper)
	{
		...
		int filled = reactor.inputTank.fillMb(Ic2FluidStack.create(Ic2Fluids.COOLANT.still(), 1000), false);
		...
		helper.succeedWhen(() ->
		{
			...
			helper.assertTrue(!reactor.addedToEnergyNet, "fluid-cooled reactor should leave the energy net");
			Ic2GameTestAssertions.assertNear(helper, reactor.getReactorEnergyOutput(), 1.0, "rod output via the redstone port signal");
			helper.assertTrue(reactor.outputTank.hasExactFluid(Ic2Fluids.HOT_COOLANT.still()) && reactor.outputTank.getFluidAmount() > 0,
				"output tank should hold hot coolant, has " + reactor.outputTank.getFluidAmount() + " mB");
			helper.assertTrue(reactor.inputTank.getFluidAmount() < 1000, "input tank coolant should be consumed");
		});
	}
	...
	public static void redstonePortControlsSealedReactor(GameTestHelper helper)
	{
		...
		{
			helper.assertTrue(reactor.isFluidCooled(), "reactor should detect the pressure vessel");
			Ic2GameTestAssertions.assertNear(helper, reactor.getReactorEnergyOutput(), 0.0, "output with an unpowered redstone port");
			helper.setBlock(VESSEL_CENTER.offset(-3, 0, 0), Blocks.REDSTONE_BLOCK);
		});

		helper.succeedWhen(() -> Ic2GameTestAssertions.assertNear(helper, reactor.getReactorEnergyOutput(), 1.0, "output once the redstone port is powered"));
	}
	...
	public static void accessHatchExposesSealedReactorGrid(GameTestHelper helper)
	{
		...
		TileEntityReactorAccessHatch hatch = getTe(helper, VESSEL_CENTER.offset(0, 2, 0), TileEntityReactorAccessHatch.class);

		helper.succeedWhen(() ->
		{
			helper.assertTrue(hatch.getReactorInstance() == reactor, "hatch should find the sealed reactor");
			helper.assertValueEqual(hatch.getContainerSize(), reactor.getContainerSize(), "hatch container size");
			hatch.setItem(0, new ItemStack(Ic2Items.REACTOR_COOLANT_CELL));
			ItemStack stack = reactor.getItemAt(0, 0);
			helper.assertTrue(stack != null && stack.getItem() == Ic2Items.REACTOR_COOLANT_CELL,
				"cell inserted through the hatch should land in grid slot (0,0), slot has " + stack);
		});
	}

	// full hot-coolant loop: fluid port ejects into a liquid heat exchanger whose heat drives a stirling generator
	...
	public static void hotCoolantLoopDrivesLiquidHeatExchanger(GameTestHelper helper)
	{
		...
		helper.setBlock(VESSEL_CENTER.offset(3, 0, 0),
			Ic2Blocks.LIQUID_HEAT_EXCHANGER.defaultBlockState().setValue(Ic2TileEntityBlock.anyFacingProperty, Direction.NORTH));
		helper.setBlock(VESSEL_CENTER.offset(3, 0, -1),
			Ic2Blocks.STIRLING_GENERATOR.defaultBlockState().setValue(Ic2TileEntityBlock.anyFacingProperty, Direction.SOUTH));
		helper.setBlock(VESSEL_CENTER.offset(3, 0, -2), Ic2Blocks.CESU);

		TileEntityReactorFluidPort port = getTe(helper, VESSEL_CENTER.offset(2, 0, 0), TileEntityReactorFluidPort.class);
		TileEntityLiquidHeatExchanger exchanger = getTe(helper, VESSEL_CENTER.offset(3, 0, 0), TileEntityLiquidHeatExchanger.class);
		TileEntityElectricCESU cesu = getTe(helper, VESSEL_CENTER.offset(3, 0, -2), TileEntityElectricCESU.class);
		...
		helper.succeedWhen(() ->
		{
			helper.assertTrue(port.getReactorInstance() == reactor, "fluid port should find the sealed reactor");
			helper.assertTrue(exchanger.getOutputTank().hasExactFluid(Ic2Fluids.COOLANT.still()) && exchanger.getOutputTank().getFluidAmount() > 0,
				"heat exchanger should cool hot coolant back into coolant, has " + exchanger.getOutputTank().getFluidAmount() + " mB");
			helper.assertTrue(cesu.energy.getEnergy() > 0.0, "CESU should bank stirling EU made from reactor heat, has " + cesu.energy.getEnergy());
		});
	}
	...
	private static TileEntityNuclearReactorElectric placeReactor(GameTestHelper helper, BlockPos pos)
	{
		helper.setBlock(pos, Ic2Blocks.NUCLEAR_REACTOR);
		return getTe(helper, pos, TileEntityNuclearReactorElectric.class);
	}

	// hollow 5x5x5 reactor vessel shell around a fully chambered reactor; tests punch ports into the shell afterwards
