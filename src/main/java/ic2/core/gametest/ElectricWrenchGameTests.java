...
public class ElectricWrenchGameTests
{
	...
	{
		return ((Ic2TileEntityBlock) Ic2Blocks.GENERATOR).getFacing(helper.getLevel(), helper.absolutePos(MACHINE_POS));
	}
	...
	public static void electricWrenchRotatesMachine(GameTestHelper helper)
	{
		...
		InteractionResult result = wrench().onItemUseFirst(stack, Ic2GameTestUtil.useOn(helper, player, MACHINE_POS, newFacing));
		...
		helper.assertValueEqual(ElectricItem.manager.getCharge(stack), MAX_CHARGE - 100.0, "charge after rotating");
		helper.succeed();
	}
	...
	public static void electricWrenchRemovesMachine(GameTestHelper helper)
	{
		...
		InteractionResult result = wrench().onItemUseFirst(stack, Ic2GameTestUtil.useOn(helper, player, MACHINE_POS, getMachineFacing(helper)));

		helper.assertValueEqual(result, InteractionResult.SUCCESS, "wrench use result");
		helper.assertValueEqual(ElectricItem.manager.getCharge(stack), MAX_CHARGE - 1000.0, "charge after removing");
		helper.succeedWhen(() ->
		{
	...
	public static void electricWrenchLowChargeCannotRemove(GameTestHelper helper)
	{
		...
		helper.assertValueEqual(ElectricItem.manager.getCharge(stack), 500.0, "charge must be untouched");
		helper.succeed();
	}
	...
	public static void electricWrenchWithoutChargeFails(GameTestHelper helper)
	{
		...
		InteractionResult result = wrench().onItemUseFirst(stack, Ic2GameTestUtil.useOn(helper, player, MACHINE_POS, facingBefore.getClockWise()));
