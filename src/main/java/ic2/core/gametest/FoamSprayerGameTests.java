...
public class FoamSprayerGameTests
{
	...
	private static ItemStack makeFilledSprayer()
	{
		...
		int filled = sprayer().fillMb(stack, Ic2FluidStack.create(Ic2Fluids.CONSTRUCTION_FOAM.still(), CAPACITY), false, null);
		if (filled != CAPACITY)
		{
	...
	public static void sprayerFillsEnclosedCavity(GameTestHelper helper)
	{
		...
		for (int x = 0; x < 3; x++)
		{
			for (int y = 0; y < 9; y++)
			{
				for (int z = 0; z < 3; z++)
				{
					helper.setBlock(new BlockPos(x, y, z), x == 1 && z == 1 && y >= 1 && y <= 7 ? Blocks.AIR : Blocks.DIRT);
				}
			}
		}
		...
		InteractionResult result = sprayer().useOn(Ic2GameTestUtil.useOn(helper, player, new BlockPos(1, 0, 1), Direction.UP));
		...
		helper.assertValueEqual(getFluidAmount(player.getMainHandItem()), CAPACITY - 7 * FLUID_PER_FOAM, "fluid after filling seven blocks");
		helper.succeed();
	}
	...
	public static void sprayerSingleModePlacesOneBlock(GameTestHelper helper)
	{
		...
		helper.assertValueEqual(getFluidAmount(player.getMainHandItem()), CAPACITY - FLUID_PER_FOAM, "fluid after a single foam block");
		helper.succeed();
	}
	...
	public static void sprayerModeSwitchTogglesSingleMode(GameTestHelper helper)
	{
		...
		try
		{
			...
			helper.assertValueEqual(StackUtil.getOrCreateNbtData(stack).getInt("mode"), 1, "mode after first toggle");

			sprayer().use(helper.getLevel(), player, InteractionHand.MAIN_HAND);
			helper.assertValueEqual(StackUtil.getOrCreateNbtData(stack).getInt("mode"), 0, "mode after second toggle");
		} finally
		{
	...
	public static void sprayerConvertsScaffolding(GameTestHelper helper)
	{
		...
		helper.setBlock(pos, Blocks.SCAFFOLDING.defaultBlockState().setValue(ScaffoldingBlock.DISTANCE, 0));
		ServerPlayer player = makePlayer(helper);
		ItemStack stack = makeFilledSprayer();
		player.setItemInHand(InteractionHand.MAIN_HAND, stack);

		InteractionResult result = sprayer().useOn(Ic2GameTestUtil.useOn(helper, player, pos, Direction.NORTH));

		helper.assertValueEqual(result, InteractionResult.SUCCESS, "sprayer use result");
		helper.assertBlockPresent(Ic2Blocks.FOAM, pos);
		helper.assertValueEqual(getFluidAmount(player.getMainHandItem()), CAPACITY - FLUID_PER_FOAM, "fluid after foaming scaffolding");
		helper.succeed();
	}
	...
	public static void sprayerFoamsCable(GameTestHelper helper)
	{
		...
		helper.assertValueEqual(getFluidAmount(player.getMainHandItem()), CAPACITY - FLUID_PER_FOAM, "fluid after foaming a cable");
		helper.succeed();
	}
