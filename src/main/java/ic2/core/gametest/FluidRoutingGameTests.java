...
public class FluidRoutingGameTests
{
	...
	public static void weightedFluidDistributorPrefersFirstPriority(GameTestHelper helper)
	{
		...
		helper.setBlock(MACHINE_POS, Ic2Blocks.WEIGHTED_FLUID_DISTRIBUTOR.defaultBlockState().setValue(Ic2TileEntityBlock.anyFacingProperty, Direction.UP));
		TileEntityWeightedFluidDistributor te = getTe(helper, MACHINE_POS, TileEntityWeightedFluidDistributor.class);
		...
		helper.succeedWhen(() ->
		{
			helper.assertValueEqual(getTankAmount(helper, EAST_POS), 1000, "water in the first priority tank");
			helper.assertValueEqual(getTankAmount(helper, WEST_POS), 0, "water in the second priority tank");
			helper.assertValueEqual(te.fluidTank.getFluidAmount(), 0, "water left in the distributor");
		});
	}
