...
public class FrequencyTransmitterGameTests
{
	...
	public static void transmitterLinksTwoTeleporters(GameTestHelper helper)
	{
		...
		helper.assertTrue(nbt.getBoolean("targetSet"), "first use should store the teleporter position");
		helper.assertValueEqual(new BlockPos(nbt.getInt("targetX"), nbt.getInt("targetY"), nbt.getInt("targetZ")), helper.absolutePos(TP1_POS), "stored target position");
		...
		helper.assertValueEqual(tp1.getTarget(), helper.absolutePos(TP2_POS), "first teleporter's target");
		helper.assertValueEqual(tp2.getTarget(), helper.absolutePos(TP1_POS), "second teleporter's target");
		helper.succeed();
	}
	...
	public static void transmitterCannotLinkTeleporterToItself(GameTestHelper helper)
	{
		...
		helper.assertFalse(getTeleporter(helper, TP1_POS).hasTarget(), "a teleporter must not link to itself");
		helper.succeed();
	}
	...
	public static void transmitterUseInAirUnlinks(GameTestHelper helper)
	{
		...
		helper.assertTrue(nbt.getBoolean("targetSet"), "target should survive the use right after linking");

		stack.getItem().use(helper.getLevel(), player, InteractionHand.MAIN_HAND);
		helper.assertFalse(nbt.getBoolean("targetSet"), "target should be cleared by a later use in the air");
		helper.succeed();
	}
}
