...
public class TeleporterGameTests
{
	...
	public static void teleporterSendsEntityToTargetAndDrainsEnergy(GameTestHelper helper)
	{
		...
		TileEntityElectricMFSU mfsu = getTe(helper, new BlockPos(0, 0, 1), TileEntityElectricMFSU.class);
		...
		helper.succeedWhen(() ->
		{
			helper.assertTrue(pig.blockPosition().distSqr(destination.above()) <= 2.0,
				"pig should arrive above the target teleporter, is at " + pig.blockPosition() + " instead of " + destination.above());
			helper.assertTrue(mfsu.energy.getEnergy() < 1.0, "teleport should consume the stored " + PIG_COST + " EU, left: " + mfsu.energy.getEnergy());
		});
	}
	...
	public static void teleporterWithoutEnergyKeepsEntity(GameTestHelper helper)
	{
		...
		{
			helper.assertTrue(pig.blockPosition().distSqr(start) <= 4.0, "pig must stay at the source teleporter without energy, is at " + pig.blockPosition());
			helper.succeed();
		});
	}
	...
	public static void teleporterIgnoresWeightlessEntities(GameTestHelper helper)
	{
		...
		helper.runAtTickTime(40, () ->
		{
			helper.assertTrue(stand.blockPosition().distSqr(start) <= 4.0, "weightless armor stand must not be teleported, is at " + stand.blockPosition());
			Ic2GameTestAssertions.assertNear(helper, mfsu.energy.getEnergy(), 1000000.0, "no energy may be consumed for a weightless entity");
			helper.succeed();
		});
	}
	...
	public static void teleporterInventoryWeightConfigAffectsPlayerWeight(GameTestHelper helper)
	{
		...
		helper.assertValueEqual(te.getWeightOf(player), 1000, "weight of a player with an empty inventory");
		...
		helper.assertValueEqual(te.getWeightOf(player), 1150, "weight of a player with 1.5 stacks of cobblestone");
		...
		try
		{
			...
			helper.assertValueEqual(te.getWeightOf(player), 1000, "weight with teleporterUseInventoryWeight disabled");
		} finally
		{
