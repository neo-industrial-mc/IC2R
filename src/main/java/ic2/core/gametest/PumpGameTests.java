...
public class PumpGameTests
{
	...
	public static void overclockedPumpDrainsWaterSourceIntoBucket(GameTestHelper helper)
	{
		...
		{
			helper.assertTrue(te.outputSlot.get(0).getItem() == Items.WATER_BUCKET,
				"the overclocked pump should fill the bucket with the pumped water, has " + te.outputSlot.get(0));
			helper.assertTrue(!helper.getBlockState(WATER_POS).is(Blocks.WATER), "the water source block should be drained");
		});
	}
	...
	public static void pumpDrainsLavaSourceAcrossAirGap(GameTestHelper helper)
	{
		...
		helper.succeedWhen(() ->
		{
			helper.assertTrue(te.outputSlot.get(0).getItem() == Items.LAVA_BUCKET,
				"the pump should fill the bucket with the pumped lava, has " + te.outputSlot.get(0));
			helper.assertTrue(!helper.getBlockState(lavaPos).is(Blocks.LAVA), "the lava source block should be drained");
		});
	}
