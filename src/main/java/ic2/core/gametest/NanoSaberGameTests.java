...
public class NanoSaberGameTests
{
	...
	public static void nanosaberActivatesWhenCharged(GameTestHelper helper)
	{
		...
		helper.assertTrue(AbstractItemNanoSaber.isActive(saber), "charged saber should activate on use");

		saber.getItem().use(helper.getLevel(), player, InteractionHand.MAIN_HAND);
		helper.assertFalse(AbstractItemNanoSaber.isActive(saber), "second use should deactivate the saber");
	...
	public static void nanosaberAttackDrainsEnergy(GameTestHelper helper)
	{
		...
		player.setItemInHand(InteractionHand.MAIN_HAND, saber);
		saber.getItem().use(helper.getLevel(), player, InteractionHand.MAIN_HAND);
		helper.assertTrue(AbstractItemNanoSaber.isActive(saber), "saber should be active before attacking");

		LivingEntity target = helper.spawn(EntityType.PIG, new BlockPos(1, 1, 1));
		saber.getItem().hurtEnemy(saber, target, player);

		helper.assertValueEqual(ElectricItem.manager.getCharge(saber), MAX_CHARGE - HIT_COST, "charge after one hit");
		helper.assertTrue(AbstractItemNanoSaber.isActive(saber), "saber should stay active while charged");
	...
	public static void nanosaberDeactivatesWhenDepleted(GameTestHelper helper)
	{
		...
		player.setItemInHand(InteractionHand.MAIN_HAND, saber);
		saber.getItem().use(helper.getLevel(), player, InteractionHand.MAIN_HAND);
		helper.assertTrue(AbstractItemNanoSaber.isActive(saber), "saber with 300 EU should still activate");

		LivingEntity target = helper.spawn(EntityType.PIG, new BlockPos(1, 1, 1));
		saber.getItem().hurtEnemy(saber, target, player);

		helper.assertFalse(AbstractItemNanoSaber.isActive(saber), "saber should turn off when it cannot pay the hit cost");
