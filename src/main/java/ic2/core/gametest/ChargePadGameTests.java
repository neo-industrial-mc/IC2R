...
public class ChargePadGameTests
{
	...
	private static void runChargePadTest(GameTestHelper helper, Block padBlock, double output)
	{
		...
		helper.onEachTick(() -> helper.getBlockState(pos).entityInside(helper.getLevel(), helper.absolutePos(pos), player));
		...
		helper.succeedWhen(() ->
		{
			...
			helper.assertTrue(charge >= target, "worn lappack should charge at the pad's rate, has " + charge + " of " + target);
			Ic2GameTestAssertions.assertNear(helper, pad.energy.getEnergy() + charge, initialEnergy, "pad energy plus item charge");
