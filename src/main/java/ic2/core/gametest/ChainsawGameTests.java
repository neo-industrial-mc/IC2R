...
public class ChainsawGameTests
{
	...
	public static void chainsawShearsWoolBlock(GameTestHelper helper)
	{
		...
		InteractionResult result = chainsaw().onBlockStartBreak(player, helper.getLevel(), InteractionHand.MAIN_HAND, helper.absolutePos(WOOL_POS), Direction.UP);

		helper.assertValueEqual(result, InteractionResult.SUCCESS, "shear result");
		helper.assertValueEqual(ElectricItem.manager.getCharge(stack), MAX_CHARGE - USE_COST, "charge after shearing");
		helper.succeedWhen(() ->
		{
	...
	public static void chainsawModeSwitchDisablesShearing(GameTestHelper helper)
	{
		...
		helper.assertTrue(StackUtil.getOrCreateNbtData(stack).getBoolean("disableShear"), "mode switch should disable shearing");
		...
		helper.assertValueEqual(ElectricItem.manager.getCharge(stack), MAX_CHARGE, "charge must be untouched with shearing disabled");
		helper.succeed();
	}
	...
	public static void chainsawShearsSheep(GameTestHelper helper)
	{
		...
		InteractionResult result = chainsaw().interactLivingEntity(stack, player, sheep, InteractionHand.MAIN_HAND);
		...
		helper.assertValueEqual(ElectricItem.manager.getCharge(stack), MAX_CHARGE - USE_COST, "charge after shearing a sheep");
		helper.succeedWhen(() -> helper.assertItemEntityPresent(Items.WHITE_WOOL, new BlockPos(1, 1, 1), 2.0));
	}
	...
	public static void chainsawAttackCostsEnergy(GameTestHelper helper)
	{
		...
		helper.assertValueEqual(ElectricItem.manager.getCharge(stack), MAX_CHARGE - USE_COST, "charge after one attack");
		helper.succeed();
	}
	...
	public static void chainsawMiningCostsEnergy(GameTestHelper helper)
	{
		...
		helper.assertValueEqual(ElectricItem.manager.getCharge(stack), MAX_CHARGE - USE_COST, "charge after mining a log");
		helper.succeed();
	}
}
