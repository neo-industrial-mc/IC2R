...
public class TeslaCoilGameTests
{
	...
	// the coil shocks every 32 powered ticks for energy/400 damage split between all targets in a 4 block radius
	...
	public static void teslaCoilShocksNearbyMobWhenPowered(GameTestHelper helper)
	{
		...
		helper.succeedWhen(() ->
		{
			helper.assertFalse(pig.isAlive(), "a 8000 EU discharge deals ~19 damage and should kill the pig");
			helper.assertTrue(te.getComponent(Energy.class).getEnergy() < 8000.0 - 400.0,
				"the shock should consume 400 EU per point of damage, energy is " + te.getComponent(Energy.class).getEnergy());
		});
	}
	...
	public static void teslaCoilStaysIdleWithoutRedstone(GameTestHelper helper)
	{
		...
		{
			helper.assertTrue(pig.isAlive() && pig.getHealth() >= pig.getMaxHealth(), "an unpowered tesla coil must not hurt anything");
			Ic2GameTestAssertions.assertNear(helper, te.getComponent(Energy.class).getEnergy(), 8000.0, "an unpowered tesla coil must not consume energy");
			helper.succeed();
		});
	}
