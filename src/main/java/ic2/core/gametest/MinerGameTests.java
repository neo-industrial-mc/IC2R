...
public class MinerGameTests
{
	...
	public static void minerDigsDownPlacingMiningPipe(GameTestHelper helper)
	{
		...
		te.drillSlot.put(0, ElectricItemManager.getCharged(Ic2Items.DIAMOND_DRILL, Double.POSITIVE_INFINITY));
		...
		helper.succeedWhen(() ->
		{
			helper.assertTrue(helper.getBlockState(new BlockPos(1, 5, 1)).is(Ic2Blocks.MINING_PIPE),
				"the first dug level should hold a mining pipe, is " + helper.getBlockState(new BlockPos(1, 5, 1)));
			helper.assertValueEqual(countItems(te.buffer, Items.DIRT), 2, "dirt stored in the miner buffer");
		});
	}
	...
	private static void scannerMinesSideOre(GameTestHelper helper, Item scanner)
	{
		...
		helper.succeedWhen(() ->
		{
			helper.assertTrue(helper.getBlockState(new BlockPos(0, 5, 1)).isAir(), "the scanner should have located and mined the iron ore");
			helper.assertValueEqual(countItems(te.buffer, Items.RAW_IRON), 1, "raw iron stored in the miner buffer");
		});
	}
	...
	public static void minerWithoutDrillWithdrawsPipes(GameTestHelper helper)
	{
		...
		helper.succeedWhen(() ->
		{
			helper.assertTrue(helper.getBlockState(new BlockPos(1, 5, 1)).isAir(), "the withdrawn pipe should be removed from the world");
			helper.assertTrue(helper.getBlockState(new BlockPos(1, 4, 1)).isAir(), "the withdrawn pipe tip should be removed from the world");
			helper.assertValueEqual(countItems(te.buffer, Ic2Items.MINING_PIPE), 2, "mining pipes returned to the miner buffer");
		});
	}
