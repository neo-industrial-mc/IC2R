...
public class ProcessingMachineGameTests
{
	...
	public static void compressorCompressesClayBallsToClay(GameTestHelper helper)
	{
		...
		helper.succeedWhen(() ->
		{
			...
			helper.assertTrue(output.getItem() == Items.CLAY && output.getCount() == 1, "compressor should produce 1 clay, has " + output);
	...
	public static void extractorExtractsResinToRubber(GameTestHelper helper)
	{
		...
		{
			ItemStack output = te.outputSlot.get(0);
			helper.assertTrue(output.getItem() == Ic2Items.RUBBER && output.getCount() == 3, "extractor should produce 3 rubber, has " + output);
	...
	public static void recyclerProducesScrapFromCobblestone(GameTestHelper helper)
	{
		...
		{
			ItemStack output = te.outputSlot.get(0);
			helper.assertTrue(output.getItem() == Ic2Items.SCRAP && output.getCount() >= 1, "recycler should eventually produce scrap, has " + output);
		});
	}

	// recycler blacklist: sticks are on the default balance blacklist, so they are consumed without producing scrap.
	...
	public static void recyclerBlacklistedInputProducesNoScrap(GameTestHelper helper)
	{
		...
		helper.runAtTickTime(90, () ->
		{
			...
			helper.assertTrue(te.outputSlot.get(0).isEmpty(), "blacklisted input must not produce scrap, has " + te.outputSlot.get(0));
			helper.succeed();
		});
	}

	// thermal centrifuge: 48 EU/t over 500 ticks, tier 2; cobblestone -> stone dust requires minHeat 100.
	...
	public static void centrifugeProcessesCobblestoneWhenHot(GameTestHelper helper)
	{
		...
		{
			ItemStack output = te.outputSlot.get(0);
			helper.assertTrue(output.getItem() == Ic2Items.STONE_DUST && output.getCount() == 1, "centrifuge should produce 1 stone dust, has " + output);
	...
	public static void metalFormerExtrudesCopperIngotToCable(GameTestHelper helper)
	{
		...
		{
			ItemStack output = te.outputSlot.get(0);
			helper.assertTrue(output.getItem() == Ic2Items.COPPER_CABLE && output.getCount() == 3, "extruding should produce 3 copper cable, has " + output);
	...
	public static void metalFormerRollsIronIngotToPlate(GameTestHelper helper)
	{
		...
		{
			ItemStack output = te.outputSlot.get(0);
			helper.assertTrue(output.getItem() == Ic2Items.IRON_PLATE && output.getCount() == 1, "rolling should produce 1 iron plate, has " + output);
			helper.assertTrue(te.inputSlot.get(0).isEmpty(), "metal former input should be consumed");
		});
	}
	...
	public static void metalFormerCutsIronCasingToCoin(GameTestHelper helper)
	{
		...
		{
			ItemStack output = te.outputSlot.get(0);
			helper.assertTrue(output.getItem() == Ic2Items.COIN && output.getCount() == 1, "cutting should produce 1 coin, has " + output);
			helper.assertTrue(te.inputSlot.get(0).isEmpty(), "metal former input should be consumed");
		});
	}
	...
	public static void oreWasherWashesCrushedIron(GameTestHelper helper)
	{
		...
		helper.succeedWhen(() ->
		{
			helper.assertTrue(hasOutput(te.outputSlot, Ic2Items.PURIFIED_IRON), "ore washer should produce purified iron, outputs " + describe(te.outputSlot));
			helper.assertTrue(te.inputSlot.get(0).isEmpty(), "ore washer input should be consumed");
		});
	}

	// blast furnace: no EU, needs heat >= 50000 and (compressed) air fluid; crushed iron -> steel ingot + slag.
	// heat and progress are set directly for a deterministic, fast-completing test (recipe duration is 6000 ticks).
	...
	public static void blastFurnaceSmeltsCrushedIronToSteel(GameTestHelper helper)
	{
		...
		helper.succeedWhen(() ->
		{
			helper.assertTrue(hasOutput(te.outputSlot, Ic2Items.STEEL_INGOT), "blast furnace should produce a steel ingot, outputs " + describe(te.outputSlot));
			helper.assertTrue(te.inputSlot.get(0).isEmpty(), "blast furnace input should be consumed");
		});
	}
	...
	public static void blockCutterCutsIronBlockWithSteelBlade(GameTestHelper helper)
	{
		...
		{
			ItemStack output = te.outputSlot.get(0);
			helper.assertTrue(output.getItem() == Ic2Items.IRON_PLATE && output.getCount() == 9, "block cutter should produce 9 iron plates, has " + output);
			helper.assertTrue(te.inputSlot.get(0).isEmpty(), "block cutter input should be consumed");
		});
	}

	// block cutter: an iron blade (hardness 3) is too weak for an iron block (hardness 5), so nothing is cut.
	@GameTest(template = EMPTY, timeoutTicks = 120)
	public static void blockCutterWeakBladeProducesNothing(GameTestHelper helper)
	{
		...
		{
			helper.assertTrue(te.outputSlot.get(0).isEmpty(), "too-weak blade must not cut anything, has " + te.outputSlot.get(0));
			helper.assertValueEqual(te.inputSlot.get(0).getCount(), 1, "block cutter input with weak blade");
			helper.succeed();
		});
	}
	...
	public static void solidCannerFillsTinCansWithFood(GameTestHelper helper)
	{
		...
		{
			ItemStack output = te.outputSlot.get(0);
			helper.assertTrue(output.getItem() == Ic2Items.FILLED_TIN_CAN && output.getCount() == 2, "solid canner should produce 2 filled tin cans, has " + output);
			helper.assertTrue(te.inputSlot.get(0).isEmpty(), "solid canner food input should be consumed");
			helper.assertTrue(te.canInputSlot.get(0).isEmpty(), "solid canner tin cans should be consumed");
		});
	}
	...
	public static void cannerEnrichesWaterWithLapisIntoCoolant(GameTestHelper helper)
	{
		...
		helper.succeedWhen(() ->
		{
			assertTankContains(helper, te.outputTank, Ic2Fluids.COOLANT.still(), 1000, "canner output tank");
	...
	// electrolyzer: 32 EU/t over 200 ticks, tier 2; 40 mB water -> 26 mB hydrogen (down) + 13 mB oxygen (up),
	...
	public static void electrolyzerSplitsWaterIntoHydrogenAndOxygen(GameTestHelper helper)
	{
		...
		te.dischargeSlot.put(0, ElectricItemManager.getCharged(Ic2Items.ADVANCED_RE_BATTERY, Double.POSITIVE_INFINITY));
		int filled = te.getInput().fillMb(Ic2FluidStack.create(net.minecraft.world.level.material.Fluids.WATER, 1000), false);
		...
		helper.succeedWhen(() ->
		{
			assertTankContains(helper, hydrogenTank, Ic2Fluids.HYDROGEN.still(), 26, "tank below the electrolyzer");
			assertTankContains(helper, oxygenTank, Ic2Fluids.OXYGEN.still(), 13, "tank above the electrolyzer");
			helper.assertValueEqual(te.getInput().getFluidAmount(), 1000 - 40, "electrolyzer water left after one operation");
		});
	}

	// fermenter: consumes 4000 hU from an adjacent heat source to turn 20 mB biomass into 400 mB biogas.
	// the electric heat generator emits 10 hU/t per coil, so 10 coils saturate the fermenter's 100 hU/t draw.
	@GameTest(template = EMPTY, timeoutTicks = 400)
	public static void fermenterFermentsBiomassIntoBiogas(GameTestHelper helper)
	{
		...
		int filled = te.getInputTank().fillMb(Ic2FluidStack.create(Ic2Fluids.BIOMASS.still(), 20), false);
	...
	private static boolean hasOutput(ic2.core.block.invslot.InvSlotOutput slot, net.minecraft.world.item.Item item)
	{
