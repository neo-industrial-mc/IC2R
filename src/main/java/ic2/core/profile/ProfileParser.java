...
public class ProfileParser
{
	...
	{
		InputStream is = root.offset("profile.xml").asStream();
		if (is == null)
		{
			return null;
		}
		return parse(root, is);
	}
	...
	public static Profile parse(ProfileTarget root, InputStream is) throws IOException
	{
		...
		String name = null;
		Version style = null;
		HashSet<TextureStyle> textures = new HashSet<>();
		ArrayList<RecipeChange> recipeChanges = new ArrayList<>();
	...
	public static class FolderNode extends ProfileParser.Node
	{
		...
		Set<ProfileTarget> getFiles(ProfileTarget root)
		{
			...
			if (!folder.isFile())
			{
				File[] files1 = folder.asFile().listFiles((FileFilter) new WildcardFileFilter("*.INI", IOCase.INSENSITIVE));
				if (files1 != null)
				{
