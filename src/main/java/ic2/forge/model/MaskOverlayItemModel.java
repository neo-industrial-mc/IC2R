...
public final class MaskOverlayItemModel implements Ic2Model, BakedModel {
          ...
          if (nbt != null) {
            ...
            if (state != null && side != null && colorMultipliers != null) {
              ItemObscurator.ObscuredRenderInfo renderInfo =
                  ItemObscurator.getRenderInfo(state, side);
              if (renderInfo != null
                  && colorMultipliers.length * 4 == renderInfo.uvs.length
                  && !MaskOverlayItemModel.this.overlayTemplates.isEmpty()) {
                overlayQuads =
                    MaskOverlayItemModel.this.buildOverlayQuads(renderInfo, colorMultipliers);
              }
            }
          }

          return new OverlaidModel(
              MaskOverlayItemModel.this.baseModel,
              MaskOverlayItemModel.this.getDefaultQuads(),
              overlayQuads);
        }
      };

  public MaskOverlayItemModel(
