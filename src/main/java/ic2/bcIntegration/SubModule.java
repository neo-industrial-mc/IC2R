   ...
   public static boolean init() {
      if (ConfigUtil.getBool(MainConfig.get(), "compat/buildcraft/enableBuildcraftFuels")) {
         TileEntityCanner.addBottleRecipe(
            ItemName.crop_res.getItemStack(CropResItemType.oil_berry), 4, ItemName.fluid_cell.getItemStack(), ItemName.fluid_cell.getItemStack("oil")
         );

         for (BCFluid bcFluid : BCEnergyFluids.crudeOil) {
            if (Recipes.semiFluidGenerator.getFuelProperty(bcFluid) == null) {
               Recipes.semiFluidGenerator.addFluid(bcFluid.getName(), 16L, 8L);
            }

            if (Recipes.fluidHeatGenerator.getBurnProperty(bcFluid) == null) {
