...
abstract class BasicEnergyTile implements ILocatable, IEnergyTile {
  ...
  public void readFromNBT(NBTTagCompound tag) {
    NBTTagCompound data = tag.func_74775_l(getNbtTagName());
    setEnergyStored(data.func_74769_h("energy"));
  }
  
  public NBTTagCompound writeToNBT(NBTTagCompound tag) {
    NBTTagCompound data = new NBTTagCompound();
    data.func_74780_a("energy", getEnergyStored());
    tag.func_74782_a(getNbtTagName(), (NBTBase)data);
    return tag;
  }
  ...
  public boolean useEnergy(double amount) {
    if (!canUseEnergy(amount) || (getWorldObj()).field_72995_K)
      return false; 
  ...
  public boolean charge(ItemStack stack) {
    if (stack == null || !Info.isIc2Available() || (getWorldObj()).field_72995_K)
      return false; 
    double energyStored = getEnergyStored();
