...
public class DynamicCategory extends IORecipeCategory<IORecipeWrapper> {
  ...
  public DynamicCategory(
      Ic2TileEntityBlock teBlock, RecipeType<IORecipeWrapper> recipeType, IGuiHelper guiHelper) {
    super(teBlock);
  ...
  static void parseWidgets(
      Ic2TileEntityBlock block,
      IGuiHelper guiHelper,
      GuiParser.ParentNode parentNode,
      List<Tuple.T2<IDrawable, SlotPosition>> elements,
      List<SlotPosition> inputSlots,
      List<SlotPosition> outputSlots) {
    label99:
        ...
        case energygauge:
          {
            ...
            SlotPosition pos =
                new SlotPosition(
                    node.x + node.style.properties.bgXOffset(),
                    node.y + node.style.properties.bgYOffset());
            ...
            IDrawableAnimated energyAnimated =
                guiHelper.createAnimatedDrawable(
                    ...
                    node.style.properties.reverse()
                        ? (node.style.properties.vertical()
                            ? StartDirection.TOP
        ...
        case gauge:
          {
            ...
            SlotPosition pos =
                new SlotPosition(node.x + properties.bgXOffset(), node.y + properties.bgYOffset());
            IDrawableStatic guageBackground =
                guiHelper.createDrawable(
                    properties.texture(),
                    properties.uBgActive(),
                    properties.vBgActive(),
                    properties.bgWidth(),
                    properties.bgHeight());
            ...
            guageBackground =
                guiHelper.createDrawable(
                    ...
                    properties.uInner(),
                    properties.vInner(),
                    properties.innerWidth(),
                    properties.innerHeight());
        ...
        case image:
          {
            ...
            IDrawable image =
                guiHelper
                    .drawableBuilder(node.src, node.u1, node.v1, node.width, node.height)
        ...
        case slotgrid:
          {
            ...
            if (dummyTe == null) {
              throw new NullPointerException(
                  "Received null dummy for " + block + " in the JeiPlugin.");
            }

            InvSlot slot = dummyTe.getInventorySlot(node.name);
            if (slot == null) {
              throw new RuntimeException(
                  "invalid invslot name " + node.name + " for base " + dummyTe);
            }
            ...
            if (size > node.offset) {
              ...
              for (int i = 0; i < dim.cols(); i++) {
                for (int j = 0; j < dim.rows(); j++) {
                  ...
                  SlotPosition posx =
                      new SlotPosition(
                          node.x + i * node.style.width(),
                          node.y + j * node.style.height(),
                          node.style);
  ...
  private void initializeWidgets(IGuiHelper guiHelper, GuiParser.ParentNode parentNode) {
    parseWidgets(
        this.block, guiHelper, parentNode, this.elements, this.inputSlots, this.outputSlots);
  }
  ...
  private int calcSize(boolean isX) {
    ...
    for (Tuple.T2<IDrawable, SlotPosition> element : this.elements) {
      ...
      max =
          Math.max(
              max,
              (isX ? element.b.x() : element.b.y())
                  + (isX ? element.a.getWidth() : element.a.getHeight()));
    }
    return max - min;
  }
  ...
  public void setRecipe(
      ...
      @NotNull IORecipeWrapper recipe,
