...
| 2 | 配方主路径走 `RecipeManager` | **partial** | **macerator 全链路试点 done**：JSON `data/ic2r/recipes/macerator/*.json`（`type: ic2r:macerator`）→ `Ic2rRecipeTypes`/`Serializers` → vanilla `RecipeManager` → `RecipeManagerMachineBridge.loadBasic` → `RecipeManagerGetter` → `Recipes.macerator`（见 `Rezepte.basicRecipe`）。同类 basic 机（extractor/compressor/…）共用 bridge。**未全量**：运行时匹配仍经 materialize 后的 `BasicMachineRecipeManager`，非每次 tick 直查 `RecipeManager`；`RecipeManagerMachineBridge` 类本身 0%（需 Level/RM）；`MachineRecipeMatchMath` 100% 纯逻辑测（6 测）；非 basic / 特殊管理器路径仍异构 | W2.3 |
