...
- `ForgePlatformServices.install()` + `PlatformLifecycleForge` 已接（W3.2）；**lifecycle 以外 facet 多为 stub**  
- 业务仍大量经 `IC2R.envProxy` / `EnvProxyForge`（W3.3 仅退役 `isClientEnv`）  
- **尚无** Gradle 子项目 `ic2r-common` / `ic2r-neoforge`
