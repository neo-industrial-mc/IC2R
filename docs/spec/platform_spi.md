...
> **状态**：**G3.5 / E2 done** — 已删 `EnvProxy#isForgeEnv`/`isFabricEnv`/`getServer`；标签等调用点改 `PlatformLifecycle#getLoaderKind`；`getServer` 调用点（G3.2）+ 方法表面（G3.5）均已切 SPI。W3.3 已删 `isClientEnv`。注册主路径仍双轨 EnvProxy（E3）。  
...
> **下一**：E3 注册调用点 / SPI 非 stub（Network、Item…）。  
...
| **未做** | 注册族 / 流体物品工厂（E3+） | 仍 EnvProxy 双轨 |

**非目标（禁止扩 scope）**：

- W3.3 / G3.5 只瘦 **一组** 环境判定；不全库拆 EnvProxy  
- 不拆多模块 / Architectury  
